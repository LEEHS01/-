...
export default {
  ...
  actions: {
    async fetchMariadbData ({ commit }) {
      try {
        const response = await axios.post(`${process.env.VUE_APP_HOST_IP}/api/mariadb`)
    ...
    // 예측 결과 axios 요청
    async fetchPredictionData ({ commit }, data) {
      const areaName = data
      try {
        const response = await axios.post(`${process.env.VUE_APP_HOST_IP}/api/analyPrediction`, { search: areaName })
        const data = response.data
    ...
    async fetchGetPumpData ({ commit }, data) {
      const areaName = data
      try {
        const response = await axios.post(`${process.env.VUE_APP_HOST_IP}/api/analyPump`, { search: areaName })
        const data = response.data
