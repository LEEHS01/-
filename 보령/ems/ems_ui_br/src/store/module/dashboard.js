...
const dashboard = {
  ...
  state: {
    ...
    zoneList: null,
    pumps: [
      { name: '보령(정)1', status: 1, pwr: 0 },
      { name: '보령(정)2', status: 0, pwr: 0 },
      { name: '보령(정)3', status: 0, pwr: 0 },
      { name: '보령(정)4', status: 1, pwr: 0 },
      { name: '보령(정)5', status: 1, pwr: 0 },
      { name: '보령(정)6', status: 1, pwr: 0 }
    ],
    valveData: [],
    nowUse: 0,
    yesterdayUse: 0,
  ...
  mutations: {
    ...
    setZoneList (state, data) {
      ...
      try {
        if (state.zoneList !== null && state.zoneList !== undefined && state.zoneList.length !== 0) {
          ...
          const tempZonePer = {}
          state.zoneList.forEach(zone => {
             //tempZonePer['Z' + zone.zone_name] = (zone.y === 0) ? 0 : (zone.y * 100 / sumY).toFixed(1)
            tempZonePer[zone.zone_name] = (zone.y === 0) ? 0 : (zone.y * 100 / sumY).toFixed(1)
          })
    ...
    setPumpStatus (state, data) {
      data.data1.forEach((pump, idx) => {
        ...
        const tempKey = '펌프' + pump.NAME[9] + '전력'
        if (data.data6 === null || data.data6.length === 0) {
          state.pumps[idx].pwr = 0
        } else if (Object.keys(data.data6[0]).includes(tempKey)) {
          state.pumps[idx].pwr = Number(data.data6[0][tempKey]).toFixed(0)
          // console.log(tempKey, state.pumps[idx].pwr)
        } else {
          state.pumps[idx].pwr = 0
        }
      })
  ...
  actions: {
    ...
    async getZoneUseList ({ commit }) {
      ...
      axios
        .post(
          `${process.env.VUE_APP_HOST_IP}/api/selectZoneUseList`,
          null,
          { params: param }
        )
        .then((resp) => {
    ...
    async getPumpStatus ({ commit }) {
      ...
      try {
        ...
        const resp = await axios
          .post(`${process.env.VUE_APP_HOST_IP}/api/analyPump`, param)
        if (resp.data !== null) {
