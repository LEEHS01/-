  ...
  setup (props, { emit }) {
    ...
    const fetchZoneData = async () => {
      try {
        ...
        if (zoneListData.value && zoneListData.value.length > 0) {
          ...
          if (Object.keys(route.query).includes('ZONE_NAME')) {
            const queryIdx = zoneData.value.indexOf(route.query.ZONE_NAME)
    ...
    const selectZone = async (index) => {
      try {
        ...
        await store.dispatch('facUse/fetchSelectFacSunsi', param)
