...
export default {
  ...
  setup () {
    ...
    const format = (date) => {
      ...
      /*  
        try {
          ...
          const month = lastMonthDate.getMonth() + 1; // 0부터 시작하므로 1을 더합니다.
          const year = lastMonthDate.getFullYear();

          return `${year}년 ${setTwoWord(month)}월`;
        } catch (err) {
          console.log(err);
        }
      */  
    ...
    const getMonthRateData = (date) => {
      ...
      // 현재 연도와 월을 가져옵니다.
      let year = date.year;
      let month = date.month + 1; // month는 0부터 시작하므로 1을 더합니다.

      // 1달 전 계산
      if (month === 1) {
          year -= 1; // 1월이면 연도를 줄입니다.
          month = 12; // 12월로 설정합니다.
      } else {
          month -= 1; // 월에서 1을 뺍니다.
      }
      ...
      // 두 자리 숫자로 변환
      const formattedMonth = setTwoWord(month);
      console.log(year)
      console.log(formattedMonth)
      
      // Vuex 액션 호출
      store.dispatch('cost/fetchMonthRateData', `${year}-${formattedMonth}`);
      */
    }
