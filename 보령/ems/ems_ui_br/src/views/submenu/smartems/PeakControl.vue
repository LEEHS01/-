  ...
  <div class="contents-body">
    ...
    <div style="width: 100%; height: calc(100% - 50px);">
      ...
      <div class=" detail_R_wrap fL" style="margin-top:23px;height: calc(100% - 23px);width: 65%; ">
        ...
        <div class="detail_R_h48_w45 fL" style="margin-left: 111px;" v-show="false">
          <div class="div_title">펌프사용 전력량 예측</div>
          <div class="detail_R_innerWrap">
            <div class="right_title_div" style="height: 12%;">
              <div ref="tab1Ref" class="div_type_btn div_type_btn_active" style="cursor:pointer" @click="getChart">보령(정)</div>
            </div>
    ...
    const leftData = async () => {
      try {
        ...
        if (searchData.value && searchData.value.length > 0) {
          totalPWI.value =
            (searchData.value[0].총순시전력 !== undefined && searchData.value[0].총순시전력 !== null)
              ? comma(searchData.value[0].총순시전력.toFixed(0))
              : 0
          pumpPWI.value =
            (searchData.value[0].펌프순시전력 !== undefined && searchData.value[0].펌프순시전력 !== null)
              ? comma(searchData.value[0].펌프순시전력.toFixed(0))
              : 0
          tariff.value =
            (searchData.value[0].요금적용전력 !== undefined && searchData.value[0].요금적용전력 !== null)
              ? comma(searchData.value[0].요금적용전력.toFixed(0))
              : 0
          peakGoal.value =
            (searchData.value[0].목표피크전력 !== undefined && searchData.value[0].목표피크전력 !== null)
              ? comma(searchData.value[0].목표피크전력.toFixed(0))
              : 0
        }
      } catch (err) {
        console.log(err)
