  ...
  <div class="cheongso">
    ...
    <div class="fL" style="height: 100%; width: 20%; display: flex; flex-direction: column;">
      <div class="pipe_first_wrap fL" style="margin-top: 30px;">
        <div class="pipe_first_wrap2">
          유입 유량
          <div class="input_box fL"><span class="input_value" >{{ dataList.pipeValueInCD1 }}</span>m3/h</div>
        </div>
      ...
      <div class="pipe_first_wrap fL" style="height: 30%;">
        <div class="pipe_first_wrap2">
          유입 유량
          <div class="input_box fL"><span class="input_value" >{{ dataList.pipeValueInSJ }}</span>m3/h</div>
        </div>
        <div class="pipe_first fL"></div>
      </div>
    ...
    <div class="pipe_center_wrap fL" style="padding-top: 10px;">
      <div class="pipe_center_top fL">
        ...
        <div class="input_box input_box_rate fL" ><span class="input_value_rate" >{{dataList.openRateCD2}}</span>%</div>
      ...
      <div class="pipe_center_top fL">
        ...
        <div v-else class="valve_off fL"></div>
        <div class="input_box input_box_rate fL" ><span class="input_value_rate" >{{dataList.openRateCD3}}</span>%</div>
        <div style="height: 32px"></div>
        <div class="pipe_line fL" >
      ...
      <div class="pipe_center_top fL">
        <div v-if="dataList.openRateSJ0" class="valve_on fL"></div>
        <div v-else class="valve_off fL"></div>
        <!-- <div class="input_box input_box_rate fL" ><span class="input_value_rate" >{{dataList.openRateSJ0}}</span>%</div> -->
        <div style="height: 32px"></div>
        <div class="pipe_line fL" >
          <div v-if="dataList.openRateSJ0" class="pipe_line_arrow blinking" ></div>
        </div>
      </div>
      <div class="pipe_center_top fL">
        <div v-if="dataList.openRateSJ1" class="valve_on fL"></div>
        <div v-else class="valve_off fL"></div>
        <!-- <div class="input_box input_box_rate fL" ><span class="input_value_rate" >{{dataList.openRateSJ1}}</span>%</div> -->
        <div style="height: 32px"></div>
        <div class="pipe_line fL" >
          <div v-if="dataList.openRateSJ1" class="pipe_line_arrow blinking" ></div>
        </div>
      </div>
    </div>
    <!-- -->
    <div class="tank_wrap fL">
        <div class="pipe_last_wrap  fL" style="margin-bottom: 10px;">
          <div class="water_wrap" >
            <div class="input_box input_water_top fL" ><span class="input_value" >{{ dataList.waterLevelCD3 }}</span>m</div>
            <div class="input_box input_water_bottom fL" ><span class="input_value" >{{ dataList.waterLevelCD2 }}</span> m</div>
          </div>
        </div>
        <div class="pipe_last_wrap  fL">
          <div class="water_wrap" >
            <div class="input_box input_water_top fL" ><span class="input_value" >{{ dataList.waterLevelSJ1 }}</span>m</div>
            <div class="input_box input_water_bottom fL" ><span class="input_value" >{{ dataList.waterLevelSJ0 }}</span> m</div>
          </div>
        </div>
    </div>
    ...
    <div class="valve_out_wrap fL"  >
      <div class="valve_out_wrap2 fL" style="height: 35%; padding-top: 65px;">
        유출유량
        <div class="input_box bm10 fL" ><span class="input_value">{{ dataList.pipeValueOutCD }}</span> m3/h</div>
      ...
      <div class="valve_out_wrap2 fL">
        유출유량
        <div class="input_box bm10 fL" ><span class="input_value">{{ dataList.pipeValueOutSJ }}</span> m3/h</div>
        <div class=" pipe_last fL">
          <div class="pipe_last_arrow blinking"></div>
        </div>
      </div>
...
export default ({
  ...
  setup (props) {
    ...
    const updateData = (data) => {
      // 성주 데이터 매칭
      dataList.pipeValueInSJ = data[0].유입유량 !== null ? fixed(data[0].유입유량) : 0.00
      dataList.pipeValueOutSJ = data[0].유출유량 !== null ? fixed(data[0].유출유량) : 0.00
      for (let i = 0; i < 2; i++) {
        dataList[`waterLevelSJ${i}`] = data[i].수위 !== null ? fixed(data[i].수위) : 0.00
      }
      // 창동신(배) 데이터 매칭
      dataList.pipeValueInCD1 = data[2].유입유량 !== null ? fixed(data[1].유입유량) : 0.00
      dataList.pipeValueOutCD = data[2].유출유량 !== null ? fixed(data[1].유출유량) : 0.00
      for (let i = 2; i < data.length; i++) {
        dataList[`openRateCD${i}`] = data[i].개도율 !== null ? fixed(data[i].개도율) : 0.00
        dataList[`waterLevelCD${i}`] = data[i].수위 !== null ? fixed(data[i].수위) : 0.00
      }
    }
    onMounted(() => {
    })
