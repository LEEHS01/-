<template>
  <div :style="pipeStyle" class="pipe_background">
    <div class="pipe_wrap">
      ...
      <div class="pipe_right_wrap">
        <div class="pipe_right_line1" :style="{ fontSize: isNameLong ? '12px' : '14px' }"> {{ info.name }}</div>
        <!-- 유출 유량 -->
...
export default {
  ...
  setup (props) {
    ...
    const isNameLong = computed(() => info.value.name.length >= 8)
