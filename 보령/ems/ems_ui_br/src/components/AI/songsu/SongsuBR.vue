        ...
        <div class="ai_pump_start">
          <div class="ai_pump_start2 fL">
            <div class="ai_pump_start2_2 fL">
              <div class="top_name img_circle fL">보령</div>
            ...
            <div class="fL" style="height: calc(100% - 105px); width: calc(100% - 15px ); margin-left: 15px; ">
              <div class="pump_area_h6 pump_img fL" :style="{ backgroundSize: '40%', opacity: getOpacityValue(pumpStatusA[0]) }">#1</div>
              <div class="pump_area_h6 pump_img fL" :style="{ backgroundSize: '40%', opacity: getOpacityValue(pumpStatusA[1]) }">#2</div>
              <div class="pump_area_h6 pump_img fL" :style="{ backgroundSize: '40%', opacity: getOpacityValue(pumpStatusA[2]) }">#3</div>
              <div class="pump_area_h6 pump_img fL" :style="{ backgroundSize: '40%', opacity: getOpacityValue(pumpStatusA[3]) }">#4</div>
              <div class="pump_area_h6 pump_img fL" :style="{ backgroundSize: '40%', opacity: getOpacityValue(pumpStatusA[4]) }">#5</div>
              <div class="pump_area_h6 pump_img fL" :style="{ backgroundSize: '40%', opacity: getOpacityValue(pumpStatusA[5]) }">#6</div>
            </div>
            <!-- 장치 이미지 구역 끝-->
          </div>
        </div>
      ...
      <div class="fL" style="width: 30%;height: 100%;">
        <div class="" style="width: 100%; height:100%; text-align: center; font-size: 20px; color: white;">
          <div class="pump_area fL">
            ...
            <!-- 상자 중간 부분 시작 -->
            <div class="middle fL">
              <div class="pump_area_h6 pump_img fL" :style="{ backgroundSize: '40%', opacity: getOpacityValue(nowPumpStatusA[0]) }">#1</div>
              <div class="pump_area_h6 pump_img fL" :style="{ backgroundSize: '40%', opacity: getOpacityValue(nowPumpStatusA[1]) }">#2</div>
              <div class="pump_area_h6 pump_img fL" :style="{ backgroundSize: '40%', opacity: getOpacityValue(nowPumpStatusA[2]) }">#3</div>
              <div class="pump_area_h6 pump_img fL" :style="{ backgroundSize: '40%', opacity: getOpacityValue(nowPumpStatusA[3]) }">#4</div>
              <div class="pump_area_h6 pump_img fL" :style="{ backgroundSize: '40%', opacity: getOpacityValue(nowPumpStatusA[4]) }">#5</div>
              <div class="pump_area_h6 pump_img fL" :style="{ backgroundSize: '40%', opacity: getOpacityValue(nowPumpStatusA[5]) }">#6</div>
            </div>
            <!-- 상자 중간 부분 끝 -->
          </div>
        </div>
        ...
        <div class="fL" style="width:60%; height: 97%;">
          <div class="pipe_right_box fL">
            ...
            <div style="width: 100%; height: 100px;">
              <div class="sub_content_middle_value fL sub_content_font" style="font-size: 18px">보령 분기점 현황</div>
              <div class="sub_content_middle_value fL">
                <div class="detail_text" style="font-size: 14px; text-align: center;">홍성(가)유입유량</div>
                <div class="sub_input_box" style="width: 23%; height: 40%;">{{ divergeInfo[0] }}</div>
                <span class="sub_content_font" style="width: 24%;">m³/h</span>
              </div>
              <div class="sub_content_middle_value fL">
                <div class="detail_text" style="font-size: 14px; text-align: center;">청양(가)분기 압력</div>
                <div class="sub_input_box" style="width: 23%; height: 40%;">{{ divergeInfo[1] }}</div>
                <span class="sub_content_font" style="width: 21%;">kg/cm2</span>
              </div>
              <div class="sub_content_middle_value fL">
                <div class="detail_text" style="font-size: 13px; text-align: center;">서천관말분기 압력</div>
                <div class="sub_input_box" style="width: 23%; height: 40%;">{{ divergeInfo[2] }}</div>
                <span class="sub_content_font" style="width: 21%;">kg/cm2</span>
              </div>
              <div class="sub_content_middle_value fL">
                <div class="detail_text" style="font-size: 13px; text-align: center;">송수터널 후단 압력</div>
...
import SongsuWaterBox from '@/components/AI/songsu/SongsuWaterBoxBR.vue'
...
export default {
  ...
  setup (props, { emit }) {
    ...
    const handleToggleClick = async () => {
      ...
      try {
        ...
        if (toggleState.value === 'OFF') {
          confirmMessage = '보령(정) AI 운영을 AI추천 모드로 하시겠습니까?'
          if (confirm(confirmMessage)) {
            // insertAIonOff DB 연동
            const actions = [
              { search: '606-359-EMS-1001', search2: '0' },
              { search: '606-359-EMS-1002', search2: '0' },
              { search: '606-359-EMS-1003', search2: '1' }
            ]
          ...
          confirmMessage = '보령(정) AI 운영을 off 하시겠습니까?'
          if (confirm(confirmMessage)) {
            // insertAIonOff DB 연동
            const actions = [
              { search: '606-359-EMS-1001', search2: '1' },
              { search: '606-359-EMS-1002', search2: '0' },
              { search: '606-359-EMS-1003', search2: '0' }
            ]
            const result = await actionInsertAIonOff(actions)
            if (result) {
    ...
    const handleAutoChange = async () => {
      // AI 자동 선택
      try {
        if (confirm('보령(정) AI 운영을 AI 모드로 하시겠습니까?')) {
          // insertAIonOff DB 연동
          const actions = [
            { search: '606-359-EMS-1001', search2: '0' },  
            { search: '606-359-EMS-1002', search2: '1' },
            { search: '606-359-EMS-1003', search2: '0' }
          ]
          const result = await actionInsertAIonOff(actions)
          if (result) {
    ...
    const handleBanAutoChange = async () => {
      // AI 반자동 선택
      try {
        if (confirm('보령(정) AI 운영을 AI추천 모드로 하시겠습니까?')) {
          // insertAIonOff DB 연동
          const actions = [
            { search: '606-359-EMS-1001', search2: '0' },  
            { search: '606-359-EMS-1002', search2: '0' },
    ...
    const leftmappingHtml = (data2) => {
      try {
        if (data2[0].정속펌프1가동상태운영_보령 === null) {
          ...
          insertPumpVal(aiFlowA, Number(data2[0].보령유량).toFixed(2))
      ...
      try {
        ...
        if (pumpSelect.value) {
          ...
          if (data3 && data3.length > 0) {
            ...
            // 분기점 현황 업데이트
            // insertPumpVal(divergeInfo[0], Number(data3[0].홍성유입유량).toFixed(2))
            // insertPumpVal(divergeInfo[1], Number(data3[0].청양분기압력).toFixed(2))
            // insertPumpVal(divergeInfo[2], Number(data3[0].서천관말분기압력).toFixed(2))
            // insertPumpVal(divergeInfo[3], Number(data3[0].송수터널후단압력).toFixed(2))
            // 물통 높이 추후 구현 필요
            gaugeHeight.value = '0%'
    ...
    const updatePTR = async () => {
      // insertPopup2 DB 연동
      try {
        await store.dispatch('songsu/fetchInsertPopup2', { search: '1' })
        if (insertPopup2.value) {
          const jsonData = JSON.parse(JSON.stringify(insertPopup2.value))
          if (jsonData.message !== 'ok') {
            // 실패했을 경우에 대한 처리는 기존 소스에 없음
