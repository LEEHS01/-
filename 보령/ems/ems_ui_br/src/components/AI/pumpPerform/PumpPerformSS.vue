  ...
  <div style="height: calc(100% - 90px); width: 100%">
    <div class="fL" style="width: 100%; height: 40%; margin-top: 10px">
      <template v-if="percentList && percentList.length > 0">
        <PerfomBox v-for="(item, index) in percentList" :key="index" :info="{ label: 'SS', data: item, ratedData: ratedData[index] }" />
      </template>
      <!-- 데이터가 없을때 보여주기 위해서 이름만 설정해 놓은 box 디자인임 -->
      <template v-else>
        <PerfomBox2 :info="{ label: 'SS', name: '서산(가)당진펌프1', ratedData: ratedData[0] }" />
        <PerfomBox2 :info="{ label: 'SS', name: '서산(가)당진펌프2', ratedData: ratedData[1] }" />
        <PerfomBox2 :info="{ label: 'SS', name: '서산(가)당진펌프3', ratedData: ratedData[2] }" />
        <PerfomBox2 :info="{ label: 'SS', name: '서산(가)당진펌프4', ratedData: ratedData[3] }" />
        <PerfomBox2 :info="{ label: 'SS', name: '서산(가)태안펌프1', ratedData: ratedData[4] }" />
        <PerfomBox2 :info="{ label: 'SS', name: '서산(가)태안펌프2', ratedData: ratedData[5] }" />
        <PerfomBox2 :info="{ label: 'SS', name: '서산(가)태안펌프3', ratedData: ratedData[6] }" />
        <PerfomBox2 :info="{ label: 'SS', name: '서산(가)태안펌프4', ratedData: ratedData[7] }" />
      </template>
    </div>
    <!-- 하단 부분 -->
...
export default {
  ...
  setup (props, { emit }) {
    ...
    const heatmapChart = async (data) => {
      try {
        data = data.filter(item => item.name.includes('서산(가)')) // gelix_lsj 예외처리
        let categories = []
    ...
    const gauge = async (data) => {
      try {
        ...
        const tempList = []
        for (const i in data) {
          if (i.includes('서산(가)')) { // gelix_lsj 예외처리
            let cnt = 0
            for (let j = 0; j < data[i].length; j++) {
