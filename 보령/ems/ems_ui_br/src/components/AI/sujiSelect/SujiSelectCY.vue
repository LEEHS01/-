...
export default {
  ...
  setup (props, { emit }) {
    ...
    const searchResult = () => {
      ...
      dataList.push(data4)
      dataList.push(data5)
      LineChart(data1, data2, data3A, data4, data5, data6)
      htmlRight(data7)
    }
    ...
    const htmlRight = (data) => {
      try {
        ...
        if (nc(data[0]["수위"])) {
        console.log('nc: ',nc(data[0]["수위"]))
        formattedData.value.수위 = commaFont(Number(data[0]["수위"]).toFixed(2))
        //formattedData.value.수위 = Number(data[0]["수위"]).toFixed(2)
        }
        formattedData.value.유입유량 = Number(data[0]["유입유량"]).toFixed(2)
        formattedData.value.밸브상태 = commaFont(Number(data[0]['밸브상태']).toFixed(0))
        formattedData.value.유출유량 = data[0]['유출유량']
        formattedData.value.정속토출관압 = commaFont(Number(data[0]['정속토출관압']).toFixed(2))
        formattedData.value.정속가동대수 = data[0]['정속가동대수']
      } catch (err) {
        console.log(err)
      }
    }
    ...
    // Line Chart func
    const LineChart = (data1, data2, data3A, data4, data5, data6) => {
      let x = []
      let y = []
      let chartData = []
      ...
      for (let i = 0; i < data6.length; i++) {
        x.push(data6[i].ts)
        //y.push(parseInt(data6[i].value,10))
        y.push(data6[i].value)
      }
    ...
    const clearChart = () => {
      ...
      Plotly.purge(chart4Ref.value)
      Plotly.purge(chart5Ref.value)
      Plotly.purge(chart6Ref.value)
    }
