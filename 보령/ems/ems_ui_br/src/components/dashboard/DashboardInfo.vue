        ...
        <div class="dashinfo-pump-div" style="color: #fff; font-family: 'KHNPHDRegular'; font-size: 13px;">
          <div style="height: calc(100%/ 2); width: 100%; margin-top:10px">
            <div class="pipe_name fL">
              <span class="right_title_font fL" style="font-size: 15px; margin-top: -15px">{{ state.activeReservoirName }}
              </span>
            </div>
            ...
            <div class="pipe_con fL">
              <div v-show="state.activeReservoirId === 0 || state.activeReservoirId === 1" class="pipe_un_y">
              </div>
            </div>
          ...
          <div
            v-show="state.activeReservoirId === 0 || state.activeReservoirId === 1"
            style="height: calc(100%/ 2);width: 100%;margin-bottom: 15px;"
          >
            <div class="pipe_name fL">
            </div>
            <div class="pipe_first_wrap fL">
...
export default {
  ...
  setup (props, context) {
    ...
    // TODO 보령정수장 배수지에 맞게 수정 필요, 임시 코드임
    const selectReservoir = (reservoirId) => {
      try {
        ...
        if (store.state.dashboard.valveData.length !== 0) {
          const valveData = []
          if (reservoirId === 0) {
            valveData.push(store.state.dashboard.valveData[85])
            valveData.push(store.state.dashboard.valveData[86])
            valveData.push(store.state.dashboard.valveData[87])
            valveData.push(store.state.dashboard.valveData[88])
          } else if (reservoirId === 1) {
            valveData.push(store.state.dashboard.valveData[89])
            valveData.push(store.state.dashboard.valveData[90])
            valveData.push(store.state.dashboard.valveData[91])
            valveData.push(store.state.dashboard.valveData[92])
            valveData.push(store.state.dashboard.valveData[93])
            valveData.push(store.state.dashboard.valveData[94])
          } else {
            valveData.push(store.state.dashboard.valveData[83])
            valveData.push(store.state.dashboard.valveData[84])
          }
          ...
          valveData.forEach((valve, idx) => {
            ...
            // const fo = valve.FO.split(',')
            // const fc = valve.FC.split(',')
            // state.valve[idx1].status[idx2] = (Number(fo[idx2]) === 0 && Number(fc[idx2]) === 1) ? 0 : 1
            state.valve[idx1].status[idx2] = (Number(valve.FO) === 0 && Number(valve.FC) === 1) ? 0 : 1
          })
        }
