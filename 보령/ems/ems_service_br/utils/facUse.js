...
exports.selectFacSunsi = (reqbody) => {
  ...
  let query = `
    ...
    WHERE a.tagname = b.tagname 
        ...
        AND b.zone = '${reqbody.search3}'

       /* AND ts >= CONCAT(CURDATE(),' 00:00:00') AND ts <= CONCAT(CURDATE(),' 23:59:00') */
  `;  

  if (reqbody.search5 === 'h' || reqbody.search5 === 'd') {
    query += ` and ts >= CONCAT('${reqbody.search}', ' 00:00:00') AND ts <= CONCAT('${reqbody.search2}', ' 23:59:00')`;
  } else if (reqbody.search5 === 'm') {
    query += ` and ts >= CONCAT('${reqbody.search}', '-01 00:00:00') AND ts <= CONCAT('${reqbody.search2}', '-31 23:59:00')`;
  } else if (reqbody.search5 === 'y') {
    query += ` and ts >= CONCAT('${reqbody.search}', '-01-01 00:00:00') AND ts <= CONCAT('${reqbody.search2}', '-12-31 23:59:00')`;
  }
