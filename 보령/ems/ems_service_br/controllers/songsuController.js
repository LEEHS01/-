...
exports.getInsertPopup3 = async (req, res) => {
  ...
  try {
    ...
    let result = queries.insertPopup3({ local });
    conn = await ems_db.getConnection();
    await conn.query(result);
    res.status(200).json({ message: 'ok' });
  } catch (err) {
...
exports.getSongsuSelect = async (req, res) => {
  ...
  try {
    logger.info('getSongsuSelect --- search: ', search);
    ...
    switch (search) {
      case '보령_정':
        result2 = queries.songsuSelect2_br();
        result3 = queries.songsuSelect3_br();
        break;
      case '서산_가':
        result2 = queries.songsuSelect2_ss();
        result3 = queries.songsuSelect3_ss();
        break;
      case '청양_가':
        result2 = queries.songsuSelect2_cy();
        result3 = queries.songsuSelect3_cy();
        break;
      case '홍성_가':
        result2 = queries.songsuSelect2_hs();
        result3 = queries.songsuSelect3_hs();
        break;
      default:
...
exports.getPumpSelect = async (req, res) => {
  ...
  try {
    logger.info('getPumpSelect --- search: ', search);
    ...
    switch (search) {
      case '보령_정':
        result3 = queries.pumpSelect3_br();
        result4 = queries.pumpSelect4_br();
        break;
      case '서산_가':
        result3 = queries.pumpSelect3_ss();
        result4 = queries.pumpSelect4_ss();
        break;
      case '청양_가':
        result3 = queries.pumpSelect3_cy();
        result4 = queries.pumpSelect4_cy();
        break;
      case '홍성_가':
        result3 = queries.pumpSelect3_hs();
        result4 = queries.pumpSelect4_hs();
        break;
      default:
        logger.error('error :no search parameter!')
        res.status(500).json({ message: 'Internal Server Error: no search parameter' });
