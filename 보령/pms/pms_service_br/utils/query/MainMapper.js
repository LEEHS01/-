...
exports.pumpBearingAll = () => {
  ...
  */
  ...
  let query = `
    SELECT 
      CONCAT('pump_scada_',lpad(ROW_NUMBER() OVER(order by CASE WHEN center_id = '보령(정)' THEN 0 ELSE 1 END  ASC ) ,2,0)) AS pump_scada_id  ,	    
      brg_motor_de_temp, 
      brg_motor_nde_temp, 
      brg_pump_de_temp, 
      brg_pump_nde_temp, 
...
exports.motorAlarm = () => {
  ...
  */
  ...
  let query = `
    ...
    ORDER BY 
      CASE WHEN m.center_id = '보령(정)' THEN 0 ELSE 1 END  ASC , m.moter_id asc
  `;
...
exports.nullDataAlarmInfo = () => {
  ...
  */
  ...
  let query = `
    /* 보령정수장 PMS 진동데이터에 문제 있을때 띄울 팝업쿼리 */
    SELECT 
      '보령정수장' AS ipc_loc ,
      (
        SELECT
        ...
        WHERE 1=1
          ...
          AND IPC_LOC IN ('보령정수장')
      ) AS cnt,
      (
        select
          CASE 
            WHEN COUNT(acq_ti) = 0 THEN '보령정수장의 IPC 전원을 확인하십시오..'
            ELSE ''
          END 
        FROM TB_PM
          WHERE 1=1
    ...
    SELECT 
      ...
      (
        ...
        WHERE 1=1
          AND acq_ti >= DATE_SUB(NOW(), INTERVAL 1 HOUR) AND acq_ti < NOW()
          AND IPC_LOC IN ('홍성가압장')
      ) AS cnt,
      (
        select
          CASE 
            WHEN COUNT(acq_ti) = 0 THEN '홍성가압장의 IPC 전원을 확인하십시오..'
            ELSE ''
          END 
        FROM TB_PM
          WHERE 1=1
...
exports.operationAll = () => {
  ...
  */
  ...
  let query = `
    ...
    ORDER BY 
    CASE WHEN center_id = '보령(정)' THEN 0 ELSE 1 END  ASC  , pump_scada_id asc
  `;
