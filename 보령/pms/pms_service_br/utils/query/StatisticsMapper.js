...
// let dbPrefix = 'SMART_PMS_CHEONAN.';
let dbPrefix = '';
  ...
  */
  ...
  let query = `
    SELECT 
      ...
      CASE 
        WHEN CAST(RIGHT(moter_id,2) AS INT) >=1 AND CAST(RIGHT(moter_id,2) AS INT) <= 6 then '보령정수장'
        WHEN CAST(RIGHT(moter_id,2) AS INT) >=7 AND CAST(RIGHT(moter_id,2) AS INT) <= 10 then '홍성가압장'
      END AS 보령그룹,
    ...
    SELECT 
      DATE_FORMAT(acq_date,'%Y-%m-%d %H:%i') AS DATE,
      CASE 
        WHEN CAST(RIGHT(pump_id,2) AS INT) >=1 AND CAST(RIGHT(pump_id,2) AS INT) <= 6 then '보령정수장'
        WHEN CAST(RIGHT(pump_id,2) AS INT) >=7 AND CAST(RIGHT(pump_id,2) AS INT) <= 10 then '홍성가압장'
      END AS 보령그룹,
