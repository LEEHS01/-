  ...
  */
  ...
  if (testMode) {
    query += ` AND center_id = '전동1(가)'  /* 가압장 이름, 변수처리 */
      ...
      AND (DATE_FORMAT(acq_date,'%Y-%m-%d') >= CONCAT('2024-05-19') AND DATE_FORMAT(acq_date,'%Y-%m-%d') <= CONCAT('2024-05-19'))
    `;
  } else {
    query += ` AND center_id = '${data.centerId}'  /* 가압장 이름, 변수처리 */
  ...
  */
  ...
  if (testMode) {
    query += ` AND ipc_loc = '전동1가압장'  /* 가압장 이름, 변수처리 */
      ...
      AND CONCAT(DATE_FORMAT(acq_ti,'%Y-%m-%d %H:%i:%S'),'.0000') = CONCAT('2023-10-12 13:54:12.0000')
    `;
  } else {
