                    ...
                    <div class="chart-area">
                        <div class="chart-con">
                            ...
                            <Linechart
                                ...
                                name1="과전류"
                                :detailData="
                                    store.state.monitor14.detailData
                                        .current_limit
                                "
                            />
                        </div>
...
export default {
    ...
    setup() {
        ...
        const state = reactive({
            model: store.state.monitor14.selectModel,
            options: computed(() => {
                let list = [];
                for (
                    let i = 0;
                    i < store.state.monitor14.modelList.length;
                    i++
                ) {
                    list.push(store.state.monitor14.modelList[i].title);
                }
                return list;
            }),
            datePop: false,
            ...
            startStr: '',
            endStr: '',
            detailInfo: computed(() => store.getters['monitor14/detailInfo']),
        });
        ...
        onMounted(() => {
            ...
            todayDate();
            dateToNumber();
            store.dispatch('monitor14/handleDatePicker');
        });

        const todayDate = () => {
            state.date.from = store.state.monitor14.startDate.split(' ')[0];
            state.date.to = store.state.monitor14.endDate.split(' ')[0];
        ...
        const dateToNumber = () => {
            if (state.date.from && state.date.to) {
                  store.state.monitor14.pickDate.from = Number(
                      state.date.from.replaceAll('-', '')
                  );
                  store.state.monitor14.pickDate.to = Number(
        ...
        const selectModel = (value) => {
            let select = store.state.monitor14.modelList.filter(
                (v) => v.title === value
            );
            store.state.monitor14.selectModel = select[0].title;
            store.state.monitor14.id = select[0].id;
            store.dispatch('monitor14/handleDatePicker');
        };
        const back = () => {
            router.push('CoolingPumpView');
        };
