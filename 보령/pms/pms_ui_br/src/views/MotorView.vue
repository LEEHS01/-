        ...
        <div class="content">
            <div v-show="store.state.monitor1.tabIndex === 1" class="content-left">
                <div class="content-panel">
                    <div v-for="index in 6" :key="index" class="pipe-line hori" :class="`line${index}`">
                        <div
                            v-for="i in 2"
                            :key="i"
                            class="water-icon2"
                    ...
                    <div class="pipe-line ver line7">
                        <div v-for="index in 12" :key="index" :class="`water-icon delay${index}`"></div>
                    </div>
                    <div class="content-header">
                        <div class="header-left"></div>
                        <div class="header-center">보령계통 송수펌프모터</div>
                        <div class="header-right"></div>
                    </div>
                    <div class="content-container two">
                        <div class="content-container-row" 
                            v-for="(motor, index) in store.state.monitor1.modelList.slice(0, 6)" 
                            :key="motor.id">
                            <div class="icon-area"
            ...
            <div v-show="store.state.monitor1.tabIndex === 2" class="content-left2">
                <div class="content-panel">
                    <div v-for="index in 4" :key="index" class="pipe-line hori" :class="`line${index}`">
                        <div
                            v-for="i in 2"
                            :key="i"
                            class="water-icon2"
                            :class="{ none: !store.state.monitor1.modelList[index + 5].eq_on, delay2: i === 2 }"
                        ></div>
                    </div>
                    ...
                    <div class="content-header">
                        <div class="header-left"></div>
                        <div class="header-center">홍성계통 송수펌프모터</div>
                        <div class="header-right"></div>
                    </div>
                    <div class="content-container two">
                        <div class="content-container-row" 
                            v-for="(motor, index) in store.state.monitor1.modelList.slice(6, 10)" 
                            :key="motor.id">
                            <div class="icon-area"
                                v-on:click="clickParam(motor.title, index + 7)"
                                :class="{ error: motor.alarm, none: !motor.eq_on }"
                            >
                                <Frame />
                                <div class="title-box">
...
export default {
    ...
    setup() {
        ...
        const clickParam = (value, idx) => {
            ...
            if (idx < 7) {  // gelix_lsj 1~6 보령계통
              store.state.monitor1.tabIndex = 1
              store.state.monitor1.centerId = '보령(정)'
            } else {  // gelix_lsj 7~14 홍성계통
              store.state.monitor1.tabIndex = 2
              store.state.monitor1.centerId = '홍성(가)'
            }
        ...
        // gelix_lsj 펌프 차트 확대
        const pumpChart = reactive([false, false, false, false, false, false])
        ...
        // gelix_lsj 분포도 차트 확대
        const distributionChart = reactive([false, false])
