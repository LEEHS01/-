                    ...
                    <div class="chart-area">
                        <div class="chart-con">
                            ...
                            <Linechart
                                ...
                                name1="과전류"
                                :detailData="
                                    store.state.monitor6.detailData
                                        .current_limit
                                "
                            />
                        </div>
                    ...
                    <div class="chart-area">
                        <div class="chart-con">
                            ...
                            <Linechart
                                ...
                                name1="전압변동률"
                                :detailData="
                                    store.state.monitor6.detailData.voltage_diff
                                "
                            />
                        </div>
                    </div>
...
export default {
    ...
    setup() {
        ...
        const state = reactive({
            model: store.state.monitor6.selectModel,
            options: computed(() => {
                let list = [];
                for (
                    let i = 0;
                    i < store.state.monitor6.modelList.length;
                    i++
                ) {
                    list.push(store.state.monitor6.modelList[i].title);
                }
                return list;
            }),
            tab: '가동여부',
            ...
            startStr: '',
            endStr: '',
            detailInfo: computed(() => store.getters['monitor6/detailInfo']),
        });
        ...
        onMounted(() => {
            ...
            todayDate();
            dateToNumber();
            store.dispatch('monitor6/handleDatePicker');
        });

        const todayDate = () => {
            state.date.from = store.state.monitor6.startDate.split(' ')[0];
            state.date.to = store.state.monitor6.endDate.split(' ')[0];
        ...
        const dateToNumber = () => {
            if (state.date.from && state.date.to) {
                  store.state.monitor6.pickDate.from = Number(
                      state.date.from.replaceAll('-', '')
                  );
                  store.state.monitor6.pickDate.to = Number(
        ...
        const selectModel = (value) => {
            let select = store.state.monitor6.modelList.filter(
                (v) => v.title === value
            );
            store.state.monitor6.selectModel = select[0].title;
            store.state.monitor6.id = select[0].id;
            store.dispatch('monitor6/handleDatePicker');
        };
        const back = () => {
            router.push('AutoWaterView');
        };
