<template>
    <div class="monitor-19">
        ...
        <div class="content">
            <div class="content-left">
                ...
                <div style="display: flex; flex-direction:row; justify-content: space-between; width: 85%;">
                    <div class="pump-name">보령정수장</div>
                    <div class="icon-button" @click="openModal_br">
                        <img src="../assets/icon/main6_s.png" alt="Settings Icon">
                    </div>
                </div>
                <PumpNav pumpNum = "1" centerId = "보령(정)" pumpMotorId = "motor_01" ipcLocId = "보령정수장" :trendChart = "trendChart"/>
                <PumpNav pumpNum = "2" centerId = "보령(정)" pumpMotorId = "motor_02" ipcLocId = "보령정수장" :trendChart = "trendChart"/>
                <PumpNav pumpNum = "3" centerId = "보령(정)" pumpMotorId = "motor_03" ipcLocId = "보령정수장" :trendChart = "trendChart"/>
                <PumpNav pumpNum = "4" centerId = "보령(정)" pumpMotorId = "motor_04" ipcLocId = "보령정수장" :trendChart = "trendChart"/>
                <PumpNav pumpNum = "5" centerId = "보령(정)" pumpMotorId = "motor_05" ipcLocId = "보령정수장" :trendChart = "trendChart"/>
                <PumpNav pumpNum = "6" centerId = "보령(정)" pumpMotorId = "motor_06" ipcLocId = "보령정수장" :trendChart = "trendChart"/>
                
                <div style="display: flex; flex-direction:row; justify-content: space-between; width: 85%;">
                    <div class="pump-name">홍성가압장</div> 
                    <div class="icon-button" @click="openModal_hs">
                        <img src="../assets/icon/main6_s.png" alt="Settings Icon">
                    </div>
                </div>
                <PumpNav pumpNum = "1" centerId = "홍성(가)" pumpMotorId = "motor_07" ipcLocId = "홍성가압장" :trendChart = "trendChart"/>
                <PumpNav pumpNum = "2" centerId = "홍성(가)" pumpMotorId = "motor_08" ipcLocId = "홍성가압장" :trendChart = "trendChart"/>
                <PumpNav pumpNum = "3" centerId = "홍성(가)" pumpMotorId = "motor_09" ipcLocId = "홍성가압장" :trendChart = "trendChart"/>
                <PumpNav pumpNum = "4" centerId = "홍성(가)" pumpMotorId = "motor_10" ipcLocId = "홍성가압장" :trendChart = "trendChart"/>
            </div>
        ...
        <div v-if="isModalOpen_hs" class="modal-overlay" @click.self="closeModal_hs">
            <div class="modal-content">
                <div class="selection-group">
                    <div class="selection">
                        ...
                        <ul>
                        <li
                            v-for="(pump, index) in pumps_hs"
                            :key="index"
                            @click="selectPump_hs(pump)"
                            :class="{ selected: selectedPump_hs === pump }"
                        >
                            {{ pump }}
                        </li>
                        </ul>
                    ...
                    <div class="selection">
                        ...
                        <ul>
                        <li
                            v-for="(motor, index) in motors_hs"
                            :key="index"
                            @click="selectMotor_hs(motor)"
                            :class="{ selected: selectedMotor_hs === motor }"
                        >
                            {{ motor }}
                        </li>
                        </ul>
                ...
                <div style="display: flex; flex-direction: column; width: 30%; margin-left: 10px; justify-content: space-between;">
                    <div style="display: flex; flex-direction: column;">
                        <p v-if="selectedMotor_hs" style="margin-top: 25px; font-size: x-large;">{{ selectedMotor_hs }}</p>
                        <p v-if="selectedPump_hs" style="margin-top: 25px; font-size: x-large;">{{ selectedPump_hs }}</p>
                        <div >
                            <div v-if="selectedPump_hs">
                    ...
                    <div style="display: flex; flex-direction: row; ">
                        ...
                        <button @click="closeModal_hs">Close</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <transition name="modal">
        <div v-if="isModalOpen_br" class="modal-overlay" @click.self="closeModal_br">
            <div class="modal-content">
                <div class="selection-group">
                    <div class="selection">
                        <h3>Pump</h3>
                        <ul>
                        <li
                            v-for="(pump, index) in pumps_br"
                            :key="index"
                            @click="selectPump_br(pump)"
                            :class="{ selected: selectedPump_br === pump }"
                        >
                            {{ pump }}
                        </li>
                        </ul>
                    ...
                    <div class="selection">
                        ...
                        <ul>
                        <li
                            v-for="(motor, index) in motors_br"
                            :key="index"
                            @click="selectMotor_br(motor)"
                            :class="{ selected: selectedMotor_br === motor }"
                        >
                            {{ motor }}
                        </li>
                        </ul>
                ...
                <div style="display: flex; flex-direction: column; width: 30%; margin-left: 10px; justify-content: space-between;">
                    <div style="display: flex; flex-direction: column;">
                        <p v-if="selectedMotor_br" style="margin-top: 25px; font-size: x-large;">{{ selectedMotor_br }}</p>
                        <p v-if="selectedPump_br" style="margin-top: 25px; font-size: x-large;">{{ selectedPump_br }}</p>
                        <div >
                            <div v-if="selectedPump_br">
                                <div style="display: flex; flex-direction: row; margin-bottom: 20px; margin-top: 20px;">
                                    <button
                                        @click="selectPumpButton('POV')"
                                        :style="buttonStyle(selectedPumpButton === 'POV')"
                    ...
                    <div style="display: flex; flex-direction: row; ">
                        <button @click="saveChanges">Save changes</button>
                        <button @click="closeModal_br">Close</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    setup() {
        ...
        const moment = require('moment');
          ...
          const pumpNum = ref(0)
        const siteNum = ref(0)
        const isModalOpen_hs = ref(false);
        const isModalOpen_br = ref(false);
        const selectedPump_hs = ref('');
        const selectedMotor_hs = ref('');
        const selectedPump_br = ref('');
        const selectedMotor_br = ref('');
        const selectedPumpButton = ref('')
        const selectedMotorButton = ref('')
        const pumps_hs = ref(['홍성가압장 #1', '홍성가압장 #2', '홍성가압장 #3', '홍성가압장 #4']);
        const motors_hs = ref(['홍성가압장 #1', '홍성가압장 #2', '홍성가압장 #3', '홍성가압장 #4']);
        const pumps_br = ref(['보령정수장 #1', '보령정수장 #2', '보령정수장 #3', '보령정수장 #4', '보령정수장 #5', '보령정수장 #6']);
        const motors_br = ref(['보령정수장 #1', '보령정수장 #2', '보령정수장 #3', '보령정수장 #4', '보령정수장 #5', '보령정수장 #6']);
        ...
        const selectPump_hs = (pump) => {
            selectedPump_hs.value = pump;
            selectedMotor_hs.value = '';  // Pump 선택 시 Motor 선택 해제
        ...
        const selectMotor_hs = (motor) => {
            selectedMotor_hs.value = motor;
            selectedPump_hs.value = '';  // Pump 선택 시 Motor 선택 해제
        ...
        const selectPump_br = (pump) => {
            selectedPump_br.value = pump;
            selectedMotor_br.value = '';  // Pump 선택 시 Motor 선택 해제
            pumpNum.value = extractNumber(pump)
            selectedPumpButton.value = '';
            selectedMotorButton.value = '';
            resetFormData(pumpFormData)
        ...
        const selectMotor_br = (motor) => {
            selectedMotor_br.value = motor;
            selectedPump_br.value = '';  // Pump 선택 시 Motor 선택 해제
            pumpNum.value = extractNumber(motor)
            selectedPumpButton.value = '';
            selectedMotorButton.value = '';
            resetFormData(pumpFormData)
        ...
        const openModal_hs = () => {
            isModalOpen_hs.value = true;
            siteNum.value = 1
            pumpNum.value = 0
            // selectedMotor_hs.value = ''
            // selectedPump_hs.value = ''
            // selectedMotor_br.value = ''
            // selectedPump_br.value = ''
            // resetFormData(pumpFormData)
            // resetFormData(motorFormData)

            selectPump_hs('홍성가압장 #1')
        };

        const closeModal_hs = () => {
            isModalOpen_hs.value = false;
            siteNum.value = 0
        };

        const openModal_br = () => {
            isModalOpen_br.value = true;
            siteNum.value = 2
            pumpNum.value = 0
            // selectedMotor_hs.value = ''
            // selectedPump_hs.value = ''
            ...
            selectPump_br('보령정수장 #1')
        };

        const closeModal_br = () => {
            isModalOpen_br.value = false;
            siteNum.value = 0 
        };
