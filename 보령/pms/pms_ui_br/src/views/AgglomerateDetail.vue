            ...
            <div class="content-right">
                ...
                <div class="location">
                    <div class="search">
                        ...
                        >
                            <!-- [gelix_lsj_231004] q-date 날짜 변경 시 input에 업데이트 되도록 수정 -->
                            <q-date
                                v-model="state.date"
                                range
                                mask="YYYY-MM-DD"
                    ...
                    <div class="chart-area">
                        <div class="chart-con">
                            ...
                            <Linechart
                                ...
                                name1="과전류"
                                :detailData="
                                    store.state.monitor8.detailData
                                        .current_limit
                                "
                            />
                        </div>
                    ...
                    <div class="chart-area">
                        <div class="chart-con">
                            ...
                            <Linechart
                                ...
                                name1="전압변동률"
                                :detailData="
                                    store.state.monitor8.detailData.voltage_diff
                                "
                            />
                        </div>
                    </div>
...
export default {
    ...
    setup() {
        ...
        const state = reactive({
            model: store.state.monitor8.selectModel,
            options: computed(() => {
                let list = [];
                for (
                    let i = 0;
                    i < store.state.monitor8.modelList.length;
                    i++
                ) {
                    list.push(store.state.monitor8.modelList[i].title);
                }
                return list;
            }),
            tab: '가동여부',
            ...
            startStr: '',
            endStr: '',
            detailInfo: computed(() => store.getters['monitor8/detailInfo']),
        });
        ...
        onMounted(() => {
            ...
            todayDate();
            dateToNumber();
            store.dispatch('monitor8/handleDatePicker');
        });

        const todayDate = () => {
            state.date.from = store.state.monitor8.startDate.split(' ')[0];
            state.date.to = store.state.monitor8.endDate.split(' ')[0];
        ...
        const dateToNumber = () => {
            if (state.date.from && state.date.to) {
                  store.state.monitor8.pickDate.from = Number(
                      state.date.from.replaceAll('-', '')
                  );
                  store.state.monitor8.pickDate.to = Number(
                      state.date.to.replaceAll('-', '')
                  );
                  state.startStr = state.date.from;
                  state.endStr = state.date.to;
            } else if (state.date) {
                // [gelix_lsj_231004] from, to를 같은 날짜로 선택 시 처리
                store.state.monitor8.pickDate.from = Number(
        ...
        const selectModel = (value) => {
            let select = store.state.monitor8.modelList.filter(
                (v) => v.title === value
            );
            store.state.monitor8.selectModel = select[0].title;
            store.state.monitor8.id = select[0].id;
            store.dispatch('monitor8/handleDatePicker');
        };
