    ...
    <div class="monitor-2">
        ...
        <div class="content">
            <div class="content-left">
                ...
                <div class="headerTitle">펌프기동반</div>
                <div class="content-container two">
                    <div
                        class="content-container-row"
                        v-for="(list, i) in store.state.monitor2.modelList"
                        :key="i"
                    >
                        <div
                            class="icon-area"
...
export default {
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            store.dispatch('monitor2/handleGraphData');
            store.dispatch('monitor2/getPumpInfo');
        });

        const clickParam = (value) => {
            store.state.monitor2.selectModel = value.title;
            store.state.monitor2.modelList.filter((list) => {
                if (list.title === value.title)
                    store.state.monitor2.id = list.id;
            });
            let index = store.state.monitor2.modelList.findIndex(
                (x) => x.title === value.title
            );
            store.state.monitor2.selectIdx = index;
            store.state.monitor2.selectIdx = router.push('PumpBoardDetail');
        };
