                    ...
                    <div class="chart-area">
                        <div class="chart-con">
                            ...
                            <Linechart
                                ...
                                name1="과전류"
                                :detailData="
                                    store.state.monitor11.detailData
                                        .current_limit
                                "
                            />
                        </div>
...
export default {
    ...
    setup() {
        ...
        const state = reactive({
            model: store.state.monitor11.selectModel,
            options: computed(() => {
                let list = [];
                for (
                    let i = 0;
                    i < store.state.monitor11.modelList.length;
                    i++
                ) {
                    list.push(store.state.monitor11.modelList[i].title);
                }
                return list;
            }),
            datePop: false,
            ...
            startStr: '',
            endStr: '',
            detailInfo: computed(() => store.getters['monitor11/detailInfo']),
        });
        ...
        onMounted(() => {
            ...
            store.state.monitor11.startDate = startDate;
            store.state.monitor11.endDate = endDate;
            todayDate();
            dateToNumber();
            store.dispatch('monitor11/handleDatePicker');
        });

        const todayDate = () => {
            state.date.from = store.state.monitor11.startDate.split(' ')[0];
            state.date.to = store.state.monitor11.endDate.split(' ')[0];
        ...
        const dateToNumber = () => {
            if (state.date.from && state.date.to) {
                  store.state.monitor11.pickDate.from = Number(
                      state.date.from.replaceAll('-', '')
                  );
                  store.state.monitor11.pickDate.to = Number(
        ...
        const selectModel = (value) => {
            let select = store.state.monitor11.modelList.filter(
                (v) => v.title === value
            );
            store.state.monitor11.selectModel = select[0].title;
            store.state.monitor11.id = select[0].id;
            store.dispatch('monitor11/handleDatePicker');
        };
        const back = () => {
            router.push('BackWashBlowerView');
        };
