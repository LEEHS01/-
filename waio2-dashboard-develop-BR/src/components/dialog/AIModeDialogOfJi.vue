...
export default {
  ...
  methods: {
    ...
    changeAIMode: function () {
      //let _data = {}
      let pStep = this.$store.state.sedimentation.processStep
      let numJi = this.$store.state.dialog.aiModeOfJi.number
      let ai = this.$store.state.sedimentation.latest['e_sc_set' + this.$store.state.dialog.aiModeOfJi.number] === 0 ? 1 : 0
      ...
      this.$store.dispatch(PUT_SEDIMENTATION_CONTROL_LOCATION, {numJi : numJi, ai : ai , processStep : pStep})
    }
  },
  mounted: function() {
  },
