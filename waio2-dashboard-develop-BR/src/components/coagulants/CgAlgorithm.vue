        ...
        <div class="injection-rate">
          <div class="apac">
            <div class="title-container">
              <span class="injection-rate__text--title">{{ this.$store.state.coagulants.latestModify.c_cf_coagulant }}</span>
            ...
            <div class="upper-limit">
              ...
              <div class="injection-rate__input-box" :class="{'': this.$store.state.coagulants.isModifyMode}">
                <input v-if="this.$store.state.coagulants.isModifyMode" type="text" 
                  :value="this.$store.state.coagulants.latestModify.c_cf_max" v-on:input="updateInput($event, 'c_cf_max')"/>
                <span v-else class="injection-rate__text--num">{{ this.$store.state.coagulants.latestModify.c_cf_max | numFormat('0.00') }}</span>
              </div>
            </div>
            ...
            <div class="lower-limit">
              ...
              <div class="injection-rate__input-box" :class="{'': this.$store.state.coagulants.isModifyMode}">
                <input v-if="this.$store.state.coagulants.isModifyMode" type="text" 
                  :value="this.$store.state.coagulants.latestModify.c_cf_min" v-on:input="updateInput($event, 'c_cf_min')"/>
                <span v-else class="injection-rate__text--num">{{ this.$store.state.coagulants.latestModify.c_cf_min | numFormat('0.00') }}</span>
              </div>
            </div>
          </div>
          <!-- 목표 침전지 탁도 -->
          <div class="turbidity">
            ...
            <div class="turbidity-value">
              <div class="injection-rate__input-box" :class="{'': this.$store.state.coagulants.isModifyMode}">
                <input v-if="this.$store.state.coagulants.isModifyMode" type="text" 
                :value="this.$store.state.coagulants.latestModify.c_user_tb_e" v-on:input="updateInput($event, 'c_user_tb_e')" maxlength="5"/>
                <span class="injection-rate__text--num" v-else>{{ this.$store.state.coagulants.latestModify.c_user_tb_e | numFormat('0.00') }}</span>
              </div>
            </div>
          </div>          
          <div v-if="$store.state.login.user.tkn !== null" class="modify-button">
            ...
            <div v-if="this.$store.state.coagulants.isModifyMode" class="custom-cancel-icon" style="margin-top: 5px;" @click="cancelControl">
...
//import { GET_COAGULANT_LATEST, GET_COAGULANTS_CLUSTER, GET_COAGULANTS_RAWWATER, PUT_COAGULANT_CONTROL_AI } from '@/store/modules/coagulants'
import { GET_COAGULANT_LATEST, PUT_COAGULANT_CONTROL_AI } from '@/store/modules/coagulants'
  ...
  methods: {
    ...
    updateControl: function() { 
      ...
      if (this.$store.state.coagulants.isModifyMode) {
        if (this.$store.state.coagulants.latestModify.c_cf_max === '' 
        || this.$store.state.coagulants.latestModify.c_cf_min === ''
        || this.$store.state.coagulants.latestModify.c_user_tb_e === '') {
        this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '값을 입력해주세요' })
        }else if (parseFloat(this.$store.state.coagulants.latestModify.c_cf_max) < rate_min || parseFloat(this.$store.state.coagulants.latestModify.c_cf_max) > rate_max) {
          this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '주입률 설정 범위', text2: rate_min + ' ~ ' + rate_max })
        } else if (parseFloat(this.$store.state.coagulants.latestModify.c_cf_min) < rate_min || parseFloat(this.$store.state.coagulants.latestModify.c_cf_min) > rate_max) {
          this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '주입률 설정 범위', text2: rate_min + ' ~ ' + rate_max })
        } else{

          let rate_min = parseFloat(this.$store.state.coagulants.latestModify.c_cf_min).toFixed(2)
          let rate_max = parseFloat(this.$store.state.coagulants.latestModify.c_cf_max).toFixed(2)
          let rate_c_user_tb_e = parseFloat(this.$store.state.coagulants.latestModify.c_user_tb_e).toFixed(2)
          let pStep = this.$store.state.coagulants.processStep
          this.$store.dispatch(PUT_COAGULANT_CONTROL_AI, { c_cf_min : rate_min, c_cf_max : rate_max, processStep: pStep, c_user_tb_e : rate_c_user_tb_e })

          this.$store.state.coagulants.isModifyMode = !this.$store.state.coagulants.isModifyMode
          this.$store.state.coagulants.latest.c_cf_max = parseFloat(this.$store.state.coagulants.latestModify.c_cf_max).toFixed(2)
          this.$store.state.coagulants.latest.c_cf_min = parseFloat(this.$store.state.coagulants.latestModify.c_cf_min).toFixed(2)
          this.$store.state.coagulants.latest.c_user_tb_e = parseFloat(this.$store.state.coagulants.latestModify.c_user_tb_e).toFixed(2)
        }
      } else{
        this.$store.state.coagulants.isModifyMode = !this.$store.state.coagulants.isModifyMode
      }
