...
export default {
  ...
  computed: {
    ...
    ChartRawWaterFlow: function () {
      ...
      chart.series = []
      if (this.$store.state.receiving.latest.ai_b_in_fr_trd !== null && Object.keys(this.$store.state.receiving.latest.ai_b_in_fr_trd).length > 0) {
        let dataSeries1 = []
        for (let key in this.$store.state.receiving.latest.ai_b_in_fr_trd) {
          dataSeries1.push([new Date(key).getTime(), parseInt(this.$store.state.receiving.latest.ai_b_in_fr_trd[key])])
        }
        chart.series.push({
          name: '유입 유량',
          data: dataSeries1,
