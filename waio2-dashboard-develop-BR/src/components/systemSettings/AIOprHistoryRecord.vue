...
export default {
  ...
  data: () => ({
    ...
    chartOptions:{
      ...
      title: null,
      xAxis: {
        categories: ['착수', '약품', '혼화응집', '침전', '여과', '전차염', '중차염', '후차염'],
        gridLineWidth: 0,
        labels: {
          style: {
            color: '#FFFFFF'
  ...
  methods: {
    getAiOprDays:function(process, opr) {
      return this.$store.state.aioprhistory.aiOprHistorySearch[`${process}_${opr}`].days
    },
    getAiOprDaysDisinfection:function(process, disinfectionIndex, opr) {
      return this.$store.state.aioprhistory.aiOprHistorySearch[`${process}_${disinfectionIndex}_${opr}`].days
    },
    getAiOprHours:function(process, opr) {
      return this.$store.state.aioprhistory.aiOprHistorySearch[`${process}_${opr}`].hours
    },
    getAiOprHoursDisinfection:function(process, disinfectionIndex, opr) {
      return this.$store.state.aioprhistory.aiOprHistorySearch[`${process}_${disinfectionIndex}_${opr}`].hours
    },
    getAiOprTotalHours:function(process, opr) {
      return this.$store.state.aioprhistory.aiOprHistorySearch[`${process}_${opr}`].totalHours
    },
    getAiOprTotalHoursDisinfection:function(process, disinfectionIndex, opr) {
      return this.$store.state.aioprhistory.aiOprHistorySearch[`${process}_${disinfectionIndex}_${opr}`].totalHours
    },
    onClickSearchBtn () {
