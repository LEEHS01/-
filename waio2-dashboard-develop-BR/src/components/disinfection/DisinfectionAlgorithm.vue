...
import { GET_DISINFECTION_LATEST, PUT_DISINFECTION_HISTORY_EVAPORATION } from '@/store/modules/disinfection'
...
export default {
  ...
  mounted: function () {
    ...
    Promise.all([
      ...
      this.$store.dispatch(PUT_DISINFECTION_HISTORY_EVAPORATION, {selectedDisinfectionIndex : this.$store.state.disinfection.selectedDisinfectionIndex}),
      this.$store.dispatch(PUT_SEDIMENTATION_HISTORY_CL, {selectedDisinfectionIndex : this.$store.state.disinfection.selectedDisinfectionIndex}),
    ]).finally(() => {
