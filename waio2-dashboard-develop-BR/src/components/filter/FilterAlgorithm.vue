      ...
      <div class="right">
        ...
        <div class="right-contents" style="margin-top: 10px; align-items: end;">
          ...
          <div class="right-contents__rightCon">
            <div class="right-contents__title">기준 가동전력</div>
            <div class="right-contents__wrap" style="justify-content: center;">
              <div class="right-contents__input-box">
                <input v-if="this.$store.state.filter.isModifyMode" type="text" 
                :value="this.$store.state.filter.latestModify.f_pw" v-on:input="updateInput($event, 'f_pw')" maxlength="5"/>
                <span v-else>{{ this.$store.state.filter.latestModify.f_pw | numFormat('0') }}</span>
              </div>
            </div>
          </div>
  ...
  methods: {
    ...
    updateControl: function() {
      ...
      if(this.$store.state.filter.isModifyMode){
        ...
        || this.$store.state.filter.latestModify.f_pw === '') {
          this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '값을 입력해주세요' })
        } else if (parseFloat(this.$store.state.filter.latestModify.f_location_wl_min) < wl_min 
        || parseFloat(this.$store.state.filter.latestModify.f_location_wl_max) > wl_max) {
          ...
          obj.f_pw = parseInt(this.$store.state.filter.latestModify.f_pw)
          ...
          this.$store.state.filter.latest.f_pw = parseInt(this.$store.state.filter.latestModify.f_pw)
        }
      }else {
        this.$store.state.filter.isModifyMode = !this.$store.state.filter.isModifyMode
      }
    ...
    cancelControl: function () {
      ...
      this.$store.state.filter.latestModify.f_pw = this.$store.state.filter.latest.f_pw
      this.$store.state.filter.isModifyMode = !this.$store.state.filter.isModifyMode
    },
