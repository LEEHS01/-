...
export default {
  ...
  state: {
    ...
    latest: {
      ...
      d_g_value2: null, 
      d_g_value3: null, 
      d_g_value1_init: null, 
      d_g_value2_init: null, 
      d_g_value3_init: null, 
      d_g_value_ctr_flag: 0, //교반강도 모드 0:수동, 1:자동
      d_loc_fc_stt1: {
        step1: null,
        step2: null,
        step3: null
  ...
  actions: {
    [GET_LATEST]: async function ({ commit }) {
      await axios.get(`${DEV_SERVER}/${URL.MIXING_LATEST}` +'/' + PROCESS_STEP)
        .then(({ data }) => {
    ...
    [PUT_HISTORY_FC_SP]: async function ({ commit }) {
      ...
      // nowTimestamp = Date.parse("2013-9-5 10:00:00"); // FIXME 추후 변경
    ...
    [PUT_CONTROL_OPERATION]: async function ({ commit }, { operation, processStep }) {
      await axios.put(`${DEV_SERVER}/${URL.MIXING_CONTROL_OPERATION}/`+processStep, { 'operation': operation })
      .then(() => {
