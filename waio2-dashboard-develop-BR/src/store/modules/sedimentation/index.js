...
export default {
  ...
  actions: {
    ...
    [GET_DISTRIBUTION_TB]: async function ({ commit }) {
      await axios.get(`${DEV_SERVER}/${URL.SEDIMENTATION_DISTRIBUTION_TB}`+'/'+PROCESS_STEP)
        .then(({ data }) => {
          ...
          //console.log('GET_DISTRIBUTION_TB', data.tb.series1)
        })
        .catch(error => {
          util.printError(error)
        })
    ...
    [PUT_HISTORY_TB]: async function ({ commit }) {
      let nowTimestamp = Date.now()
      // nowTimestamp = Date.parse("2013-09-04 00:00:00"); // FIXME 현재 날짜 수정
      let oneDayTimestamp = 1000 * 60 * 60 * 24
      await axios.put(`${DEV_SERVER}/${URL.SEDIMENTATION_HISTORY_TB}`+ '/' + PROCESS_STEP, { 'start_time': new Date(nowTimestamp - oneDayTimestamp).toISOString(), 'end_time': new Date(nowTimestamp).toISOString() })
        .then(({ data }) => {
          ...
          //console.log('PUT_HISTORY_TB', data.tb.series1)
        })
        .catch(error => {
          util.printError(error)
        })
    ...
    [PUT_HISTORY_CL]: async function ({ commit }, {selectedDisinfectionIndex}) {
      ...
      let nowTimestamp = Date.now()
      let oneDayTimestamp = 1000 * 60 * 60 * 24
      await axios.put(`${DEV_SERVER}/${URL.SEDIMENTATION_HISTORY_CL}/`+PROCESS_STEP+'/'+selectedDisinfectionIndex, { 'start_time': new Date(nowTimestamp - oneDayTimestamp).toISOString(), 'end_time': new Date(nowTimestamp).toISOString() })
      // await axios.put(`${DEV_SERVER}/${URL.SEDIMENTATION_HISTORY_CL}/`+PROCESS_STEP+'/'+selectedDisinfectionIndex, { 'start_time': new Date('2013-09-03 00:00:00').toISOString(), 'end_time': new Date('2013-09-04 23:59:59').toISOString() })
        .then(({ data }) => {
    ...
    [PUT_CONTROL_SC]: async function ({ commit }, { e_sc_set_sludge_q, e_sc_set_max_wait, e_set_lt, e_sc_set_ti, e_low_hour, processStep }) {
