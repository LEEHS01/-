...
export default {
  ...
  getters: {
    ...
    getLocationState21: function (state) {
      if (state.latest.f_loc_stt && state.latest.f_loc_stt.location21) {
        return LOCATION_STATE[state.latest.f_loc_stt.location21]
      } else {
        return '--'
      }
    },
    getLocationState22: function (state) {
      if (state.latest.f_loc_stt && state.latest.f_loc_stt.location22) {
        return LOCATION_STATE[state.latest.f_loc_stt.location22]
      } else {
        return '--'
      }
    }
  ...
  actions: {
    ...
    [PUT_CONTROL_OPERATION]: async function ({ commit }, { operation, processStep }) {
      await axios.put(`${DEV_SERVER}/${URL.FILTER_CONTROL_OPERATION}/`+processStep, { 'operation': operation })
      .then(() => {
    ...
    [PUT_CONTROL_TI]: async function ({ commit }, { f_location_ti_set_max, f_location_wl_max, f_pw }) {
      await axios.put(`${DEV_SERVER}/${URL.FILTER_CONTROL_TI}/`+ PROCESS_STEP, { f_location_ti_set_max, f_location_wl_max, f_pw })
      .then(() => {
        commit(PUT_CONTROL_TI, { f_location_ti_set_max, f_location_wl_max, f_pw })
        // commit('dialog/'+ CLOSE_AI_MODE_DIALOG, null, { root: true })
        let _data = {
          visible: true,
          title: '설정 성공',
