...
public class MixingRealtimeDAOImpl implements IProcessRealtimeDAO
{
    ...
    public List<InterfaceAlarmControlHistoryDTO> selectAllRealTime(LocalDateTime today) {
        ...
        for (int processStep : CommonValue.PROCESS_STEP_ARRAY) {
            for (String process : processArr) {
                if(process.equals(CommonValue.PROCESS_FILTER) && (processStep == 1)) { // 여과는 2단계생활, 3단계만
                    continue;
                }
                strQuery.append("(SELECT '").append(process).append(processStep == 1 ? "2_IND" : processStep == 2 ? "2_LIV" : processStep)
                        .append("' AS PROCESS, MAX(UPD_TI) AS UPD_TI FROM TB_")
                        .append(process).append(processStep == 1 ? "2_IND" : processStep == 2 ? "2_LIV" : processStep == 4 ? "_PUMP" : processStep).append("_RT PARTITION(p_")
    ...
    public String getTableByProcessStep(int processStep) {
        String tableNm = "";
        if(processStep == 1) {
            tableNm = "TB_D2_IND_RT";
        } else if(processStep == 2) {
            tableNm = "TB_D2_LIV_RT";
