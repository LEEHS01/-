<template>
  <div class="drawer-container">
                ...
                >
                  ...
                  <template v-for="(child, i) in item.children">
                    <v-list-item
                      ...
                      @click="goURL(item.disinfectionStep, child.processStep, item.text)"
                      link
                      >
                      <v-list-item-action>
    ...
    <div style="position:absolute; display:none">
      <form ref="form">
        <input name="token" v-model="$store.state.login.user.tkn">
      </form>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    goURL: function(disinfectionStep, processStep, title) {
      if(disinfectionStep !== undefined) {
        this.$store.state[processStep].disinfectionStep = disinfectionStep
        this.$store.state[processStep].title = title.split(' ')[1]
      }
      // if(route == '/fstDisinfectionAlgorithm'){
      //   this.$store.state.fstDisinfection.disinfectionStep = processStep
      //   this.$store.state.fstDisinfection.title = title.split(' ')[1]
      // } else if (route == '/sndDisinfectionAlgorithm'){
      //   this.$store.state.sndDisinfection.disinfectionStep = processStep
      //   this.$store.state.sndDisinfection.title = title.split(' ')[1]
      // } else if (route == '/trdDisinfectionAlgorithm'){
      //   this.$store.state.trdDisinfection.disinfectionStep = processStep
      //   this.$store.state.trdDisinfection.title = title.split(' ')[1]
      // }
    },
    /**
     * PMS 메뉴로 이동하는 함수
