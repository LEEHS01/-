      ...
      <div class="top-contents-container">
        ...
        <div class="top-container">
          <div class="top-one-contents-box">
            <div class="toggle-wrap">
              ...
              <div class="btn-contents" style="width: 60px;">
                <div class="btn-contents__btn">
                  <input type="checkbox" class="checkbox" @click="openPopup" :class="{ 'checked': this.$store.state.indMixing.latest.d_g_value_ctr_flag == 1 }">
                </div>
              </div>
            </div>
            <div class="top-one-contents-value">
              <div class="top-one-contents-value__text">#1</div>
              <input ref="gValueInputBox" v-if="this.$store.state.indMixing.isModifyMode && this.$store.state.indMixing.latest.d_g_value_ctr_flag == 0" type="text" class="top-one-contents-value__input" :value="this.$store.state.indMixing.latestModify.d_g_value1" v-on:input="updateInput($event, 'd_g_value1')"/>
              <span class="top-one-contents-value__span" v-else>{{ this.$store.state.indMixing.latest.d_g_value_ctr_flag == 0 ? this.$store.state.indMixing.latest.d_g_value1 : this.$store.state.indMixing.latest.ai_d_g_value1 | numFormat('0.0')}}</span>
              <div class="top-one-contents-value__unit">sec<sup>-1</sup></div>
            </div>
            <div class="top-one-contents-value">
              <div class="top-one-contents-value__text">#4</div>
              <input v-if="this.$store.state.indMixing.isModifyMode && this.$store.state.indMixing.latest.d_g_value_ctr_flag == 0" type="text" class="top-one-contents-value__input" :value="this.$store.state.indMixing.latestModify.d_g_value2" v-on:input="updateInput($event, 'd_g_value2')"/>
              <span class="top-one-contents-value__span" v-else>{{ this.$store.state.indMixing.latest.d_g_value_ctr_flag == 0 ? this.$store.state.indMixing.latest.d_g_value2 : this.$store.state.indMixing.latest.ai_d_g_value2 | numFormat('0.0')}}</span>
              <div class="top-one-contents-value__unit">sec<sup>-1</sup></div>
            </div>
            ...
            <div class="auto-btn" v-show="$store.state.indMixing.latest.d_g_value_ctr_flag == 1" ><button class="auto-btn__detail" @click="openAutoDetail()">자동모드 상세</button></div> 
          </div>
          <div class="top-two-contents-box">
            ...
            <div class="top-two-contents-value">
              ...
              <div class="top-two-contents-value__value">{{ this.$store.state.indMixing.latest.d_dv | numFormat('0.00000') }}</div>
              <div class="top-two-contents-value__unit">N·s/m·<sup>3</sup></div>
            </div>
            <div class="top-two-contents-value">
              <div class="top-two-contents-value__text">· 임펠러 직경</div>
              <div class="top-two-contents-value__value">{{ this.$store.state.indMixing.latest.d_im_d | numFormat('0.00') }}</div>
              <div class="top-two-contents-value__unit">m</div>
            </div>
            <div class="top-two-contents-value">
              <div class="top-two-contents-value__text">· Power Number</div>
              <div class="top-two-contents-value__value">{{ this.$store.state.indMixing.latest.d_pw | numFormat('0.00') }}</div>
              <div class="top-two-contents-value__unit"></div>
            </div>
            <div class="top-two-contents-value">
    ...
    <div class="bottom-contents">
      ...
      <div class="bottom-chart-box">
        ...
        <div class="select-box">
          <div v-for="index in [9,10]" :key="index" class="select-box__element" @click="selectedLocation = index" :class="{ 'select-box__element--focused': selectedLocation === index}">{{ index }}지</div>
          </div>
        <div class="bottom-chart">
...
export default {
  ...
  methods: {
    ...
    updateControl: function() {      
      ...
      if (this.$store.state.indMixing.isModifyMode) {
        if (this.$store.state.indMixing.latestModify.d_g_value1 === ''
          || this.$store.state.indMixing.latestModify.d_g_value2 === '') {
          this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '값을 입력해주세요' })
        } else if (parseFloat(this.$store.state.indMixing.latestModify.d_g_value1) <= min 
                  || parseFloat(this.$store.state.indMixing.latestModify.d_g_value1) >= max
                  || parseFloat(this.$store.state.indMixing.latestModify.d_g_value2) <= min
                  || parseFloat(this.$store.state.indMixing.latestModify.d_g_value2) >= max) {
          this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '설정 범위', text2: min + ' ~ ' + max })
        } else {
          let obj = {}
          obj.d_g_value_step1 = parseFloat(this.$store.state.indMixing.latestModify.d_g_value1)
          obj.d_g_value_step2 = parseFloat(this.$store.state.indMixing.latestModify.d_g_value2)
    ...
    downloadExcel: function() {
      ...
      for (let i = 0; i < Object.keys(speedPrediction).length; i++) {
        for (let speedPrediction_obj in speedPrediction['location' + (i + 9)]) {
          for (let time in speedPrediction['location' + (i + 9)]['step1']) {
            const excelLogs = {
              DateTime: '', 
              계열1: '',
              계열2: '',
            ...
            excelLogs.계열1 = speedPrediction['location' + (i + 9)]['step1'][time]
            excelLogs.계열2 = speedPrediction['location' + (i + 9)]['step2'][time]
            // excelLogs.계열3 = speedPrediction['location' + (i + 2)]['step3'][time]

            const speedPrediction_length = Object.keys(speedPrediction['location' + (i + 9)][speedPrediction_obj]).length
            ...
            if ( i === 0 ) {
              ...
              if( speedPrediction_length != this.ji10.length ) {
                this.ji10.push(excelLogs);
              }
            }
          }
      ...
      const dataWS10 = XLSX.utils.json_to_sheet(this.ji10);
      
      XLSX.utils.book_append_sheet(wb, dataWS9, '9지');
      XLSX.utils.book_append_sheet(wb, dataWS10, '10지');
  ...
  watch: {
    // 교반강도 #1
    '$store.state.indMixing.latestModify.d_g_value1': function(newVal) {
      if (newVal === '') {
    ...
    // 교반강도 #2
    '$store.state.indMixing.latestModify.d_g_value2': function(newVal) {
      if (newVal === '') {
    ...
    // 교반강도 #3
    // '$store.state.indMixing.latestModify.d_g_value3': function(newVal, oldVal) {
    //   if (newVal === '') {
