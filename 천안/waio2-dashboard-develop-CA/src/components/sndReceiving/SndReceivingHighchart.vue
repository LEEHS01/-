  ...
  export default {
    ...
    computed: {
      ...
      ChartReceivingWaterFlow: function () {
        ...
        chart.series = []
        if (this.$store.state.sndReceiving.latest.ai_b_out_fr_rtd !== null) {
          let dataSeries1 = []
          for (let key in this.$store.state.sndReceiving.latest.ai_b_out_fr_rtd) {
            dataSeries1.push([new Date(key).getTime(), this.$store.state.sndReceiving.latest.ai_b_out_fr_rtd[key]])
          }
          chart.series.push({
            name: '유출 유량',
            data: dataSeries1,
      ...
      ChartClearWaterLevel: function () {
        ...
        chart.series = []
        if (this.$store.state.sndClear.le !== null && Object.keys(this.$store.state.sndClear.le).length > 0) {
          if (this.$store.state.sndClear.le.h_bnd_uplmt && this.$store.state.sndClear.le.h_bnd_lolmt) {
            let _data = []
            for (let key in this.$store.state.sndClear.le.h_bnd_uplmt) {
              _data.push([new Date(key).getTime(), this.$store.state.sndClear.le.h_bnd_uplmt[key], this.$store.state.sndClear.le.h_bnd_lolmt[key]])
            }
            chart.series.push({
              name: '범위',
              type: 'arearange',
          ...
          // if (this.$store.state.sndClear.le.wide_up && this.$store.state.sndClear.le.wide_down) {
          //   let _data = []
          //   for (let key in this.$store.state.sndClear.le.wide_up) {
          //     _data.push([new Date(key).getTime(), this.$store.state.sndClear.le.wide_up[key], this.$store.state.sndClear.le.wide_down[key]])
          //   }
          //   chart.series.push({
          //     name: '넓은범위',
          //     type: 'arearange',
      ...
      ChartDataClearLevelFromLocation: function () {
        let data = []
        if (this.$store.state.sndClear.le !== null 
        && this.$store.state.sndClear.le['location' + this.selectedLocation] !== undefined 
        && this.$store.state.sndClear.le['location' + this.selectedLocation] !== null 
        && Object.keys(this.$store.state.sndClear.le['location' + this.selectedLocation]).length > 0) {
          for (let key in this.$store.state.sndClear.le['location' + this.selectedLocation]) {
            data.push([new Date(key).getTime(), this.$store.state.sndClear.le['location' + this.selectedLocation][key]])
          }
        }
        return data
      },
      ...
      ChartRawWaterFlow: function () {
        ...
        chart.series = []
        if (this.$store.state.sndReceiving.latest.ai_b_in_fr_rtd !== null && Object.keys(this.$store.state.sndReceiving.latest.ai_b_in_fr_rtd).length > 0) {
          let dataSeries1 = []
          for (let key in this.$store.state.sndReceiving.latest.ai_b_in_fr_rtd) {
            dataSeries1.push([new Date(key).getTime(), parseInt(this.$store.state.sndReceiving.latest.ai_b_in_fr_rtd[key])])
          }
          chart.series.push({
            name: '유입 유량',
            data: dataSeries1,
    ...
    methods: {
      downloadExcel: function() {
        let ret = this.$store.state.sndClear.le
        console.log('ret', ret)
