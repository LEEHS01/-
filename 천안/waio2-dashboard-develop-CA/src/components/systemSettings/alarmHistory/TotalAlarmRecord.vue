      ...
      <div class="record-title">
        <div class="record-title-contents">
          <div class="record-text">
            {{ this.$store.state.alarm.selectedIndex == 'autoControl' ? '자동' : this.$store.state.alarm.selectedIndex == 'userControl' ? '사용자' : '전체' }} 제어
          </div>
        </div>
      </div>
      <!-- 날짜 검색 -->
          ...
          <tbody class="search-contents-container ">
            ...
            <template v-for="(item) in paginatedData"> 
              <tr class="table-contents" :key="item.seq">
                <td>{{ item.ctrYn == 'A' ? '자동' : item.ctrYn != 'A' && item.almTy == $store.state.alarm.semiAuto ? '반자동' : '임계치'}}</td>
                <td>{{ item.ctrYn == 'A' ? '시스템' : item.usrNm === null ? '-' : item.usrNm }}</td>
                <td>{{ item.processTitle }}</td>
                <td>{{ item.tagDp === null ? '-' : item.tagDp }}</td>
                <td>{{ item.tagCmpVal === null ? '-' : item.tagCmpVal }}</td>
                <td>{{ item.tagVal === null ? '-' : item.tagVal }}</td>
                <td>{{ item.ctrTi | dateFormat }}</td>
                <td>{{ item.ctrYn == 'Y' ? '제어' : item.ctrYn == 'N' ? '취소' :  item.ctrYn == 'A' ? '자동 제어' : '-'}}</td>
  ...
  export default {
    ...
    computed: {
      ...
      alarmHistorySearchData () {
        return this.$store.state.alarm.alarms.map((it) => {
          ...
          _it.processTitle = _processStep + this.$getProcessTitle(it.process == 'G' ? it.process + it.disinfectionIndex : it.process) 

          // 자동제어/사용자제어 값 세팅
          if(_it.ctrYn === 'A') {
            _it.selectedIndex = 'autoControl'
          } else {
            _it.selectedIndex = 'userControl'
          }
          ...
          if (this.isSearch === true && this.search !== '' && this.search !== null) {
            Object.keys(data).forEach((key) => {
              if(data[key] != null && data[key].toString().includes(this.search)) {
                if(this.$store.state.alarm.selectedIndex == 'total' || this.$store.state.alarm.selectedIndex == data.selectedIndex) {
                  flag = true
                  return flag
                }
              }
      ...
      controlCnt() {
        ...
        this.calcData.forEach((it) => {
          if(it.ctrYn === 'Y') {
            if(this.$store.state.alarm.selectedIndex == 'total' || this.$store.state.alarm.selectedIndex == it.selectedIndex) {
              count++
            }
          }
        })
    ...
    methods: {
      ...
      onClickDetail: function(obj) {
        ...
        this.$store.state.alarm.selectedDisinfectionIndex = obj.disinfectionIndex
        Promise.all([
          this.$store.dispatch('alarm/PUT_ALARMS_SEARCH_DETAIL', obj)
        ]).finally(() => {
          this.$store.commit(SET_OVERLAY, false)
        })
      }
    ...
    watch: {
      ...
      '$store.state.alarm.selectedIndex': function() {
        this.search = ''
