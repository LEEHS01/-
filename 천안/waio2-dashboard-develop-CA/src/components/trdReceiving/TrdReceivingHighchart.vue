  ...
  export default {
    ...
    computed: {
      ...
      ChartReceivingWaterFlow: function () {
        ...
        chart.series = []
        if (this.$store.state.trdReceiving.latest.ai_b_out_fr_rtd !== null) {
          let dataSeries1 = []
          for (let key in this.$store.state.trdReceiving.latest.ai_b_out_fr_rtd) {
            dataSeries1.push([new Date(key).getTime(), this.$store.state.trdReceiving.latest.ai_b_out_fr_rtd[key]])
          }
          chart.series.push({
            name: '유출 유량',
            data: dataSeries1,
      ...
      ChartClearWaterLevel: function () {
        ...
        chart.series = []
        if (this.$store.state.trdClear.le !== null && Object.keys(this.$store.state.trdClear.le).length > 0) {
          if (this.$store.state.trdClear.le.h_bnd_uplmt && this.$store.state.trdClear.le.h_bnd_lolmt) {
            let _data = []
            for (let key in this.$store.state.trdClear.le.h_bnd_uplmt) {
              _data.push([new Date(key).getTime(), this.$store.state.trdClear.le.h_bnd_uplmt[key], this.$store.state.trdClear.le.h_bnd_lolmt[key]])
            }
            chart.series.push({
              name: '범위',
              type: 'arearange',
          ...
          // if (this.$store.state.trdClear.le.wide_up && this.$store.state.trdClear.le.wide_down) {
          //   let _data = []
          //   for (let key in this.$store.state.trdClear.le.wide_up) {
          //     _data.push([new Date(key).getTime(), this.$store.state.trdClear.le.wide_up[key], this.$store.state.trdClear.le.wide_down[key]])
          //   }
          //   chart.series.push({
          //     name: '넓은범위',
          //     type: 'arearange',
      ...
      ChartDataClearLevelFromLocation: function () {
        let data = []
        if (this.$store.state.trdClear.le !== null 
        && this.$store.state.trdClear.le['location' + this.selectedLocation] !== undefined 
        && this.$store.state.trdClear.le['location' + this.selectedLocation] !== null 
        && Object.keys(this.$store.state.trdClear.le['location' + this.selectedLocation]).length > 0) {
          for (let key in this.$store.state.trdClear.le['location' + this.selectedLocation]) {
            data.push([new Date(key).getTime(), this.$store.state.trdClear.le['location' + this.selectedLocation][key]])
          }
        }
        return data
      },
      ...
      ChartRawWaterFlow: function () {
        ...
        chart.series = []
        if (this.$store.state.trdReceiving.latest.ai_b_in_fr_rtd !== null && Object.keys(this.$store.state.trdReceiving.latest.ai_b_in_fr_rtd).length > 0) {
          let dataSeries1 = []
          for (let key in this.$store.state.trdReceiving.latest.ai_b_in_fr_rtd) {
            dataSeries1.push([new Date(key).getTime(), parseInt(this.$store.state.trdReceiving.latest.ai_b_in_fr_rtd[key])])
          }
          chart.series.push({
            name: '유입 유량',
            data: dataSeries1,
    ...
    methods: {
      downloadExcel: function() {
        let ret = this.$store.state.trdClear.le
        console.log('ret', ret)
