...
export const GET_INDCOAGULANT_LATEST = 'indCoagulants/get'
...
export const PUT_COAGULANT_CONTROL_COAGULANT = 'indCoagulants/control/coagulant/put'
...
export default {
  namespaced: true,
  state: {
    coagulant_type: ['PACS', 'APAC', 'Polymax', 'Mid-BACS'],
    isModifyMode: false,
  ...
  mutations: {
    ...
    [PUT_CONTROL_COAGULANT]: function (state, c1_cf_coagulant){
      state.latest.c1_cf_coagulant = c1_cf_coagulant
    }
  },
  actions: {
    ...
    [PUT_CONTROL_COAGULANT]: async function ({ commit }, { c1_cf_coagulant }) {
      await axios.put(`${DEV_SERVER}/${URL.COAGULANT_CONTROL_COAGULANT}/` + PROCESS_STEP, { c1_cf_coagulant })
      .then((response) => {
        let _data = {
          visible: true,
          title: '설정 성공',
        ...
        commit(PUT_CONTROL_COAGULANT, c1_cf_coagulant)
        commit('alertDialog/OPEN_DIALOG', _data, { root: true })
        console.log('date', response)
      })
      .catch(error => {
