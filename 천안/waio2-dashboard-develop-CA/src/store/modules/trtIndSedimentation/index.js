...
export const GET_TRTINDSEDIMENTATION_LATEST = 'trtIndSedimentation/latest/get'
export const GET_SEDIMENTATION_LOCATION_BY_JI = 'trtIndSedimentation/location/numJi/get'
export const PUT_SEDIMENTATION_INTERFACE = 'trtIndSedimentation/history/interface/numJi/put'
export const GET_SEDIMENTATION_DISTRIBUTION_TB = 'trtIndSedimentation/distribution/tb/get'
export const PUT_SEDIMENTATION_HISTORY_TB = 'trtIndSedimentation/history/tb/put'
export const PUT_SEDIMENTATION_HISTORY_CL = 'trtIndSedimentation/history/cl/put'
export const PUT_TRTINDSEDIMENTATION_CONTROL_OPERATION = 'trtIndSedimentation/control/operation/put'
export const PUT_SEDIMENTATION_CONTROL_SC = 'trtIndSedimentation/control/sc/put'
export const PUT_SEDIMENTATION_CONTROL_LOCATION = 'trtIndSedimentation/control/location/put'
const GET_LATEST = GET_TRTINDSEDIMENTATION_LATEST.substr(GET_TRTINDSEDIMENTATION_LATEST.indexOf('/') + 1)
...
const PUT_CONTROL_OPERATION = PUT_TRTINDSEDIMENTATION_CONTROL_OPERATION.substr(PUT_TRTINDSEDIMENTATION_CONTROL_OPERATION.indexOf('/') + 1)
...
import { CLOSE_TRTIND_AI_MODE_OF_JI_DIALOG } from '@/store/modules/dialog'
...
export default {
  ...
  getters: {
    ...
    isOpenInBalValve5: function (state) {
      if (state.latest.e_drawing_vv5_1 === 1 || state.latest.e_drawing_vv5_2 === 1 || state.latest.e_drawing_vv5_3 === 1 || state.latest.e_drawing_vv5_4 === 1 || state.latest.e_drawing_vv5_5 || state.latest.e_drawing_vv5_6) {
        return true
      } else {
        return false
      }
    },
    isOperateJi1: function (state) {
      if (state.latest.e_operation_mode_1 === 1) {
        return true
      } else {
        return false
      }
    },
    isOperateJi2: function (state) {
      if (state.latest.e_operation_mode_2 === 1) {
        return true
      } else {
        return false
      }
    },
    isOperateJi3: function (state) {
      if (state.latest.e_operation_mode_3 === 1) {
        return true
      } else {
        return false
      }
    },
    isOperateJi4: function (state) {
      if (state.latest.e_operation_mode_4 === 1) {
        return true
      } else {
        return false
      }
    },
    isOperateJi5: function (state) {
      if (state.latest.e_operation_mode_5 === 1) {
        return true
      } else {
        return false
      }
  ...
  actions: {
    ...
    [PUT_INTERFACE]: async function ({ commit }, obj) {
      ...
      await axios.put(`${DEV_SERVER}/${URL.SEDIMENTATION_HISTORY_INTERFACE}/${obj.numJi}/` + PROCESS_STEP, { 'start_time': new Date(nowTimestamp - oneDayTimestamp).toISOString(), 'end_time': new Date(nowTimestamp).toISOString(), 'processStep' : PROCESS_STEP })
    ...
    [PUT_HISTORY_TB]: async function ({ commit }) {
      let nowTimestamp = Date.now()
      // nowTimestamp = Date.parse("2013-09-04 10:00:00"); // FIXME 현재 날짜 수정
      let oneDayTimestamp = 1000 * 60 * 60 * 24
    ...
    [PUT_HISTORY_CL]: async function ({ commit }, {disinfectionStep}) {
      let nowTimestamp = Date.now()
      // nowTimestamp = Date.parse("2023-08-21 10:00:00"); // FIXME 현재 날짜 수정
      let oneDayTimestamp = 1000 * 60 * 60 * 24
      await axios.put(`${DEV_SERVER}/${URL.SEDIMENTATION_HISTORY_CL}/` + PROCESS_STEP + `/` + disinfectionStep, { 'start_time': new Date(nowTimestamp - oneDayTimestamp).toISOString(), 'end_time': new Date(nowTimestamp).toISOString() })
        .then(({ data }) => {
    ...
    [PUT_CONTROL_LOCATION]: async function ({ commit }, { numJi, ai }) {
      ...
      .then(() => {
        ...
        commit('dialog/'+ CLOSE_TRTIND_AI_MODE_OF_JI_DIALOG, null, { root: true })
        let _data = {
          visible: true,
          title: '설정 성공',
          text1: '설정값이 변경되었습니다'
