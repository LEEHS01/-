...
export const GET_PUMPRECEIVING_LATEST = 'pumpReceiving/latest/get'
export const PUT_PUMPRECEIVING_HISTORY_FLOW_OUT = 'pumpReceiving/latest/put'
export const PUT_PUMPRECEIVING_CONTROL_OPERATION = 'pumpReceiving/control/operation/put'
export const PUT_PUMPRECEIVING_CONTROL_LEVEL = 'pumpReceiving/control/le/put'
const GET_LATEST = GET_PUMPRECEIVING_LATEST.substr(GET_PUMPRECEIVING_LATEST.indexOf('/') + 1)
const PUT_HISTORY_FLOW_OUT = PUT_PUMPRECEIVING_HISTORY_FLOW_OUT.substr(PUT_PUMPRECEIVING_HISTORY_FLOW_OUT.indexOf('/') + 1)
const PUT_CONTROL_OPERATION = PUT_PUMPRECEIVING_CONTROL_OPERATION.substr(PUT_PUMPRECEIVING_CONTROL_OPERATION.indexOf('/') + 1)
const PUT_CONTROL_LEVEL = PUT_PUMPRECEIVING_CONTROL_LEVEL.substr(PUT_PUMPRECEIVING_CONTROL_LEVEL.indexOf('/') + 1)
...
export default {
  ...
  mutations: {
    [GET_LATEST]: function(state, data) {
      ...
      if (state.isModifyMode === false) {
        state.latestModify = Object.assign({})
      }
    },
  ...
  actions: {
    ...
    [PUT_CONTROL_LEVEL]: async function ({ commit }, { h_target_le_max, h_target_le_min }) {
      await axios.put(`${DEV_SERVER}/${URL.RECEIVING_CONTROL_LEVEL}/` + PROCESS_STEP, { h_target_le_max, h_target_le_min })
      .then(() => {
        let _data = {
          visible: true,
          title: '설정 성공',
