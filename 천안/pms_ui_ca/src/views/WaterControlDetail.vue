                    ...
                    <div class="chart-area">
                        <div class="chart-con">
                            ...
                            <Linechart
                                ...
                                name1="과전류"
                                :detailData="
                                    store.state.monitor3.detailData.current
                                "
                            />
                        </div>
                    </div>
                    <div class="chart-area">
                        <div class="chart-con">
                            ...
                            <Linechart
                                ...
                                name1="전압변동률"
                                :detailData="
                                    store.state.monitor3.detailData
...
export default {
    ...
    setup() {
        ...
        const state = reactive({
            model: store.state.monitor3.selectModel,
            options: computed(() => {
                let list = [];
                for (
                    let i = 0;
                    i < store.state.monitor3.modelList.length;
                    i++
                ) {
                    list.push(store.state.monitor3.modelList[i].title);
                }
                return list;
            }),
            tab: '가동여부',
        ...
        onMounted(() => {
            ...
            todayDate();
            dateToNumber();
            store.dispatch('monitor3/handleDatePicker');
        });

        const todayDate = () => {
            state.date.from = store.state.monitor3.startDate.split(' ')[0];
            state.date.to = store.state.monitor3.endDate.split(' ')[0];
        ...
        const dateToNumber = () => {
            if (state.date.from && state.date.to) {
                  store.state.monitor3.pickDate.from = Number(
                      state.date.from.replaceAll('-', '')
                  );
                  store.state.monitor3.pickDate.to = Number(
        ...
        const selectModel = (value) => {
            let select = store.state.monitor3.modelList.filter(
                (v) => v.title === value
            );
            store.state.monitor3.selectedMotor = select[0].title;
            store.state.monitor3.id = select[0].id;
            store.dispatch('monitor3/handleDatePicker');
        };
