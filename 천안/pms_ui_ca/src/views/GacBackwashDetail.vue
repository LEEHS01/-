                    ...
                    <div class="chart-area">
                        <div class="chart-con">
                            ...
                            <Linechart
                                ...
                                name1="과전류"
                                :detailData="
                                    store.state.monitor12.detailData
                                        .current_limit
                                "
                            />
                        </div>
...
export default {
    ...
    setup() {
        ...
        const state = reactive({
            model: store.state.monitor12.selectModel,
            options: computed(() => {
                let list = [];
                for (
                    let i = 0;
                    i < store.state.monitor12.modelList.length;
                    i++
                ) {
                    list.push(store.state.monitor12.modelList[i].title);
                }
                return list;
            }),
            datePop: false,
            ...
            startStr: '',
            endStr: '',
            detailInfo: computed(() => store.getters['monitor12/detailInfo']),
        });
        ...
        onMounted(() => {
            ...
            todayDate();
            dateToNumber();
            store.dispatch('monitor12/handleDatePicker');
        });

        const todayDate = () => {
            state.date.from = store.state.monitor12.startDate.split(' ')[0];
            state.date.to = store.state.monitor12.endDate.split(' ')[0];
        ...
        const dateToNumber = () => {
            if (state.date.from && state.date.to) {
                  store.state.monitor12.pickDate.from = Number(
                      state.date.from.replaceAll('-', '')
                  );
                  store.state.monitor12.pickDate.to = Number(
        ...
        const selectModel = (value) => {
            let select = store.state.monitor12.modelList.filter(
                (v) => v.title === value
            );
            store.state.monitor12.selectModel = select[0].title;
            store.state.monitor12.id = select[0].id;
            store.dispatch('monitor12/handleDatePicker');
        };
        const back = () => {
            router.push('GacBackwashView');
        };
