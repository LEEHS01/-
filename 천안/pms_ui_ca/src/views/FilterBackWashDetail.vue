                    ...
                    <div class="chart-area">
                        <div class="chart-con">
                            ...
                            <Linechart
                                ...
                                name1="과전류"
                                :detailData="
                                    store.state.monitor10.detailData
                                        .current_limit
                                "
                            />
                        </div>
...
export default {
    ...
    setup() {
        ...
        const state = reactive({
            model: store.state.monitor10.selectModel,
            options: computed(() => {
                let list = [];
                for (
                    let i = 0;
                    i < store.state.monitor10.modelList.length;
                    i++
                ) {
                    list.push(store.state.monitor10.modelList[i].title);
                }
                return list;
            }),
            datePop: false,
            ...
            startStr: '',
            endStr: '',
            detailInfo: computed(() => store.getters['monitor10/detailInfo']),
        });
        ...
        onMounted(() => {
            ...
            todayDate();
            dateToNumber();
            store.dispatch('monitor10/handleDatePicker');
        });

        const todayDate = () => {
            state.date.from = store.state.monitor10.startDate.split(' ')[0];
            state.date.to = store.state.monitor10.endDate.split(' ')[0];
        ...
        const dateToNumber = () => {
            if (state.date.from && state.date.to) {
                  store.state.monitor10.pickDate.from = Number(
                      state.date.from.replaceAll('-', '')
                  );
                  store.state.monitor10.pickDate.to = Number(
        ...
        const selectModel = (value) => {
            let select = store.state.monitor10.modelList.filter(
                (v) => v.title === value
            );
            store.state.monitor10.selectModel = select[0].title;
            store.state.monitor10.id = select[0].id;
            store.dispatch('monitor10/handleDatePicker');
        };
        const back = () => {
            router.push('FilterBackWashView');
        };
