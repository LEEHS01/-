...
export default {
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            store.dispatch('monitor17/handleGraphData');
            store.dispatch('monitor17/getPumpInfo');
        });

        const clickParam = (value) => {
            store.state.monitor17.selectModel = value.title;
            store.state.monitor17.modelList.filter((list) => {
                if (list.title === value.title)
                    store.state.monitor17.id = list.id;
            });
            router.push('TransformersDetail');
        };
