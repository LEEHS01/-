...
export default {
    ...
    // 수집기 운영 스케줄 차트
      chartDataSchedule: {
        ...
        yAxis: {
          ...
          gridLineColor: false,
          categories : ['8지','7지','6지','5지','4지','3지','2지','1지'],
          labels: {
            style:{
              fontFamily:'NanumSquare',
              fontSize: 13,
  ...
  computed: {
    ...
    ChartSchedule: function () {
      ...
      let minMax = this.$getMinMaxTimestampAIELocationSchedule(this.$store.state.indSedimentation.latest)
      //1. 가장 작은 시간
      ...
      for (let i = 1; i < 9; i ++) {
        // 현재 대차 스케줄
        if (this.$store.state.indSedimentation.latest['e_sc' + i + '_schedule'].start !== null 
        && this.$store.state.indSedimentation.latest['e_sc' + i + '_schedule'].stop !== null) {    
          data.push({
            x: new Date(this.$store.state.indSedimentation.latest['e_sc' + i + '_schedule'].start).getTime(),
            x2: new Date(this.$store.state.indSedimentation.latest['e_sc' + i + '_schedule'].stop).getTime(),
            y: 8 - i,
            color: colorXRange
          })
        }
        // 다음 대차 스케줄
        if (this.$store.state.indSedimentation.latest['e_sc' + i + '_schedule'].next_start !== null 
        && this.$store.state.indSedimentation.latest['e_sc' + i + '_schedule'].next_end !== null) {    
          data.push({
            x: new Date(this.$store.state.indSedimentation.latest['e_sc' + i + '_schedule'].next_start).getTime(),
            x2: new Date(this.$store.state.indSedimentation.latest['e_sc' + i + '_schedule'].next_end).getTime(),
            y: 8 - i,
            color: colorXRange
          })
        }
        // inbal 스케줄
        if (this.$store.state.indSedimentation.latest['e_sc' + i + '_schedule'].inbal !== null 
        && this.$store.state.indSedimentation.latest['e_sc' + i + '_schedule'].inbal !== '') {
          data.push({
            x: new Date(this.$store.state.indSedimentation.latest['e_sc' + i + '_schedule'].inbal).getTime(),
            x2: new Date(this.$store.state.indSedimentation.latest['e_sc' + i + '_schedule'].inbal).getTime() + inbalTimeLength,
            y: 8 - i,
            color: colorInbal
          })
        }
  ...
  methods: {
    ...
    downloadXLSX: function() {
      ...
      for (const excel of ret) {
        ...
        if(this.$moment(excel.x).isValid()) {
