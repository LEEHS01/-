      ...
      <div class="popup__body">
        <div class="popup__wrap" :style=" { maxHeight: containerHeight + 'px', overflowY: containerHeight > 200 ? 'auto' : 'hidden' }">
          <ul style="padding-right:10px;">
            <li v-for="(item,index) in this.ctrList.filter((it) => it.tag_sn != null)" :key="index">
...
<script>
import { PUT_ALARM_CONTROL, PUT_ALARM_CONTROL_HISTORY } from '@/store/modules/alarm/alarm'
  ...
  methods: {
    async closePopup() {
      if(this.alm_ty === 2 || this.alm_ty === 4) {
        if (this.$store.state.login.user.tkn === null) {
          await this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '로그인이 필요한 기능입니다' })
        } else {
          await this.almHistoryInsert('N')
          await this.$store.dispatch(PUT_ALARM_CONTROL, {alm_id: this.alarmId, alm_ntf_ti: this.alm_ntf_ti, kfk_flg: 4})
          await this.$store.commit(DEL_NOTIFY, this.seq)
        }
    ...
    goToURL: function () {
      ...
      if (_url.includes(_host)) {
        ...
        this.$store.dispatch(PUT_ALARM_CONTROL, {alm_id: this.alarmId, alm_ntf_ti: this.alm_ntf_ti, kfk_flg: 2})
      // 다른 호스트
      } else {
        if (this.$store.state.login.user.tkn !== null) {
    ...
    async control() {
      if (this.$store.state.login.user.tkn === null) {
        ...
        await this.almHistoryInsert('Y')
        await this.$store.dispatch(PUT_ALARM_CONTROL, {alm_id: this.alarmId, alm_ntf_ti: this.alm_ntf_ti, kfk_flg: 3})
        await this.$store.commit(DEL_NOTIFY, this.seq)
      }
    },
    almHistoryInsert: function(ctrYn) {
      ...
      return this.$store.dispatch(PUT_ALARM_CONTROL_HISTORY, obj)
    },
