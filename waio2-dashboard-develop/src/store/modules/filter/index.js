...
export const PUT_FILTER_CONTROL = URL.FILTER_CONTROL + '/put'
...
const PUT_CONTROL = PUT_FILTER_CONTROL.substr(PUT_FILTER_CONTROL.indexOf('/') + 1)
...
export default {
  ...
  actions: {
    ...
    [PUT_CONTROL]: async function ({ commit }, { f_location_ti_set_max, f_location_wl_max, f_h_le , f_back_le  }) {
      await axios.put(`${DEV_SERVER}/${URL.FILTER_CONTROL}/` + PROCESS_STEP, { f_location_ti_set_max, f_location_wl_max, f_h_le , f_back_le  })
      .then(() => {
        commit(PUT_CONTROL, {f_location_ti_set_max, f_location_wl_max, f_h_le , f_back_le })
        commit('dialog/'+ CLOSE_AI_MODE_DIALOG, null, { root: true })
        let _data = {
          visible: true,
          title: '설정 성공',
