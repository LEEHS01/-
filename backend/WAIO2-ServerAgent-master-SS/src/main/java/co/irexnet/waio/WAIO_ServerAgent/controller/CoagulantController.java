...
//        strPartitionName = "20221231";	// FIXME 현재 날짜 수정
        ...
        if(aiCoagulantRealtime != null)
        {
            ...
            Float b_in_fr = 0.0f;
			Float b_in_fr_ctr = 0.0f;
			Float b_in_fr2 = 0.0f;
			Float b_in_fr2_ctr = 0.0f;
			Float b_in_fr3 = 0.0f;
			Float b_in_fr3_ctr = 0.0f;
			Float b_rtrn_in_fr = 0.0f;
			...
			for(TagManageDTO tagManage: tagManageList)
			{
			    for(ProcessRealtimeDTO dto : coagulantRealtime)
			    {
			        ...
			        //여기부터 유입 유량 계산 관련 추가 태그
			        else if(tagManage.getItm().equalsIgnoreCase("b_in_fr2") == true &&
			                tagManage.getTag_sn().equalsIgnoreCase(dto.getTag_sn()) == true)
			        {
			            //부여(취) 토출유량
			        	b_in_fr2 = Float.parseFloat(dto.getTag_val());
			            break;
			        }
			        else if(tagManage.getItm().equalsIgnoreCase("b_in_fr2_ctr") == true &&
			                tagManage.getTag_sn().equalsIgnoreCase(dto.getTag_sn()) == true)
			        {
			            //석성공업(정) 부여취수장 유량선택 CMD
			        	b_in_fr2_ctr = Float.parseFloat(dto.getTag_val());
			            break;
			        }
			        ...
			        {
			            //석성공업(정) 사용자 입력 원수유량
			        	b_in_fr3 = Float.parseFloat(dto.getTag_val());
			            break;
			        }
			        else if(tagManage.getItm().equalsIgnoreCase("b_in_fr3_ctr") == true &&
			                tagManage.getTag_sn().equalsIgnoreCase(dto.getTag_sn()) == true)
			        {
			            //석성공업(정) 사용자 입력 유량선택 CMD
			        	b_in_fr3_ctr = Float.parseFloat(dto.getTag_val());
			            break;
			        }
			        else if(tagManage.getItm().equalsIgnoreCase("b_in_fr_ctr") == true &&
			                tagManage.getTag_sn().equalsIgnoreCase(dto.getTag_sn()) == true)
			        {
			            //석성공업(정) 원수유량선택 CMD
			        	b_in_fr_ctr = Float.parseFloat(dto.getTag_val());
			            break;
			        }
			        ...
			        {
			            //석성(정) 배출수 회수조 유출유량
			        	b_rtrn_in_fr = Float.parseFloat(dto.getTag_val());
			            break;
			        }
			    }
			}
...
//            aiCoagulantInfo.put("c2_mm_fr", d2_mm_fr_apac > d2_mm_fr_polymax ? d2_mm_fr_apac : d2_mm_fr_polymax);
			...
			if(processStep == 1) {
				...
				aiCoagulantInfo.put("c1_cf", d1_mm_fr_pacs/(((b_in_fr * b_in_fr_ctr) + (b_in_fr2 * b_in_fr2_ctr) + (b_in_fr3 * b_in_fr3_ctr)) + b_rtrn_in_fr) * 1000);
				aiCoagulantInfo.put("c2_cf", d2_mm_fr_pacs/(((b_in_fr * b_in_fr_ctr) + (b_in_fr2 * b_in_fr2_ctr) + (b_in_fr3 * b_in_fr3_ctr)) + b_rtrn_in_fr) * 1000);		
			}
