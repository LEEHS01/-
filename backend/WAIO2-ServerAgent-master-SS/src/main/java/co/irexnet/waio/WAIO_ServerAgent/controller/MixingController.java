...
//        strPartitionName = "20231119"; // FIXME 현재 날짜 수정
...
//                databaseService.modAiMixingOperationMode(nOperationMode), nOperationMode);
     ...
     * @return ResponseEntity<String> 메시지
     */
    @RequestMapping(value = "/mixing/control/ai/{processStep}/{jiNum}", method = RequestMethod.PUT)
    public ResponseEntity<String> putAiControlMixing(@RequestBody InterfaceMixingAiDTO mixingAi, @PathVariable int processStep, @PathVariable int jiNum) throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException
    {
        ...
        if(d_g_value_ctr_flag == 0)  {// 수동인 경우 G값 변경        	
    		...
    		result = (databaseService.modAiMixingInit("d_g_value_step"+jiNum+"_1", (float)mixingAi.getClass().getMethod("getD_g_value_step"+jiNum+"_1").invoke(mixingAi), processStep) == 1) && result;
    		
    		// update 응집기 2단 교반강도 G 값
    		result = (databaseService.modAiMixingInit("d_g_value_step"+jiNum+"_2", (float)mixingAi.getClass().getMethod("getD_g_value_step"+jiNum+"_2").invoke(mixingAi), processStep) == 1) && result;
    		
    		// update 응집기 3단 교반강도 G 값
    		result = (databaseService.modAiMixingInit("d_g_value_step"+jiNum+"_3", (float)mixingAi.getClass().getMethod("getD_g_value_step"+jiNum+"_3").invoke(mixingAi), processStep) == 1) && result;
        }	
