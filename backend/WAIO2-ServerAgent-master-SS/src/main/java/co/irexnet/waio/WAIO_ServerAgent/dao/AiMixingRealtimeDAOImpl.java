...
public class AiMixingRealtimeDAOImpl implements IAiMixingRealtimeDAO
{
    ...
    public AiMixingRealtimeDTO select(int processStep)
    {
        ...
        + ", SUBSTR(JSON_EXTRACT(IN_VAL, '$[*].D_PW'), 2, LENGTH(JSON_EXTRACT(IN_VAL, '$[*].D_PW'))-2) AS d_pw"
        + ", SUBSTR(JSON_EXTRACT(IN_VAL, '$[*].D_IM_D'), 2, LENGTH(JSON_EXTRACT(IN_VAL, '$[*].D_IM_D'))-2) AS d_im_d"
        + ", SUBSTR(JSON_EXTRACT(IN_VAL, '$[*].D_G.location1'), 2, LENGTH(JSON_EXTRACT(IN_VAL, '$[*].D_G.location1'))-2) AS d_g"
...
//        + ", SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AI_D_LOC_FC_SP'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AI_D_LOC_FC_SP'))-2) AS ai_d_loc_fc_sp FROM " + getTableByProcessStep(processStep) + " ORDER BY upd_ti DESC LIMIT 1";
        ...
        + ", SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AI_D_LOC_FC_SP'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AI_D_LOC_FC_SP'))-2) AS ai_d_loc_fc_sp FROM " + getTableByProcessStep(processStep) + " ORDER BY upd_ti DESC LIMIT 1";
//        + ", SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AI_D_LOC_FC_SP'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AI_D_LOC_FC_SP'))-2) AS ai_d_loc_fc_sp FROM " + getTableByProcessStep(processStep) + " ORDER BY upd_ti ASC LIMIT 1";
//        
//        TODO: 날짜 하드코딩 제거
//        if(processStep == 2) {
//            strQuery += ", SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AI_D_LOC_FC_SP'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AI_D_LOC_FC_SP'))-2) AS ai_d_loc_fc_sp FROM " + getTableByProcessStep(processStep) + " WHERE UPD_TI ='2013-09-04 00:00:00.000'";
//        }else {
//            strQuery += ", SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AI_D_LOC_FC_SP'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AI_D_LOC_FC_SP'))-2) AS ai_d_loc_fc_sp FROM " + getTableByProcessStep(processStep) + " ORDER BY upd_ti DESC LIMIT 1";        	
//        }
    ...
    public String getTableByProcessStep(int processStep) {
        String tableNm = "";
        if(processStep == 1) {
            tableNm = "TB_AI_D_LIV_RT";
        } else if(processStep == 2) {
            tableNm = "TB_AI_D_IND_RT";
        }
        return tableNm;
    }
}
