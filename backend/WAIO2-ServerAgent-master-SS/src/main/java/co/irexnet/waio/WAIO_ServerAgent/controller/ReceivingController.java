...
import co.irexnet.waio.WAIO_ServerAgent.dto.InterfaceReceivingHzDTO;
...
//        strPartitionName = "20221231"; // FIXME 현재 날짜 수정
        ...
        if(aiReceivingRealtime != null)
        {
            ...
            {	
                ...
                // get ai_receiving_realtime -- 인버터 예측 (dashboard 데이터용)
                AiReceivingRealtimeDTO aiReceivingRealtimeForDashboard = databaseService.getLatestAiReceivingRealtimeValue(2);
            	aiReceivingInfo.put("ai_b_hz_dashboard", aiReceivingRealtimeForDashboard.getAi_b_hz());
                
                // ems_mode
                aiReceivingInfo.put("ems_mode", aiReceivingRealtime.getEms_opr());

                //인버터 Hz 예측
            	aiReceivingInfo.put("ai_b_hz", aiReceivingRealtime.getAi_b_hz());
                ...
                for(AiProcessInitDTO dto : aiReceivingInitList)
                {
                    ...
                    else if(dto.getItm().equalsIgnoreCase("b_hz_max") == true)
                    {
                        // 인버터 Hz 상한
                        aiReceivingInfo.put(dto.getItm(), dto.getInit_val());
                    }
                    else if(dto.getItm().equalsIgnoreCase("b_hz_min") == true)
                    {
                        // 인버터 Hz 하한
                        aiReceivingInfo.put(dto.getItm(), dto.getInit_val());
                    }
                ...
                for(TagManageDTO tagManage : tagManageList)
                {
                    for(ProcessRealtimeDTO dto : receivingRealtime)
                    {
                        ...
                        else if(tagManage.getItm().equalsIgnoreCase("b_hz") == true &&
                                tagManage.getTag_sn().equalsIgnoreCase(dto.getTag_sn()) == true)
                        {
                            // 인버터 Hz
                            aiReceivingInfo.put(tagManage.getItm(), Float.parseFloat(dto.getTag_val()));
                        }
                        else if(tagManage.getItm().equalsIgnoreCase("b_rtrn_in_fr") == true &&
                                tagManage.getTag_sn().equalsIgnoreCase(dto.getTag_sn()) == true)
                        {
                            // 회수조 유출유량
                            aiReceivingInfo.put(tagManage.getItm(), Float.parseFloat(dto.getTag_val()));
            ...
            try
            {
                ...
                for(AiReceivingRealtimeDTO receivingDTO : aiReceivingRealtimeList)
                {
                    ...
                    if(processStep == 1) {
                        ...
                        location4.put(strDate, Float.valueOf(receivingDTO.getH_loc_le()));
                    }
                }
...
//                databaseService.modAiReceivingOperationMode(nOperationMode), nOperationMode);
     ...
     * 착수 인버터 Hz 설정
     * 
     * @param receivingHz     Front-end 착수 인버터 Hz 값을 저장하기 위한 DTO
     * @param processStep 공정단계
     * @return ResponseEntity<String> 메시지
     */
    @RequestMapping (value = "/receiving/control/hz/{processStep}", method = RequestMethod.PUT)
    public ResponseEntity<String> putHzControlReceiving(@RequestBody InterfaceReceivingHzDTO receivingHz, @PathVariable int processStep){
        
        log.debug("putHzControlReceiving, receivingHz:[{}]", receivingHz);
        boolean result = true;
        
        // 인버터 Hz 상한
        result = (databaseService.modAiReceivingInit("b_hz_max", receivingHz.getB_hz_max(), processStep) == 1) && result;

        // 인버터 Hz 하한
        result = (databaseService.modAiReceivingInit("b_hz_min", receivingHz.getB_hz_min(), processStep) == 1) && result;
