...
//		    		    + "(SELECT "
//		    		    + "    CASE "
//		    		    + "        WHEN TAI.DP_NM LIKE '%소독%' THEN SUBSTRING_INDEX(TAI.DP_NM, ' ', 3)"
//		    		    + "        WHEN TAI.DP_NM LIKE '%여과%' THEN SUBSTRING_INDEX(TAI.DP_NM, ' ', 1)"
//		    		    + "        ELSE SUBSTRING_INDEX(TAI.DP_NM, ' ', 2)"
//		    		    + "    END AS PROCESS_NAME "
//		    		    + ") AS PROCESS_NAME, "
//		    		    + "TAI.ALM_ID "
...
//            "FROM TB_AI_B_LIV_CTR " +
//    		"WHERE UPD_TI BETWEEN ? AND ? " +
//            "UNION ALL " +
//            "SELECT TAG_SN, UPD_TI, TAG_VAL, TAG_CMP_VAL " +
//            "FROM TB_AI_B_IND_CTR " +
//    		"WHERE UPD_TI BETWEEN ? AND ? " +
//            "UNION ALL " +
//            "SELECT TAG_SN, UPD_TI, TAG_VAL, TAG_CMP_VAL " +
//            "FROM TB_AI_C_LIV_CTR " +
//    		"WHERE UPD_TI BETWEEN ? AND ? " +
//            "UNION ALL " +
//            "SELECT TAG_SN, UPD_TI, TAG_VAL, TAG_CMP_VAL " +
//            "FROM TB_AI_C_IND_CTR " +
//    		"WHERE UPD_TI BETWEEN ? AND ? " +
//            "UNION ALL " +
//            "SELECT TAG_SN, UPD_TI, TAG_VAL, TAG_CMP_VAL " +
//            "FROM TB_AI_D_LIV_CTR " +
//    		"WHERE UPD_TI BETWEEN ? AND ? " +
//            "UNION ALL " +
//            "SELECT TAG_SN, UPD_TI, TAG_VAL, TAG_CMP_VAL " +
//            "FROM TB_AI_D_IND_CTR " +
//    		"WHERE UPD_TI BETWEEN ? AND ? " +
//            "UNION ALL " +
//            "SELECT TAG_SN, UPD_TI, TAG_VAL, TAG_CMP_VAL " +
//            "FROM TB_AI_E_LIV_CTR " +
//    		"WHERE UPD_TI BETWEEN ? AND ? " +
//            "UNION ALL " +
//            "SELECT TAG_SN, UPD_TI, TAG_VAL, TAG_CMP_VAL " +
//            "FROM TB_AI_E_IND_CTR " +
//    		"WHERE UPD_TI BETWEEN ? AND ? " +
//            "UNION ALL " +
//            "SELECT TAG_SN, UPD_TI, TAG_VAL, TAG_CMP_VAL " +
//            "FROM TB_AI_F_LIV_CTR " +
//    		"WHERE UPD_TI BETWEEN ? AND ? " +
//            "UNION ALL " +
//            "SELECT TAG_SN, UPD_TI, TAG_VAL, TAG_CMP_VAL " +
//            "FROM TB_AI_PRE_G_LIV_CTR " +
//    		"WHERE UPD_TI BETWEEN ? AND ? " +
//            "UNION ALL " +
//            "SELECT TAG_SN, UPD_TI, TAG_VAL, TAG_CMP_VAL " +
//            "FROM TB_AI_PERI_G_LIV_CTR " +
//    		"WHERE UPD_TI BETWEEN ? AND ? " +
//            "UNION ALL " +
//            "SELECT TAG_SN, UPD_TI, TAG_VAL, TAG_CMP_VAL " +
//            "FROM TB_AI_POST_G_LIV_CTR " +
//    		"WHERE UPD_TI BETWEEN ? AND ? " +
//            "UNION ALL " +
//            "SELECT TAG_SN, UPD_TI, TAG_VAL, TAG_CMP_VAL " +
//            "FROM TB_AI_PRE_G_IND_CTR " +
//    		"WHERE UPD_TI BETWEEN ? AND ? " +
//            ") " +
            "SELECT " +
            "    A.SEQ, " +
            ...
            "    (SELECT " +
            "        CASE " +
            "            WHEN B.DP_NM LIKE '%소독%' THEN SUBSTRING_INDEX(B.DP_NM, ' ', 3) " +
            "            WHEN B.DP_NM LIKE '%여과%' THEN SUBSTRING_INDEX(B.DP_NM, ' ', 1) " +
            "            ELSE SUBSTRING_INDEX(B.DP_NM, ' ', 2) " +
            "        END " +
            "    ) AS PROCESS_NAME, " +
...
//            "    IF(COUNT(ctr_tables.TAG_CMP_VAL) >= 2, '-', ctr_tables.TAG_CMP_VAL) AS TAG_CMP_VAL, " +
            "    ( CASE " +
            "        WHEN SUBSTRING(B.ALM_ID, 3, 1) = 1 THEN 'B' " +
            "        WHEN SUBSTRING(B.ALM_ID, 3, 1) = 2 THEN 'C' " +
            "        WHEN SUBSTRING(B.ALM_ID, 3, 1) = 3 THEN 'D' " +
            "        WHEN SUBSTRING(B.ALM_ID, 3, 1) = 4 THEN 'E' " +
            "        WHEN SUBSTRING(B.ALM_ID, 3, 1) = 5 THEN 'F' " +
...
//	    		updTiStartTime, end_time,
    ...
    public String getCtrTblNm(int almId) {
    	...
    	String cdNm = "";
    	if(alarmInfoList.getAlarmInfoFromAlarmId(almId) != null) {
    		cdNm = alarmInfoList.getAlarmInfoFromAlarmId(almId).getCd_nm();    		
    	}
    	String processStep = Integer.toString(almId).charAt(3) == '1' ? "LIV" : "IND";
        if(CommonValue.ALARM_CODE_RECEIVING_AI_CONTROL.equals(cdNm)|| CommonValue.ALARM_CODE_RECEIVING_AI_THRESHOLD.equals(cdNm)) {
        	ctrTblNm +="B_"+processStep;
        } else if (CommonValue.ALARM_CODE_COAGULANT_AI_CONTROL.equals(cdNm) || CommonValue.ALARM_CODE_COAGULANT_AI_THRESHOLD.equals(cdNm)) {
        	ctrTblNm +="C_"+processStep;
        } else if (CommonValue.ALARM_CODE_MIXING_AI_CONTROL.equals(cdNm)|| CommonValue.ALARM_CODE_MIXING_AI_THRESHOLD.equals(cdNm)) {
        	ctrTblNm +="D_"+processStep;
        } else if (CommonValue.ALARM_CODE_SEDIMENTATION_AI_CONTROL.equals(cdNm)) {
        	ctrTblNm +="E_"+processStep;
        } else if (CommonValue.ALARM_CODE_FILTER_AI_CONTROL.equals(cdNm)) {
        	ctrTblNm +="F_"+processStep;
        	...
        	ctrTblNm +="PRE_G_"+processStep;
        } else if (CommonValue.ALARM_CODE_DISINFECTION_AI_PERI_CONTROL.equals(cdNm)|| CommonValue.ALARM_CODE_DISINFECTION_AI_PERI_THRESHOLD.equals(cdNm)) {
        	ctrTblNm +="PERI_G_"+processStep;
