...
//                databaseService.modAiFilterOperationMode(nOperationMode), nOperationMode);
    ...
    {	
        ...
        log.debug("putControlFilter, filterDto:[{}], ", filterDto);
        // 잘못된 값 검사
        if(nTi < 0 )
        {
     ...
     * @return
     */
    @RequestMapping(value= "/filter/control/location/{locationNumber}")
    public ResponseEntity<String> putLocationControlFilter(@PathVariable int locationNumber, @RequestBody InterfaceAiOnOffDTO aiOnOff)
    {
        ...
        // 지별 AI 모드 업데이트
        if(databaseService.modAiFilterInit("f_operation_ji" + locationNumber, (float)aiOnOff.getAi(), 1) == 1) {
        	return new ResponseEntity<>("", HttpStatus.OK);
        }else {
            String strErrorBody = "{\"reason\":\"ai_filter_init update_fail\"}";
            return new ResponseEntity<>(strErrorBody, HttpStatus.BAD_REQUEST);
