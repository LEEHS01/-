    ...
    public List<InterfaceAlarmControlHistoryDTO> selectAllRealTime(LocalDateTime today) {
        ...
        for (int processStep : CommonValue.PROCESS_STEP_ARRAY) {
            for (String process : processArr) {
                if(process.equals(CommonValue.PROCESS_FILTER) && (processStep == 2)) { // 여과는 1단계만
                    continue;
                }
                strQuery.append("(SELECT '").append(process).append(processStep == 1? "_LIV" : "_IND")
                        .append("' AS PROCESS, MAX(UPD_TI) AS UPD_TI FROM TB_")
                        .append(process).append(processStep == 1? "_LIV" : "_IND").append("_RT PARTITION(p_")
