...
//                    log.info("[Collector]Delete ai_filter_alarm:[{}]", databaseService.delAiFilterAlarm(deleteTime));
                	// 소독
...
//                        log.debug("Send AI_RESULT kafka ai_b_ti tag:[{}], value:[{}]", dto.getTag_sn(), strDate);
        				...
        				else if(dto.getItm().equalsIgnoreCase("ai_b_in_fri") == true)
        				{
        					...
        					kafkaProducer.sendMessageToLocal(CommonValue.KAFKA_TOPIC_RESULT, strBody);
    						sendCnt++;
        				}else if(dto.getItm().equalsIgnoreCase("ai_b_hz") == true) {
        					// 펌프 총 가동 대수 예측
        					controlMap = new LinkedHashMap<>();
        					controlMap.put("tag", dto.getTag_sn());
        					controlMap.put("value", aiReceivingRealtime.getAi_b_hz());
        					controlMap.put("time", strDate);
        					strBody = objectMapper.writeValueAsString(controlMap);
        					kafkaProducer.sendMessageToLocal(CommonValue.KAFKA_TOPIC_RESULT, strBody);
    						sendCnt++;
        				}
        				//석성은 원수 조절 밸브개도 데이터 없음
...
//                        kafkaProducer.sendMessageToLocal(CommonValue.KAFKA_TOPIC_RESULT, strBody);
                    ...
                    for(TagManageDTO dto : tagManageList)
                    {
                    	...
                    	else if(dto.getItm().equalsIgnoreCase("ai_c1_cf_coagulant") == true)
                    	{
                    		...
                    		controlMap = new LinkedHashMap<>();
                    		controlMap.put("tag", dto.getTag_sn());
                    		controlMap.put("value",coagulantUsrMngDTO.getInit_val());	// TODO 현재는 약품종류 PACS2호로 임의로 전송
                    		controlMap.put("time", strDate);
                    		strBody = objectMapper.writeValueAsString(controlMap);
                    		kafkaProducer.sendMessageToLocal(CommonValue.KAFKA_TOPIC_RESULT, strBody);
    						sendCnt++;
                    	}
                    	else if(dto.getItm().equalsIgnoreCase("ai_c2_cf_coagulant") == true)
                    	{
                    		// AI 약품 종류 예측
                    		controlMap = new LinkedHashMap<>();
                    		controlMap.put("tag", dto.getTag_sn());
                    	...
                    	else if(dto.getItm().equalsIgnoreCase("ai_c2_cf") == true)
                    	{
                    		// AI 약품 주입률 예측 최종값
                    		controlMap = new LinkedHashMap<>();
                    		controlMap.put("tag", dto.getTag_sn());
...
//                            log.debug("Send AI_RESULT kafka ai_d_ti tag:[{}], value:[{}]", dto.getTag_sn(), strDate);
                    ...
                    keyList = new ArrayList<>(mapTemp.keySet());    // location1...4
...
//                            break;
                    ...
                    // 데이터 값이 JSON으로 저장되어 있으므로 JSON 처리
                    // 1단계 생활
                    if(aiSedimentationRealtime.getAIE_9001() != null && processStep == 1) {
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4007");
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4008");
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4009");
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4010");
                    ...
                    if(aiSedimentationRealtime.getAIE_9001() != null && processStep == 2) {
                    	...
                    	// location1 - AI 수집기 대차 시작 시간 예측
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4023");
                    	if(location1Dto.getAIE_6001().getStart().equalsIgnoreCase("") == true)
                    	{
                    		controlMap.put("value", "--");
                    	}
                    	...
                    	// location1 - AI 수집기 대차 종료 시간 예측
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4024");
                    	if(location1Dto.getAIE_6001().getStop().equalsIgnoreCase("") == true)
                    	{
                    		controlMap.put("value", "--");
                    	}
                    	...
                    	// location1 - AI 다음 수집기 시작 시간 예측
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4025");
                    	if(location1Dto.getAIE_6001().getNext_start().equalsIgnoreCase("") == true)
                    	{
                    		controlMap.put("value", "--");
                    	}
                    	...
                    	// location1 - AI 인발 시간 예측 - 석성 공업은 인발 데이터 없음.
                    	// controlMap = new LinkedHashMap<>();
                    	// controlMap.put("tag", "740-351-SWI-4026");
                    	// if(location1Dto.getAIE_6001().getInbal().equalsIgnoreCase("") == true)
                    	// {
                    	// 	controlMap.put("value", "--");
                    	// }
                    ...
                    // 데이터 값이 JSON으로 저장되어 있으므로 JSON 처리
                    // 1단계 생활
                    if(aiSedimentationRealtime.getAIE_9002() != null && processStep == 1) {
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4011");
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4012");
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4013");
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4014");
                    ...
                    if(aiSedimentationRealtime.getAIE_9002() != null && processStep == 2) {
                    	...
                    	// location2 - AI 수집기 대차 종료 시간 예측
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4027");
                    	if(location2Dto.getAIE_6002().getStop().equalsIgnoreCase("") == true)
                    	{
                    		controlMap.put("value", "--");
                    	}
                    	...
                    	// location2 - AI 다음 수집기 시작 시간 예측
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4028");
                    	if(location2Dto.getAIE_6002().getNext_start().equalsIgnoreCase("") == true)
                    	{
                    		controlMap.put("value", "--");
                    	}
                    	...
                    	// location2 - AI 인발 시간 예측 -석성 공업 인발 없음
                    	// controlMap = new LinkedHashMap<>();
                    ...
                    // 데이터 값이 JSON으로 저장되어 있으므로 JSON 처리
                    // 1단계 생활
                    if(aiSedimentationRealtime.getAIE_9003() != null && processStep == 1) {
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4015");
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4016");
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4017");
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4018");
                    ...
                    if(aiSedimentationRealtime.getAIE_9003() != null && processStep == 2) {
                    	...
                    	// location3 - AI 수집기 대차 시작 시간 예측
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4029");
                    	if(location3Dto.getAIE_6003().getStart().equalsIgnoreCase("") == true)
                    	{
                    		controlMap.put("value", "--");
                    	}
                    	...
                    	// location3 - AI 수집기 대차 종료 시간 예측
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4030");
                    	if(location3Dto.getAIE_6003().getStop().equalsIgnoreCase("") == true)
                    	{
                    		controlMap.put("value", "--");
                    	}
                    	...
                    	// location3 - AI 다음 수집기 시작 시간 예측
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4031");
                    	if(location3Dto.getAIE_6003().getNext_start().equalsIgnoreCase("") == true)
                    	{
                    		controlMap.put("value", "--");
                    	}
                    	...
                    	// location3 - AI 인발 시간 예측 - 석성 공업 인발 없음
                    	// controlMap = new LinkedHashMap<>();
                    ...
                    // 데이터 값이 JSON으로 저장되어 있으므로 JSON 처리
                    // 1단계 생활
                    if(aiSedimentationRealtime.getAIE_9004() != null && processStep == 1) {
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4019");
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4020");
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4021");
                    	...
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4022");
                    ...
                    if(aiSedimentationRealtime.getAIE_9004() != null && processStep == 2) {
                    	...
                    	// location4 - AI 수집기 대차 시작 시간 예측
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4032");
                    	if(location4Dto.getAIE_6004().getStart().equalsIgnoreCase("") == true)
                    	{
                    		controlMap.put("value", "--");
                    	}
                    	...
                    	// location4 - AI 수집기 대차 종료 시간 예측
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4033");
                    	if(location4Dto.getAIE_6004().getStop().equalsIgnoreCase("") == true)
                    	{
                    		controlMap.put("value", "--");
                    	}
                    	...
                    	// location4 - AI 다음 수집기 시작 시간 예측
                    	controlMap = new LinkedHashMap<>();
                    	controlMap.put("tag", "740-351-SWI-4034");
                    	if(location4Dto.getAIE_6004().getNext_start().equalsIgnoreCase("") == true)
                    	{
                    		controlMap.put("value", "--");
                    	}
                    	...
                    	// location4 - AI 인발 시간 예측 - 석성 공업 인발 없음
                    	// controlMap = new LinkedHashMap<>();
...
//                        log.debug("Send AI_RESULT kafka ai_f_ti tag:[{}], value:[{}]", dto.getTag_sn(), strDate);
    ...
    public int sendAiDisinfectionData() {
        ...
        for (int processStep : CommonValue.PROCESS_STEP_ARRAY) {
    		...
    		if(aiDisinfectionRealtime != null) {
    			try {
    				...
    				for(TagManageDTO dto : tagManageList)
    				{
                    	...
                    	else if(dto.getItm().equalsIgnoreCase("ai_g_evap") == true)
    					{
    						// 증발량 예측
    						controlMap = new LinkedHashMap<>();
    						controlMap.put("tag", dto.getTag_sn());
        	...
        	//중차염 (1단계만 해당)
    		if(processStep == 1) {
            	aiDisinfectionRealtime = databaseService.getLatestAiDisinfectionRealtimeValue(processStep, 2);
            	if(aiDisinfectionRealtime != null) {
        			try {
            ...
            for(AiProcessAlarmDTO dto : alarmList) {
            	...
            	if(alarmInfo != null) {
                    ...
                    if(CommonValue.PROCESS_RECEIVING.equals(process)) {             // 착수
                        databaseService.modAiReceivingAlarmKafkaFlag(updateDto, processStep);
                        ...
                        databaseService.modAiSedimentationAlarmKafkaFlag(updateDto, processStep);
                    } else if(CommonValue.PROCESS_FILTER.equals(process)) {         // 여과
                        databaseService.modAiFilterAlarmKafkaFlag(updateDto, processStep);
                        ...
                        databaseService.modAiDisinfectionAlarmKafkaFlag(updateDto, processStep, CommonValue.DISINFECTION_PERI_STEP);
            ...
            if(nOperationMode > CommonValue.OPERATION_MODE_MANUAL) {
                ...
                if(aiDisinfectionControlList.size() > 0) {
                    ...
                    try {
                        for(AiProcessControlDTO dto : aiDisinfectionControlList) {
                            ...
                            if(alarmExceededInfo != null) { //임계치 제어에 해당
                                ...
                                if(nOperationMode == CommonValue.OPERATION_MODE_SEMI_AUTO) {
                                    ...
                                    // 3-1. send control value to kafka ai_popup
                                    AlarmInfoDTO alarmInfo =
                                            alarmInfoList.getAlarmInfoFromAlarmCode(CommonValue.ALARM_CODE_DISINFECTION_AI_PERI_CONTROL, 1);
                                    if(alarmInfo != null) {
                                        // KAFKA topic is called only once.
                                        if(bFirst == true) {
                                            LinkedHashMap<String, Object> popupMap = new LinkedHashMap<>();
                        ...
                        if(nOperationMode == CommonValue.OPERATION_MODE_FULL_AUTO && !alarmExceededFlag) {
                            List<AlmCtrHisDTO> autoAlmCtrHisList = new ArrayList<>(); // 자동 제어시의 히스토리 리스트
                            AlarmInfoDTO alarmInfo = alarmInfoList.getAlarmInfoFromAlarmCode(CommonValue.ALARM_CODE_DISINFECTION_AI_PERI_CONTROL, 1);
                            Date currentDate = new Date();
                            if(alarmInfo != null) {
	                            // insert alarm_notify & get almSeq
	                            int almSeq = alarmService.alarmNotify(
                    ...
                    try {
                        for(AiProcessControlDTO dto : aiFilterControlList) {
                            if(nOperationMode == CommonValue.OPERATION_MODE_SEMI_AUTO) {
                                ...
                                // 3-1. send control value to kafka ai_popup
                                AlarmInfoDTO alarmInfo =
                                        alarmInfoList.getAlarmInfoFromAlarmCode(CommonValue.ALARM_CODE_FILTER_AI_CONTROL, 1);
                                if(alarmInfo != null) {
                                    // KAFKA topic is called only once.
                                    if(bFirst == true) {
                                        LinkedHashMap<String, Object> popupMap = new LinkedHashMap<>();
                        ...
                        if(nOperationMode == CommonValue.OPERATION_MODE_FULL_AUTO) {
                            List<AlmCtrHisDTO> autoAlmCtrHisList = new ArrayList<>(); // 자동 제어시의 히스토리 리스트
                            AlarmInfoDTO alarmInfo = alarmInfoList.getAlarmInfoFromAlarmCode(CommonValue.ALARM_CODE_FILTER_AI_CONTROL, 1);
                            Date currentDate = new Date();
                            if(alarmInfo != null) {
	                            // insert alarm_notify & get almSeq
	                            int almSeq = alarmService.alarmNotify(
    ...
    public void updateAiOprRt(@RequestHeader("X-ACCESS-TOKEN") String token) {
        ...
        if(currentDate.getTime() - aiOprRtDate.getTime() > CommonValue.ONE_MINUTE)
        {
            ...
            // 전 공정 운전모드 조회 (소독 제외)
            String[] processArray = {"B", "C", "D", "E", "F"}; //공정 리스트 (소독 제외)
//            String[] processStepArray;
            ...
            for(int processStep = 1; processStep <= CommonValue.PROCESS_STEP_ARRAY.length; processStep++) {
            	for(int i = 0; i < processArray.length; i++) {
            		AiOprRealtimeDTO oprRt = new AiOprRealtimeDTO();
             		oprRt.setProc_cd(processArray[i] + (processStep == 1 ? "_LIV" : "_IND"));
            		...
            		case CommonValue.PROCESS_RECEIVING:
            			init = databaseService.getAiReceivingInit(processArray[i].toLowerCase()+"_operation_mode", processStep);
            			break;
            ...
            for(int processStep = 1; processStep <= CommonValue.PROCESS_STEP_ARRAY.length; processStep++) {
	            for(int i = 0; i < disinfectionStepArray.length; i++) {
	            	if(processStep == 2 && disinfectionStepArray[i] != 1) {
	            		break;
	            	}
	            	AiOprRealtimeDTO oprRt = new AiOprRealtimeDTO();
	            	oprRt.setProc_cd("G" + (processStep == 1 ? "_LIV" : "_IND"));
	            	AiProcessInitDTO init = new AiProcessInitDTO();
