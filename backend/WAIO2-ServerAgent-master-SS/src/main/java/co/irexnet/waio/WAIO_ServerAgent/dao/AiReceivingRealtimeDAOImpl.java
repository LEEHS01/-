...
public class AiReceivingRealtimeDAOImpl implements IAiReceivingRealtimeDAO
{
    ...
    public AiReceivingRealtimeDTO select(int processStep)
    {
        ...
        + ", SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AI_B_Hz'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AI_B_Hz'))-2) AS ai_b_hz "             /* 인버터 Hz */
        ...
        + " FROM "
        + getTableByProcessStep(processStep) + " ORDER BY upd_ti DESC LIMIT 1"; // FIXME DESC수정
		try
        {
