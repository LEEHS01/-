...
public class ReceivingRealtimeDAOImpl implements IProcessRealtimeDAO
{
    ...
    public void addPartition(String partitionName, String end_time)
    {
        for(int processStep : CommonValue.PROCESS_STEP_ARRAY) {
            String strQuery = String.format("ALTER TABLE " + getTableByProcessStep(processStep) + " REORGANIZE PARTITION p_max INTO (" +
    ...
    {
        for(int processStep : CommonValue.PROCESS_STEP_ARRAY) {
            String strQuery = String.format("ALTER TABLE " + getTableByProcessStep(processStep) + " DROP PARTITION p_%s", partitionName);
            jdbcTemplate.execute(strQuery);
        }
    }
