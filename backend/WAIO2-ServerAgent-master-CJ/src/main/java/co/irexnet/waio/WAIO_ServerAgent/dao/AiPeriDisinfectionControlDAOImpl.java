...
public class AiPeriDisinfectionControlDAOImpl implements IAiProcessControlDAO
{
    ...
    {
        String strQuery = "SELECT * FROM TB_AI_PERI_G2_CTR WHERE upd_ti=? and rnti=? and tag_sn=? and kfk_flg=?";
    ...
    public List<AiProcessControlDTO> select(Date rnti, int kfk_flg, int processStep)
    {
        String strQuery = "SELECT * FROM " +
                "(SELECT * FROM TB_AI_PERI_G2_CTR WHERE rnti>=? and kfk_flg=? ORDER BY rnti DESC LIMIT 100) " +
                "TB_AI_PERI_G2_CTR GROUP BY tag_sn ORDER BY rnti";
    ...
    {
        String strQuery = "UPDATE TB_AI_PERI_G2_CTR SET kfk_flg=? WHERE upd_ti=? and rnti=? and tag_sn=?";
    ...
    {
        String strQuery = "DELETE FROM TB_AI_PERI_G2_CTR WHERE upd_ti < ?";
