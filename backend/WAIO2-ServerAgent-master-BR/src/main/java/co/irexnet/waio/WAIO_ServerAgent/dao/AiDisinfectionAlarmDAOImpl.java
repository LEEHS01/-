...
public class AiDisinfectionAlarmDAOImpl implements IAiDisinfectionProcessAlarmDAO
{
    ...
    @Override
    public List<AiProcessAlarmDTO> select(Date alm_ti, int kfk_flg, int disinfectionStep)
    {
        String strQuery = "SELECT * FROM " + getTableByProcessStep(disinfectionStep) + " WHERE alm_ti>=? AND kfk_flg=? ORDER BY alm_ti ASC";
    ...
    @Override
    public int updateKafkaFlag(int alm_id, Date alm_ti, int kfk_flg, int disinfectionStep)
    {
        String strQuery = "UPDATE " + getTableByProcessStep(disinfectionStep) + " SET kfk_flg=? WHERE alm_id=? AND alm_ti=?";
    ...
    public String getTableByProcessStep(int disinfectionStep) {
        String tableNm = "";
        if(disinfectionStep == 1) {
            tableNm = "TB_AI_PRE_G_ALM";
        } else if(disinfectionStep == 2) {
            tableNm = "TB_AI_PERI_G_ALM";
