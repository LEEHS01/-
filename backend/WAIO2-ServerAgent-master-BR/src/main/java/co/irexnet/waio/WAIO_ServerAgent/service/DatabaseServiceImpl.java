...
//    }
    ...
    {
        return aiReceivingAlarm.updateKafkaFlag(dto.getAlm_id(), dto.getAlm_ti(), dto.getKfk_flg());
    }
    ...
    {
        return aiCoagulantAlarm.updateKafkaFlag(dto.getAlm_id(), dto.getAlm_ti(), dto.getKfk_flg());
    }
    ...
    {
        return aiMixingAlarm.updateKafkaFlag(dto.getAlm_id(), dto.getAlm_ti(), dto.getKfk_flg());
    }
    ...
    {
        return aiSedimentationAlarm.updateKafkaFlag(dto.getAlm_id(), dto.getAlm_ti(), dto.getKfk_flg());
    }
    ...
    {
        return aiFilterAlarm.updateKafkaFlag(dto.getAlm_id(), dto.getAlm_ti(), dto.getKfk_flg());
    }
    ...
    {
        return aiGacAlarm.updateKafkaFlag(dto.getAlm_id(), dto.getAlm_ti(), dto.getKfk_flg());
    }
    ...
    @Override
    public List<AiProcessAlarmDTO> getAllAiDisinfectionAlarm(Date alarm_time, int kafka_flag, int disinfectionStep)
    {
        return aiDisinfectionAlarm.select(alarm_time, kafka_flag, disinfectionStep);
    }

    @Override
    public int modAiDisinfectionAlarmKafkaFlag(AiProcessAlarmDTO dto, int disinfectionStep)
    {
        return aiDisinfectionAlarm.updateKafkaFlag(dto.getAlm_id(), dto.getAlm_ti(), dto.getKfk_flg(), disinfectionStep);
    }
    ...
    {
        return aiOzoneAlarm.updateKafkaFlag(dto.getAlm_id(), dto.getAlm_ti(), dto.getKfk_flg());
    }
...
//                }
            	...
            	/**
            	 * 지별 응집기 상태, 응집기 속도 (실시간 데이터)
            	 * [ITM 코드생성 규칙]
            	 * 
            	 * d_fc_on1_1 : A지 1 step 상태(인버터 RUN)  
            	 * d_fc_on3_2 : C지 2 step 상태(인버터 RUN)
            	 * 
            	 * d_fc_sp2_2 : B지 2 step 속도(RPM)
            	 * d_fc_sp5_3 : E지 3 step 속도(RPM)
            	 */
                ...
                //지별 응집기 속도 (IN_VAL에서 데이터 보내기)
                /*mapTemp = objectMapper.readValue(aiMixingRealtimeDto.getD_loc_fc_sp(), LinkedHashMap.class);
                List<String> keyList = new ArrayList<>(mapTemp.keySet());
                ...
                for(String key : keyList) {
                	LinkedHashMap<String, Object> locationMapTemp = objectMapper.convertValue(mapTemp.get(key), LinkedHashMap.class);
                	aiMixingInfo.put(key.replaceAll("location", "d_loc_fc_sp"), locationMapTemp);
                }*/
    ...
    @Override
    public int modAiOprRealTime(String process, String disinfectionStep, int value) {
        return aiOprRealTime.update(process, disinfectionStep, value);
    }
