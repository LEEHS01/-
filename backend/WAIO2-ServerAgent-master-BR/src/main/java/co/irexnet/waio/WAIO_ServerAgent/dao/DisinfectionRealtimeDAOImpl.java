...
public class DisinfectionRealtimeDAOImpl implements IProcessRealtimeDAO
{
    ...
    {
        String strQuery = String.format("SELECT * FROM TB_G_RT PARTITION(p_%s) WHERE upd_ti IN " +
                "(SELECT MAX(upd_ti) FROM TB_G_RT PARTITION(p_%s) GROUP BY tag_sn)", partitiontag_sn, partitiontag_sn);
        ...
        {
            strQuery = String.format("SELECT * FROM TB_G_RT PARTITION(p_max) WHERE upd_ti IN " +
                    "(SELECT MAX(upd_ti) FROM TB_G_RT PARTITION(p_max) GROUP BY tag_sn)");
    ...
    {
        String strQuery = "SELECT * FROM TB_G_RT WHERE upd_ti > ? ORDER BY upd_ti DESC";
    ...
    {
        String strQuery = "SELECT * FROM TB_G_RT WHERE tag_sn=? AND upd_ti > ? AND upd_ti < ?";
        return jdbcTemplate.query(
                strQuery,
    ...
    {
        String strQuery =  "SELECT * FROM TB_G_RT WHERE tag_sn=? ORDER BY upd_ti DESC limit 1";
        try
        {
