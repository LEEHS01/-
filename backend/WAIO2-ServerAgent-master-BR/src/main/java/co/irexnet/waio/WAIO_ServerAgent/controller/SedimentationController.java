...
//        strPartitionName = "20231201"; // FIXME 현재 날짜 수정
        ...
        // get ai_sedimentation_realtime (RT_AI_E_INIT 조회)
        ...
        // get ai_sedimentation_init(e_operation_mode) (RT_AI_E_INIT 조회)
        ...
        // get ai_sedimentation_init (RT_AI_E_INIT 조회)
        ...
        if(aiSedimentationRealtime != null)
        {
            ...
            Float d1_mm_fr = 0.0f; //약품공정 1호기 약품 사용량
            Float d2_mm_fr = 0.0f; //약품공정 2호기 약품 사용량
            Float d3_mm_fr = 0.0f; //약품공정 3호기 약품 사용량
            
            Float c1_cf = 0.0f; //약품공정 1호기 약품 주입률
            Float c2_cf = 0.0f; //약품공정 2호기 약품 주입률
            Float c3_cf = 0.0f; //약품공정 3호기 약품 주입률
            
            Float c_mm_fr_etc1 = 0.0f;	//활성탄 주입량
            Float c_mm_fr_etc2 = 0.0f;	//소석회 주입량
            Float c_mm_fr_etc3_1 = 0.0f;	//폴리아민 1호기 주입량
            Float c_mm_fr_etc3_2 = 0.0f;	//폴리아민 2호기 주입량
            
            Float b_in_if = 0.0f;
            
            // 약품 사용량 계산을 위한 원수 유입 유량 선 조회
            for(TagManageDTO tagManage : tagManageList) {
            	for(ProcessRealtimeDTO dto : sedimentationRealtime) {
            		if(tagManage.getItm().equalsIgnoreCase("b_in_fr") == true && tagManage.getTag_sn().equalsIgnoreCase(dto.getTag_sn()) == true) {
                        ...
                        b_in_if = Float.parseFloat(dto.getTag_val());
                    }
            	}
            }
            ...
            for(TagManageDTO tagManage : tagManageList)
            {
                for(ProcessRealtimeDTO dto : sedimentationRealtime)
                {
                    ...
                    // 2호기 약품 사용량 계산
						...
						d2_mm_fr = (c2_cf * b_in_if / 1000);
					}
                    // 3호기 약품 사용량 계산
                    else if(tagManage.getItm().equalsIgnoreCase("c3_cf") == true &&
                            tagManage.getTag_sn().equalsIgnoreCase(dto.getTag_sn()) == true) {
                    	c3_cf = Float.parseFloat(dto.getTag_val());
                    	d3_mm_fr = (c3_cf * b_in_if / 1000);
                    }
                    // 활성탄 사용량 계산
                    else if(tagManage.getItm().equalsIgnoreCase("c_mm_fr_etc1") == true &&
                          tagManage.getTag_sn().equalsIgnoreCase(dto.getTag_sn()) == true) {
                    	c_mm_fr_etc1 = Float.parseFloat(dto.getTag_val());
                    	c_mm_fr_etc1 = (c_mm_fr_etc1 * b_in_if / 1000);
                    }
                    // 소석회 사용량 계산
                    else if(tagManage.getItm().equalsIgnoreCase("c_mm_fr_etc2") == true &&
                          tagManage.getTag_sn().equalsIgnoreCase(dto.getTag_sn()) == true) {
                    	c_mm_fr_etc2 = Float.parseFloat(dto.getTag_val());
                    	c_mm_fr_etc2 = (c_mm_fr_etc2 * b_in_if / 1000);
                    }
                    // 폴리아민 1호기 사용량
                    else if(tagManage.getItm().equalsIgnoreCase("c_mm_fr_etc3_1") == true &&
                          tagManage.getTag_sn().equalsIgnoreCase(dto.getTag_sn()) == true) {
                    	c_mm_fr_etc3_1 = Float.parseFloat(dto.getTag_val());
                    }
                    // 폴리아민 2호기 사용량
                    else if(tagManage.getItm().equalsIgnoreCase("c_mm_fr_etc3_2") == true &&
                          tagManage.getTag_sn().equalsIgnoreCase(dto.getTag_sn()) == true) {
                    	c_mm_fr_etc3_2 = Float.parseFloat(dto.getTag_val());
                    }
                }
            }

            // 약품명
//            aiSedimentationInfo.put("d_mm_coagulant", "APAC-1050");
            
            // 약품 사용량 (1호기, 2호기, 3호기 합산)
            aiSedimentationInfo.put("mm_fr", d1_mm_fr + d2_mm_fr + d3_mm_fr);
            
            // 약품사용량 1호기 주입량, 2호기, 3호기 주입량
            aiSedimentationInfo.put("d1_mm_fr", d1_mm_fr);
            aiSedimentationInfo.put("d2_mm_fr", d2_mm_fr);
            aiSedimentationInfo.put("d3_mm_fr", d3_mm_fr);
            
            // 활성탄 주입량
            aiSedimentationInfo.put("c_mm_fr_etc1", c_mm_fr_etc1);
            // 소석회 주입량
            aiSedimentationInfo.put("c_mm_fr_etc2", c_mm_fr_etc2);
            // 폴리아민 주입량
            aiSedimentationInfo.put("c_mm_fr_etc3", c_mm_fr_etc3_1 + c_mm_fr_etc3_2);
            ...
            try
            {
            	...
            	// 활성탄 주입률
//            	aiSedimentationInfo.put("c_mm_fr_etc1", aiSedimentationRealtime.getC_mm_fr_etc1());
            	
            	// 소석회 주입률
//            	aiSedimentationInfo.put("c_mm_fr_etc2", aiSedimentationRealtime.getC_mm_fr_etc2());
            	
            	// 폴리아민 주입률
//            	aiSedimentationInfo.put("c_mm_fr_etc3", aiSedimentationRealtime.getC_mm_fr_etc3_1() + aiSedimentationRealtime.getC_mm_fr_etc3_2());
            	
                // AI 총 슬러지 발생량 예측
                // 데이터 값이 JSON으로 저장되어 있으므로 JSON 처리
				String strTempAiSludge = aiSedimentationRealtime.getAIE_5200();
				LinkedHashMap<String, Object> mapTempAiSludge = objectMapper.readValue(strTempAiSludge, LinkedHashMap.class);
				
				ArrayList<String> keyListAiSludge = new ArrayList<>(mapTempAiSludge.keySet());
				Object objectTempAiSludge = mapTempAiSludge.get(keyListAiSludge.get(0));
				LinkedHashMap<String, Float> aiSludgeMap = objectMapper.convertValue(objectTempAiSludge, LinkedHashMap.class);
				aiSedimentationInfo.put("ai_e_total_sludge", aiSludgeMap);
            	
            	
                // 데이터 값이 JSON으로 저장되어 있으므로 JSON 처리
                String strTemp = aiSedimentationRealtime.getAIE_9001(); //AIE-9001 : 1지 데이터
                ...
                AiSedimentationLocation1RealtimeDTO location1Dto = objectMapper.convertValue(mapTemp, AiSedimentationLocation1RealtimeDTO.class);
                ...
                // 데이터 값이 JSON으로 저장되어 있으므로 JSON 처리
                strTemp = aiSedimentationRealtime.getAIE_9002(); // AIE-9002 : 2지 데이터
                strTemp = strTemp.replaceAll("NaN", "\"\"");
...
//                }
    ...
    /**
     * [소독 전차염/중차염공정] 혼화지 or 침전지 잔류염소 트렌드 그래프 데이터 조회
     * 전차염 : 혼화지 잔류염소 그래프 데이터 조회
     * 중차염 : 침전지 잔류염소 그래프 데이터 조회
     * 
        ...
        if(aiDisinfectionRealtimeList.size() > 0) {
            ...
            for(AiDisinfectionRealtimeDTO dto : aiDisinfectionRealtimeList) {
                ...
                if(processStep == 1 && disinfectionStep == 1) {
                	series1.put(strDate, dto.getE1_cl()); //전차염일 경우 침전지 잔류염소 set
                }else if(processStep == 1 && disinfectionStep == 2) {
                	series1.put(strDate, dto.getE1_cl()); //중차염일 경우 침전지 잔류염소 set
                }
            }

            LinkedHashMap<String, Object> seriesClInfo = new LinkedHashMap<>();
            if(processStep == 1 && disinfectionStep == 1) {
            	seriesClInfo.put("series1", series1); //혼화지 잔류염소 그래프
            }else if(processStep == 1 && disinfectionStep == 2) {
            	seriesClInfo.put("series1", series1); //침전지 잔류염소 그래프
            }
    ...
    // [약품공정] 침전지 탁도 정규분포 그래프 데이터 조회
    
    /**
     * 침전지 탁도 정규분포 그래프 데이터 조회
     * 
     * @param processStep 공정단계
     * @return ResponseEntity<String> 메시지
     */
    ...
    public ResponseEntity<String> getTbDistributionSedimentation(@PathVariable int processStep)
    {
        ...
        calendar.add(Calendar.DATE, -7); // FIXME (시범사업 기준 7일 전 데이터 조회)
//        calendar.add(Calendar.MONTH, -9);	// FIXME TODO 운영 반영시 본 코드라인 제거
        ...
        for(TagManageDTO tagManage: tagManageList)
        {
            ...
            {
                // 침전지 탁도 정규분포 데이터를 조회하여 series1에 저장
...
//                databaseService.modAiSedimentationOperationMode(nOperationMode), nOperationMode);
        ...
        try {
        	if(aiSedimentationInit != null) {
        		ObjectMapper objectMapper = new ObjectMapper();
        		String strBody = objectMapper.writeValueAsString(tagMapService.getControlMap(aiSedimentationInit.getTag_sn(), nOperationMode, strDate));
