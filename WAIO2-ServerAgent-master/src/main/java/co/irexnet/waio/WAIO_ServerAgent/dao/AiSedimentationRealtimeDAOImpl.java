...
public class AiSedimentationRealtimeDAOImpl implements IAiSedimentationRealtimeDAO
{
    ...
    public AiSedimentationRealtimeDTO select(int processStep)
    {
        ...
        if(processStep == 2) {
        	strQuery += ", CONCAT('{\"AIE-9009\":', SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AIE-9009'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AIE-9009'))-2),'}') AS AIE_9009"
        	+ ", CONCAT('{\"AIE-9010\":', SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AIE-9010'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AIE-9010'))-2),'}') AS AIE_9010";
        }
        strQuery += ", CONCAT('{\"AIE-5200\":', SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AIE-5200'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AIE-5200'))-2),'}') AS AIE_5200" // AI 침전지 총 슬러지 발생량 예측 JSON 
