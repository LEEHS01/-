...
//                databaseService.modAiMixingOperationMode(nOperationMode), nOperationMode);
        ...
        try {
        	if(aiMixingInit != null) {
        		...
        		ObjectMapper objectMapper = new ObjectMapper();
        		String strBody = objectMapper.writeValueAsString(tagMapService.getControlMap(aiMixingInit.getTag_sn(), nOperationMode, strDate));
    ...
    public ResponseEntity<String> putAiControlMixing(@RequestBody InterfaceMixingAiDTO mixingAi, @PathVariable int processStep)
    {
        ...
        if(d_g_value_ctr_flag == 0)  {// 수동인 경우 G값 변경
	        ...
	        result = (databaseService.modAiMixingInit("d_g_value_step1", mixingAi.getD_g_value_step1(), processStep) == 1) && result;
	
	        // update 응집기 2단 교반강도 G 값
	        result = (databaseService.modAiMixingInit("d_g_value_step2", mixingAi.getD_g_value_step2(), processStep) == 1) && result;
	
	        // update 응집기 3단 교반강도 G 값
	        result = (databaseService.modAiMixingInit("d_g_value_step3", mixingAi.getD_g_value_step3(), processStep) == 1) && result;
        }
     ...
     * 혼화응집 교반강도 한계값 변경
     * 
     * @param mixingAi Front-end 혼화응집 교반강도 한계값을 저장하기 위한 DTO
     * @param processStep 공정단계
     * @return ResponseEntity<String> 메시지
     */
    @RequestMapping(value = "/mixing/glimit/control/ai/{processStep}", method = RequestMethod.PUT)
    public ResponseEntity<String> putAiGLimitMixing(@RequestBody InterfaceMixingAiDTO mixingAi, @PathVariable int processStep)
    {
        log.debug("putAiGLimitMixing, ai:[{}]", mixingAi);

        boolean result = true;

        // update 교반강도 1열 상한값
    	result = (databaseService.modAiMixingInit("d_g_step1_max", mixingAi.getD_g_step1_max(), processStep) == 1) && result;
    	// update 교반강도 1열 하한값
    	result = (databaseService.modAiMixingInit("d_g_step1_min", mixingAi.getD_g_step1_min(), processStep) == 1) && result;
    	
    	// update 교반강도 2열 상한값
    	result = (databaseService.modAiMixingInit("d_g_step2_max", mixingAi.getD_g_step2_max(), processStep) == 1) && result;
    	// update 교반강도 2열 하한값
    	result = (databaseService.modAiMixingInit("d_g_step2_min", mixingAi.getD_g_step2_min(), processStep) == 1) && result;
    	
		// update 교반강도 3열 상한값
    	result = (databaseService.modAiMixingInit("d_g_step3_max", mixingAi.getD_g_step3_max(), processStep) == 1) && result;
    	// update 교반강도 3열 하한값
    	result = (databaseService.modAiMixingInit("d_g_step3_min", mixingAi.getD_g_step3_min(), processStep) == 1) && result;
     ...
     * 
     * @param mixingAi Front-end 혼화응집 교반강도 보정계수 설정값을 저장하기 위한 DTO
     * @param processStep 공정단계
     * @return ResponseEntity<String> 메시지
     */
    @RequestMapping(value = "/mixing/gcrt/control/ai/{processStep}", method = RequestMethod.PUT)
    public ResponseEntity<String> putAiGCrtMixing(@RequestBody InterfaceMixingAiDTO mixingAi, @PathVariable int processStep)
    {
        log.debug("putAiGCrtMixing, ai:[{}]", mixingAi);

        boolean result = true;

        // update 교반강도 1열 보정계수
    	result = (databaseService.modAiMixingInit("d_g_step1_crt", mixingAi.getD_g_step1_crt(), processStep) == 1) && result;
    	// update 교반강도 2열 보정계수
    	result = (databaseService.modAiMixingInit("d_g_step2_crt", mixingAi.getD_g_step2_crt(), processStep) == 1) && result;
		// update 교반강도 3열 보정계수
		result = (databaseService.modAiMixingInit("d_g_step3_crt", mixingAi.getD_g_step3_crt(), processStep) == 1) && result;
