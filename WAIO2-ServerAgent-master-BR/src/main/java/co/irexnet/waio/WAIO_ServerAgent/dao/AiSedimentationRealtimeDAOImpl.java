...
public class AiSedimentationRealtimeDAOImpl implements IAiSedimentationRealtimeDAO
{
    ...
    public AiSedimentationRealtimeDTO select(int processStep)
    {
    	...
    	//+ ", SUBSTR(JSON_EXTRACT(IN_VAL, '$[*].b_in_fr'), 2, LENGTH(JSON_EXTRACT(IN_VAL, '$[*].b_in_fr1'))-2) AS b_in_fr"
    	+ ", (CAST(SUBSTR(JSON_EXTRACT(IN_VAL, '$[*].b_in_fr1'), 2, LENGTH(JSON_EXTRACT(IN_VAL, '$[*].b_in_fr1'))-2) as FLOAT) + "
    	+ "   CAST(SUBSTR(JSON_EXTRACT(IN_VAL, '$[*].b_in_fr2'), 2, LENGTH(JSON_EXTRACT(IN_VAL, '$[*].b_in_fr2'))-2) as FLOAT)) AS b_in_fr"
        ...
        + ", SUBSTR(JSON_EXTRACT(IN_VAL, '$[*].c_mm_fr_etc3_1'), 2, LENGTH(JSON_EXTRACT(IN_VAL, '$[*].c_mm_fr_etc3_1'))-2) AS c_mm_fr_etc3_1"
        + ", SUBSTR(JSON_EXTRACT(IN_VAL, '$[*].c_mm_fr_etc3_2'), 2, LENGTH(JSON_EXTRACT(IN_VAL, '$[*].c_mm_fr_etc3_2'))-2) AS c_mm_fr_etc3_2"
        + ", CONCAT('{\"AIE_5200\":', SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AIE-5200'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AIE-5200'))-2),'}') AS AIE_5200"
        ...
        + ", CONCAT('{\"AIE-9008\":', SUBSTR(JSON_EXTRACT(OUT_VAL, '$[*].AIE-9008'), 2, LENGTH(JSON_EXTRACT(OUT_VAL, '$[*].AIE-9008'))-2),'}') AS AIE_9008 FROM " 
