...
public class CoagulantRealtimeDAOImpl implements IProcessRealtimeDAO
{
    ...
    {
        String strQuery = "INSERT IGNORE INTO TB_C_RT VALUES(?, ?, ?, ?);";
    ...
    public String getTableByProcessStep(int processStep) {
        String tableNm = "";
        if(processStep == 1) {
            tableNm = "TB_C_RT"; //보령(정) 약품실시간 TBL
        }
        return tableNm;
    }
    ...
    {
    	String strQuery = String.format("ALTER TABLE TB_C_RT DROP PARTITION p_%s", partitionName);
        jdbcTemplate.execute(strQuery);
    }
}
