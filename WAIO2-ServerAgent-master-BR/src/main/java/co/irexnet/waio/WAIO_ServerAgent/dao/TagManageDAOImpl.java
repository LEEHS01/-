...
public class TagManageDAOImpl implements ITagManageDAO
{
    ...
    @Override
    public List<TagManageDTO> select(int tag_ty, String process)
    {
        String strQuery = "select * from TB_TAG_MNG where tag_ty=? AND ai_cd=?";
        return jdbcTemplate.query(strQuery, new Object[]{tag_ty, process}, new BeanPropertyRowMapper<>(TagManageDTO.class));
    }
