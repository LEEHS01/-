...
export const PUT_SEDIMENTATION_CONTROL_LOCATION = 'indSedimentation/control/location/put'
...
import { CLOSE_IND_AI_MODE_OF_JI_DIALOG } from '@/store/modules/dialog'
...
export default {
  ...
  getters: {
    ...
    isOpenInBalValve9: function (state) {
      if (state.latest.e_drawing_vv9_1 === 1 || state.latest.e_drawing_vv9_2 === 1 || state.latest.e_drawing_vv9_3 === 1 || state.latest.e_drawing_vv9_4 === 1 || state.latest.e_drawing_vv9_5 === 1 || state.latest.e_drawing_vv9_6 === 1) {
        return true
      } else {
        return false
      }
    ...
    isOpenInBalValve10: function (state) {
      if (state.latest.e_drawing_vv10_1 === 1 || state.latest.e_drawing_vv10_2 === 1 || state.latest.e_drawing_vv10_3 === 1 || state.latest.e_drawing_vv10_4 === 1 || state.latest.e_drawing_vv10_5 === 1 || state.latest.e_drawing_vv10_6 === 1) {
        return true
      } else {
        return false
      }
    },
    isOperateJi9: function (state) {
      if (state.latest.e_operation_mode_9 === 1) {
        return true
      } else {
        return false
      }
    },
    isOperateJi10: function (state) {
      if (state.latest.e_operation_mode_10 === 1) {
        return true
      } else {
        return false
      }
  ...
  actions: {
    ...
    [PUT_INTERFACE]: async function ({ commit }, obj) {
      ...
      await axios.put(`${DEV_SERVER}/${URL.SEDIMENTATION_HISTORY_INTERFACE}/${obj.numJi}`, { 'start_time': new Date(nowTimestamp - oneDayTimestamp).toISOString(), 'end_time': new Date(nowTimestamp).toISOString(), 'processStep' : PROCESS_STEP })
    ...
    [PUT_CONTROL_LOCATION]: async function ({ commit }, { numJi, ai }) {
      ...
      .then(() => {
        ...
        commit('dialog/'+ CLOSE_IND_AI_MODE_OF_JI_DIALOG, null, { root: true })
        let _data = {
          visible: true,
          title: '설정 성공',
          text1: '설정값이 변경되었습니다'
