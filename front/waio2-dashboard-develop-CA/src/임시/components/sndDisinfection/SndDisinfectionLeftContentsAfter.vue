<template>
  <div class="main">
    ...
    <div class="btn-tab">
      ...
      <div class="btn-tab__box-disabled" @click="$routingByIndex(7.7)">3단계</div>
    </div>
    ...
    <div class="contents-wrap">
      <div class="contents-value">
        <div class="contents-value__row">
          ...
          <div class="contents-value__number"><input v-if="this.$store.state.sndDisinfection.isModifyMode" type="text" :value="this.$store.state.sndDisinfection.latestModify.g_post_set_max" v-on:input="updateInput($event, 'g_post_set_max')"/><span v-else>{{ this.$store.state.sndDisinfection.latestModify.g_post_set_max | numFormat('0.00') }}</span></div>
        </div>
        <div class="contents-value__row">
          <div class="contents-value__title">주입률하한</div>
          <div class="contents-value__number"><input v-if="this.$store.state.sndDisinfection.isModifyMode" type="text" :value="this.$store.state.sndDisinfection.latestModify.g_post_set_min" v-on:input="updateInput($event, 'g_post_set_min')"/><span v-else>{{ this.$store.state.sndDisinfection.latestModify.g_post_set_min | numFormat('0.00') }}</span></div>
        </div>
        <div class="contents-value__row">
          <div class="contents-value__title">1회 변경 주입률</div>
          <div class="contents-value__number"><input v-if="this.$store.state.sndDisinfection.isModifyMode" type="text" :value="this.$store.state.sndDisinfection.latestModify.g_post_chg_limit_for_onetime" v-on:input="updateInput($event, 'g_post_chg_limit_for_onetime')"/><span v-else>{{ this.$store.state.sndDisinfection.latestModify.g_post_chg_limit_for_onetime | numFormat('0.00') }}</span></div>
        </div>
        <div class="contents-value__row">
          <div class="contents-value__title">정수지유입<br/>목표 잔류염소</div>
          <div class="contents-value__number"><input v-if="this.$store.state.sndDisinfection.isModifyMode" type="text" :value="this.$store.state.sndDisinfection.latestModify.g_h_in_obj_residual_cl" v-on:input="updateInput($event, 'g_h_in_obj_residual_cl')"/><span v-else>{{ this.$store.state.sndDisinfection.latestModify.g_h_in_obj_residual_cl | numFormat('0.00') }}</span></div>
        </div>
        <div class="contents-value__row">
          <div class="contents-value__title">정수지유입<br/>잔류염소 홀딩범위</div>
          <div class="contents-value__number"><input v-if="this.$store.state.sndDisinfection.isModifyMode" type="text" :value="this.$store.state.sndDisinfection.latestModify.g_h_in_residual_cl_holding" v-on:input="updateInput($event, 'g_h_in_residual_cl_holding')" maxlength="5"/><span v-else>{{ this.$store.state.sndDisinfection.latestModify.g_h_in_residual_cl_holding | numFormat('0.00') }}</span></div>
        </div>
        <div class="contents-value__row">
          <div class="contents-value__title">보정주기</div>
          <div class="contents-value__number"><input v-if="this.$store.state.sndDisinfection.isModifyMode" type="text" :value="this.$store.state.sndDisinfection.latestModify.g_post_calib_cycle" v-on:input="updateInput($event, 'g_post_calib_cycle')"/><span v-else>{{ this.$store.state.sndDisinfection.latestModify.g_post_calib_cycle | numFormat('0') }}</span></div>
        </div>
        <div class="contents-value__row">
          <div class="contents-value__title">보정상수</div>
          <div class="contents-value__number"><input v-if="this.$store.state.sndDisinfection.isModifyMode" type="text" :value="this.$store.state.sndDisinfection.latestModify.g_post_calib_num" v-on:input="updateInput($event, 'g_post_calib_num')" maxlength="5"/><span v-else>{{ this.$store.state.sndDisinfection.latestModify.g_post_calib_num | numFormat('0.0') }}</span></div>
        </div>
        <div class="contents-value__row">
...
<script>
import { PUT_DISINFECTION_CONTROL_POST } from '@/store/modules/sndDisinfection'
export default {
  data: () => ({
    // isModifyMode: false
  }),
  ...
  methods: {
    ...
    updateControl: function() {
      ...
      if (this.$store.state.sndDisinfection.isModifyMode) {
        if (this.$store.state.sndDisinfection.latestModify.g_post_set_max === ''
           || this.$store.state.sndDisinfection.latestModify.g_post_set_min === ''
           || this.$store.state.sndDisinfection.latestModify.g_post_chg_limit_for_onetime === ''
           || this.$store.state.sndDisinfection.latestModify.g_post_calib_cycle === ''
           || this.$store.state.sndDisinfection.latestModify.g_h_in_obj_residual_cl === ''
           || this.$store.state.sndDisinfection.latestModify.g_post_calib_num === ''
           || this.$store.state.sndDisinfection.latestModify.g_h_in_residual_cl_holding === '') {
          this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '값을 입력해주세요' })
        } else {
          let obj = {}
          obj.g_post_set_max = parseFloat(this.$store.state.sndDisinfection.latestModify.g_post_set_max)
          obj.g_post_set_min = parseFloat(this.$store.state.sndDisinfection.latestModify.g_post_set_min)
          obj.g_post_chg_limit_for_onetime = parseFloat(this.$store.state.sndDisinfection.latestModify.g_post_chg_limit_for_onetime)
          obj.g_post_calib_cycle = parseFloat(this.$store.state.sndDisinfection.latestModify.g_post_calib_cycle)
          obj.g_h_in_obj_residual_cl = parseFloat(this.$store.state.sndDisinfection.latestModify.g_h_in_obj_residual_cl)
          obj.g_post_calib_num = parseFloat(this.$store.state.sndDisinfection.latestModify.g_post_calib_num)
          obj.g_h_in_residual_cl_holding = parseFloat(this.$store.state.sndDisinfection.latestModify.g_h_in_residual_cl_holding)
