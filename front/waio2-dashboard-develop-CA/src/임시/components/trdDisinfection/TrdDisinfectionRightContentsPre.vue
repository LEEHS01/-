  ...
  <div class="main">
    <div class="top-contents">
      ...
      <div class="one-contents">
        <div class="one-contents__title">주요인자 3단계 전차염</div>
        <div class="big-contents">
          <div class="contents-value">
            <div class="value-container">
              <div class="value-container__one">· 기온(°C)</div>
              <div class="value-container__two">{{ this.$store.state.trdDisinfection.latest.g_tei | numFormat('0.00') }}</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 수온(°C)</div>
              <div class="value-container__two">{{ this.$store.state.trdDisinfection.latest.b_te | numFormat('0.00') }}</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 정수 탁도(NTU)</div>
              <div class="value-container__two">{{ this.$store.state.trdDisinfection.latest.h_tb | numFormat('0.00') }}</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 정수 pH</div>
              <div class="value-container__two">{{ this.$store.state.trdDisinfection.latest.h_ph | numFormat('0.00') }}</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 원수유입유량(m<sup>3</sup>/h)</div>
              <div class="value-container__two">{{ this.$store.state.trdDisinfection.latest.b_in_fr | numFormat('0,0') }}</div>
              <!-- <div class="value-container__two">{{ this.$store.state.trdDisinfection.latest.d_ser_in_fr | numFormat('0,0') }}</div> -->
            </div>
            <div class="value-container">
              <div class="value-container__one">· 혼화지 잔류 염소(ppm)</div>
              <div class="value-container__two">{{ this.$store.state.trdDisinfection.latest.g_d_residual_cl | numFormat('0.00') }}</div>
              <!-- <div class="value-container__two">{{ this.$store.state.trdDisinfection.latest.e_ser_cl | numFormat('0.0') }}</div> -->
            </div>
            <div class="value-container">
              <div class="value-container__one">· 침전지 잔류 염소(ppm)</div>
              <div class="value-container__two">{{ this.$store.state.trdDisinfection.latest.g_e_residual_cl | numFormat('0.00') }}</div>
              <!-- <div class="value-container__two">{{ this.$store.state.trdDisinfection.latest.d_ser_cl | numFormat('0.0') }}</div> -->
            </div>
            <div class="value-container">
              <div class="value-container__one">· 현재 주입률(ppm)</div>
              <div class="value-container__two">{{ this.$store.state.trdDisinfection.latest.g_pre_chol_rate | numFormat('0.00') }}</div>
              <!-- <div class="value-container__two">{{ this.$store.state.trdDisinfection.latest.g_pre2_chlorination | numFormat('0.0') }}</div> -->
            </div>
          </div>
        </div>
      </div>
      <div class="two-contents">
        ...
        <div class="small-contents">
          <div class="little-title-one">차염 증발량 예측</div>
          ...
          <div class="small-contents-value">
            ...
            <div class="value">
              <div class="value__num">{{ this.$store.state.trdDisinfection.latest.ai_g_evap | numFormat('0.00') }}</div>
      ...
      <div class="three-contents">
        <div class="three-contents__title">
          ...
          <div class="three-contents__timerbox-outter">
            <div class="timerbox">
              {{ this.$store.state.trdDisinfection.latest.upd_ti | moment('YYYY-MM-DD HH:mm:ss') }}
            </div>
          </div>
        </div>
        <div class="three-small-contents">
          ...
          <div class="real-box real-two">
            ...
            <div class="real-value">{{ this.$store.state.trdDisinfection.latest.g_pre_chol_rate | numFormat('0.00') }}</div>
          </div> 
          <div class="three-small-contents-value">
            <div class="three-small-contents-value__title">{{ getPumpRun }}</div>
            <div class="three-value">
              <div class="three-value__num">{{ this.$store.state.trdDisinfection.latest.ai_g_chol_rate | numFormat('0.00') }}</div>
    ...
    <div class="bottom-contents">
      <div class="contents">
        <div class="contents__title">차염 증발량 예측</div>
        <div class="contents-gragh">
          <highcharts :options="ChartOptionEvaporation" style="height:100%;"/>
        </div>
      </div>
      <div class="contents paddingleft">
        <div class="contents__title">침전지 잔류염소</div>
        <div class="real-box real-three">
          <div class="real-text">현재 잔류염소</div>
          <div class="real-value">{{ this.$store.state.trdDisinfection.latest.g_e_residual_cl | numFormat('0.00') }}</div>
        </div>        
        <div class="contents-gragh">
...
export default {
  ...
  computed: {
    getPumpRun: function() {
      let pumpRun = ''
      if(this.$store.state.trdDisinfection.latest.g_pump_1_run == 1) pumpRun += '1호기'
      if(this.$store.state.trdDisinfection.latest.g_pump_2_run == 1) pumpRun += '2호기'
      return pumpRun
    },
    ChartOptionEvaporation: function () {
      ...
      chart.series = []
      if (this.$store.state.trdDisinfection.pre_evaporation['series' + this.selectedLine] !== null && Object.keys(this.$store.state.trdDisinfection.pre_evaporation['series' + this.selectedLine]).length > 0) {
        let yesterdayData = []
        let todayData = []
        ...
        for (let key in this.$store.state.trdDisinfection.pre_evaporation['series' + this.selectedLine]) {
          let _timestamp = new Date(key).getTime()
          if (_timestamp < todayTimestamp) {
            yesterdayData.push([_timestamp + this.$getMilliSecondFromHour(24), this.$store.state.trdDisinfection.pre_evaporation['series' + this.selectedLine][key]])
          } else {
            todayData.push([_timestamp, this.$store.state.trdDisinfection.pre_evaporation['series' + this.selectedLine][key]])
          }
        }
        chart.series.push({
          name: '어제',
      ...
      chart.exporting.filename = this.$moment().format('YYYYMMDDHHmmss') + '_전차염 증발량 예측 비교'
    ...
    ChartOptionCl: function () {
      ...
      chart.series = []
      if (this.$store.state.trtIndSedimentation.clTrend.series1 !== null && Object.keys(this.$store.state.trtIndSedimentation.clTrend.series1).length > 0) {
        let dataSeries1 = []
        for (let key in this.$store.state.trtIndSedimentation.clTrend.series1) {
          dataSeries1.push([new Date(key).getTime(), this.$store.state.trtIndSedimentation.clTrend.series1[key]])
        }
        chart.series.push({
          name: '잔류염소',
          data: dataSeries1,
      ...
      chart.exporting.filename = this.$moment().format('YYYYMMDDHHmmss') + '_침전수 잔류염소'
      return chart
    }
  },
  ...
  methods: {
    downloadExcel: function() {
      const eva = this.$store.state.trdDisinfection.pre_evaporation
