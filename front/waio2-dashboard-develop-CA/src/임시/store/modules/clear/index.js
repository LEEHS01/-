...
export default {
  ...
  actions: {
    ...
    [PUT_HISTORY_LEVEL]: async function ({ commit }, { processStep }) {
      ...
      // await axios.put(`${DEV_SERVER}/${URL.CLEAR_HISTORY_LEVEL}/` + processStep, { 'start_time': new Date(2022, 11, 23, 0, 0, 0).toISOString(), 'end_time': new Date(2022, 11, 24, 23, 59, 59).toISOString() })
          .then(({ data }) => {
    ...
    [PUT_HISTORY_CL]: async function ({ commit }, {disinfectionStep}) {
      ...
      let oneDayTimestamp = 1000 * 60 * 60 * 24
      // FIXME 현재 날짜 수정
      await axios.put(`${DEV_SERVER}/${URL.CLEAR_HISTORY_CL}/` + disinfectionStep, { 'start_time': new Date(nowTimestamp - oneDayTimestamp).toISOString(), 'end_time': new Date(nowTimestamp).toISOString() })
      // await axios.put(`${DEV_SERVER}/${URL.CLEAR_HISTORY_CL}/` + disinfectionStep, { 'start_time': new Date('2023-8-21 00:00:00.000').toISOString(), 'end_time': new Date('2023-8-21 23:59:10.000').toISOString() })
          .then(({ data }) => {
