...
export default {
  ...
  mutations: {
    ...
    [PUT_CONTROL_COAGULANT]: function (state, c2_cf_coagulant){
      state.latest.c2_cf_coagulant = c2_cf_coagulant
    }
  },
  actions: {
    ...
    [PUT_CONTROL_COAGULANT]: async function ({ commit }, { c2_cf_coagulant }) {
      await axios.put(`${DEV_SERVER}/${URL.COAGULANT_CONTROL_COAGULANT}/` + PROCESS_STEP, { c2_cf_coagulant })
      .then((response) => {
        let _data = {
          visible: true,
          title: '설정 성공',
        ...
        commit(PUT_CONTROL_COAGULANT, c2_cf_coagulant)
        commit('alertDialog/OPEN_DIALOG', _data, { root: true })
        console.log('date', response)
      })
      .catch(error => {
