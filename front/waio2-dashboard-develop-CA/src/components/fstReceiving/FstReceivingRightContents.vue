<template>
    <div class="center-container">
      ...
      <div class="img-contents">
        <div class="btn-tab">
          ...
          <div class="btn-tab__box-disabled" @click="$routingByIndex(1.4)">펌프제어</div>
        </div>
        <div class="valve-box one-value-position">
          <div class="valve-box__text">원수 조절 밸브</div>
          <div class="box">
            <div class="box__value">{{ this.$store.state.fstReceiving.latest.b1_vv_po | numFormat('0.00') }}</div>
            <div class="box__unit">%</div>
          </div>
        </div>
        <div class="valve-box two-value-position">
          <div class="valve-box__text">회수 유량 사용 여부</div>
          <div class="btn-contents" style="width: 60px;">
            <div class="btn-contents__btn">
              <input type="checkbox" class="checkbox" @click="openPopup" :class="{ 'checked': this.$store.state.fstReceiving.latest.b_back_in_fr_flag == 1 }">
            </div>
          </div>
        </div>
        <div class="valve-box three-value-position">
          <div class="valve-box__text">회수 유량 사용 시 <br>추가 개도율 설정</div>
          <div class="box">
            <div class="box__value" :class="{'': this.$store.state.fstReceiving.isModifyModePo}">
              <input v-if="this.$store.state.fstReceiving.isModifyModePo" type="text" 
              :value="this.$store.state.fstReceiving.latestModify.b_back_in_fr_po" v-on:input="updateInput($event, 'b_back_in_fr_po')"/>
              <span class="contents-purify__text--num" v-else>
                {{ this.$store.state.fstReceiving.latestModify.b_back_in_fr_po | numFormat('0.0') }}
              </span>
            </div>
            <div class="box__unit">%</div>
          </div>
          <div v-if="$store.state.login.user.tkn !== null" class="modify-button">
            <div class="btn-bg" @click="updateControl">
              <div :class="[ this.$store.state.fstReceiving.isModifyModePo ? 'custom-icon__checkbox' : 'custom-icon__pencil' ]"></div>
            </div>
            <div v-if="this.$store.state.fstReceiving.isModifyModePo" class="custom-cancel-icon" style="margin-top: 5px;" @click="cancelControl">
        ...
        <div class="last-container">
            ...
            <div class="first-result-contents">
              <div class="result-value">
                <div class="result-text">
                  ...
                  <div class="result-text__value">{{ this.$store.state.fstReceiving.latest.b_in_fr | numFormat('0,0') }}</div>
                  <div class="result-text__unit">m<sup>3</sup>/h</div>
                </div>
                <!-- <div class="result-text">
                  <div class="result-text__text">· 원수 유입 압력</div>
                  <div class="result-text__value">{{ this.$store.state.fstReceiving.latest.b_in_pr | numFormat('0.00') }}</div>
                  <div class="result-text__unit">kgf/cm<sup>2</sup></div>
                </div> -->
                <div class="result-text">
                  <div class="result-text__text">· 정수지#1 수위</div>
                  <div class="result-text__value">{{ this.$store.state.fstReceiving.latest.h_location_le1 | numFormat('0.00') }}</div>
                  <div class="result-text__unit">m</div>
                </div>
                <div class="result-text">
                  <div class="result-text__text">· 정수지#2 수위</div>
                  <div class="result-text__value">{{ this.$store.state.fstReceiving.latest.h_location_le2 | numFormat('0.00') }}</div>
                  <div class="result-text__unit">m</div>
                </div>
                <div class="result-text">
                  <div class="result-text__text">· 정수지 총 유출 유량</div>
                  <div class="result-text__value">{{ this.$store.state.fstReceiving.latest.h_out_fr | numFormat('0,0') }}</div>
                  <div class="result-text__unit">m<sup>3</sup>/h</div>
                </div>
                <div class="result-text">
                  <div class="result-text__text">· 배출수지 회수 유량</div>
                  <div class="result-text__value">{{ this.$store.state.fstReceiving.latest.b_back_in_fr | numFormat('0,0') }}</div>
                  <div class="result-text__unit">m<sup>3</sup>/h</div>
                </div>
                <div class="result-text">
                  <div class="result-text__text">· 배출수지 유출거수위#1</div>
                  <div class="result-text__value">{{ this.$store.state.fstReceiving.latest.b_back_le1 | numFormat('0.00') }}</div>
                  <div class="result-text__unit">m</div>
                </div>
                <div class="result-text">
                  <div class="result-text__text">· 배출수지 유출거수위#2</div>
                  <div class="result-text__value">{{ this.$store.state.fstReceiving.latest.b_back_le2 | numFormat('0.00') }}</div>
                  <div class="result-text__unit">m</div>
                </div>
              </div>
            </div>
          ...
          <div class="first">
            ...
            <div class="second-result-contents paddingtop">
              ...
              <div class="value-box">
                <div class="value-box__value" v-html="this.$getNumeralWithCommaAndFontFamily(this.$store.state.fstReceiving.latest.ai_b_in_fri)"></div>
          ...
          <div class="first">
            <div class="first__title">
              ...
              <div class="first__timerbox-outter">
                <div class="timerbox">
                  {{ this.$store.state.fstReceiving.latest.upd_ti | moment('YYYY-MM-DD HH:mm:ss') }}
                </div>
              </div>
            </div>
            <div class="last-result-contents paddingtop">
              ...
              <div class="contents-title">
                ...
                <div class="real-box real-one">
                  ...
                  <div class="real-value">{{ this.$store.state.fstReceiving.latest.b1_vv_po | numFormat('0.00') }}</div>
                </div>
              </div>
              <div class="value-box">
                <div class="value-box__value">{{ this.$store.state.fstReceiving.latest.ai_b_vv_po | numFormat('0') }}</div>
  ...
  <script>
  import { OPEN_RECEIVING_DIALOG } from '@/store/modules/dialog'
  import { PUT_FSTRECEIVING_CONTROL_PO } from '@/store/modules/fstReceiving'
  export default {
    name: 'FstReceivingRightContents',
    data: () => ({
    ...
    methods: {
      /**
      * 회수 유량 사용 여부 on/off 버튼을 눌렀을 때
      * 확인 Dialog 오픈
      */
      openPopup: function () {
        if( this.$store.state.login.user.tkn !== null ) {
          this.$store.dispatch('dialog/' + OPEN_RECEIVING_DIALOG)
        }
      },
      ...
      updateControl: function() {
        let min = 0
        let max = 100
        if (this.$store.state.fstReceiving.isModifyModePo) {
          if (this.$store.state.fstReceiving.latestModify.b_back_in_fr_po === '') {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '값을 입력해주세요' })
          } else if (parseFloat(this.$store.state.fstReceiving.latestModify.b_back_in_fr_po) < min || parseFloat(this.$store.state.fstReceiving.latestModify.b_back_in_fr_po) > max) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '원수 조절 밸브 개도 설정 범위', text2: min + ' ~ ' + max })
          } else {
            let obj = {}
            obj.b_back_in_fr_po = parseFloat(this.$store.state.fstReceiving.latestModify.b_back_in_fr_po).toFixed(1)
            this.$store.dispatch(PUT_FSTRECEIVING_CONTROL_PO, obj)
            this.$store.state.fstReceiving.isModifyModePo = !this.$store.state.fstReceiving.isModifyModePo
            this.$store.state.fstReceiving.latest.b_back_in_fr_po = parseFloat(this.$store.state.fstReceiving.latestModify.b_back_in_fr_po).toFixed(1)
          }
        } else {
          this.$store.state.fstReceiving.isModifyModePo = !this.$store.state.fstReceiving.isModifyModePo
        }
  ...
  .center-container{
    ...
    .img-contents{
      width: 620px;
      height: 430px;
      background-image: url('../../assets/ca_images/sub_01.png');
