    ...
    <div class="contents-wrap">
      <div class="contents-value">
        <div class="contents-value__row">
          ...
          <div class="contents-value__number"><input v-if="this.$store.state.sndDisinfection.isModifyMode" type="text" :value="this.$store.state.sndDisinfection.latestModify.g_pre_set_max" v-on:input="updateInput($event, 'g_pre_set_max')"/><span v-else>{{ this.$store.state.sndDisinfection.latestModify.g_pre_set_max | numFormat('0.00') }}</span></div>
        </div>
        <div class="contents-value__row">
          <div class="contents-value__title">주입률하한</div>
          <div class="contents-value__number"><input v-if="this.$store.state.sndDisinfection.isModifyMode" type="text" :value="this.$store.state.sndDisinfection.latestModify.g_pre_set_min" v-on:input="updateInput($event, 'g_pre_set_min')"/><span v-else>{{ this.$store.state.sndDisinfection.latestModify.g_pre_set_min | numFormat('0.00') }}</span></div>
        </div>
        <div class="contents-value__row">
          <div class="contents-value__title">1회 변경 주입률</div>
          <div class="contents-value__number"><input v-if="this.$store.state.sndDisinfection.isModifyMode" type="text" :value="this.$store.state.sndDisinfection.latestModify.g_pre_chg_limit_for_onetime" v-on:input="updateInput($event, 'g_pre_chg_limit_for_onetime')"/><span v-else>{{ this.$store.state.sndDisinfection.latestModify.g_pre_chg_limit_for_onetime | numFormat('0.00') }}</span></div>
        </div>
        <div class="contents-value__row">
        ...
        <div class="contents-value__row">
          ...
          <div class="contents-value__number"><input v-if="this.$store.state.sndDisinfection.isModifyMode" type="text" :value="this.$store.state.sndDisinfection.latestModify.g_pre_calib_cycle" v-on:input="updateInput($event, 'g_pre_calib_cycle')"/><span v-else>{{ this.$store.state.sndDisinfection.latestModify.g_pre_calib_cycle | numFormat('0') }}</span></div>
        </div>
        <div class="contents-value__row">
...
<script>
import { PUT_DISINFECTION_CONTROL_PRE } from '@/store/modules/sndDisinfection'
export default {
  data: () => ({
    // isModifyMode: false
  }),
  ...
  methods: {
    ...
    updateControl: function() {
      ...
      if (this.$store.state.sndDisinfection.isModifyMode) {
        if (this.$store.state.sndDisinfection.latestModify.g_pre_set_max === ''
           || this.$store.state.sndDisinfection.latestModify.g_pre_set_min === ''
           || this.$store.state.sndDisinfection.latestModify.g_pre_chg_limit_for_onetime === ''
           || this.$store.state.sndDisinfection.latestModify.g_pre_calib_cycle === ''
          ...
          let obj = {}
          obj.g_pre_set_max = parseFloat(this.$store.state.sndDisinfection.latestModify.g_pre_set_max)
          obj.g_pre_set_min = parseFloat(this.$store.state.sndDisinfection.latestModify.g_pre_set_min)
          obj.g_pre_chg_limit_for_onetime = parseFloat(this.$store.state.sndDisinfection.latestModify.g_pre_chg_limit_for_onetime)
          obj.g_pre_calib_cycle = parseFloat(this.$store.state.sndDisinfection.latestModify.g_pre_calib_cycle)
  ...
  watch: {
    // '$store.state.sndDisinfection.latestModify.g_pre_set_max': function(newVal, oldVal) {
    //   if (newVal === '') {
    //     this.$store.state.sndDisinfection.latestModify['g_pre_set_max'] = newVal
    //   } else {
    //     if (this.checkNumberFormat(newVal)) {
    //       this.$store.state.sndDisinfection.latestModify['g_pre_set_max'] = newVal
    //     } else {
    //       this.$store.state.sndDisinfection.latestModify['g_pre_set_max'] = oldVal
    //     }
    //   }
    // },
    // '$store.state.sndDisinfection.latestModify.g_pre_set_min': function(newVal, oldVal) {
    //   if (newVal === '') {
    //     this.$store.state.sndDisinfection.latestModify['g_pre_set_min'] = newVal
    //   } else {
    //     if (this.checkNumberFormat(newVal)) {
    //       this.$store.state.sndDisinfection.latestModify['g_pre_set_min'] = newVal
    //     } else {
    //       this.$store.state.sndDisinfection.latestModify['g_pre_set_min'] = oldVal
    //     }
    //   }
    // },
    // '$store.state.sndDisinfection.latestModify.g_pre_chg_limit_for_onetime': function(newVal, oldVal) {
    //   if (newVal === '') {
    //     this.$store.state.sndDisinfection.latestModify['g_pre_chg_limit_for_onetime'] = newVal
    //   } else {
    //     if (this.checkNumberFormat(newVal)) {
    //       this.$store.state.sndDisinfection.latestModify['g_pre_chg_limit_for_onetime'] = newVal
    //     } else {
    //       this.$store.state.sndDisinfection.latestModify['g_pre_chg_limit_for_onetime'] = oldVal
    //     }
    //   }
    // },
    // '$store.state.sndDisinfection.latestModify.g_pre_calib_cycle': function(newVal, oldVal) {
    //   if (newVal === '') {
    //     this.$store.state.sndDisinfection.latestModify['g_pre_calib_cycle'] = newVal
    //   } else {
    //     if (this.checkNumberFormat(newVal)) {
    //       this.$store.state.sndDisinfection.latestModify['g_pre_calib_cycle'] = newVal
    //     } else {
    //       this.$store.state.sndDisinfection.latestModify['g_pre_calib_cycle'] = oldVal
    //     }
    //   }
    // },
