...
export default {
  ...
  data() {
    ...
    ChartData: function () {
      ...
      for (let i = 1, j = 3; i < 5; i ++, j --) {
        if (this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1))].start !== '0' 
        && this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1))].end !== '0') {
          _data.push({
            x: new Date(this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1))].start).getTime(),
            x2: new Date(this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1))].end).getTime(),
            y: j,
            color: colorXRange,
        ...
        if (this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1))].next_start !== '0' 
        && this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1))].next_end !== '0') {
          _data.push({
            x: new Date(this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1))].next_start).getTime(),
            x2: new Date(this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1))].next_end).getTime(),
            y: j,
            color: colorXRange,
            opacity: opacityRange,
          })
        ...
        if (this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1))].bw_start !== '0' 
        && this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1))].bw_end !== '0') {
          _data.push({
            x: new Date(this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1))].bw_start).getTime(),
            x2: new Date(this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + (i + (this.selectedLine - 1))].bw_end).getTime(),
            y: j,
            color: colorBW,
          })
        }
    ...
    ChartSchedule: function () {
      ...
      if (this.$store.state.trdFilter.latest.ai_f_location_schedule !== null) {
        let now = new Date()
        ...
        let minMax = this.$getMinMaxTimestampAIFLocationSchedule(this.$store.state.trdFilter.latest.ai_f_location_schedule)
        // 1. 가장 작은 시간
  ...
  methods: {
    ...
    downloadExcel: function() {
      ...
      for (let i = 1; i < 5; i ++) {
        if (this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + i].start !== '0' 
        && this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + i].end !== '0') {
          ret.push({
            x: new Date(this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + i ].start).getTime(),
            x2: new Date(this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + i ].end).getTime(),
            y: i
          })
        }
        if (this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + i ].next_start !== '0' 
        && this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + i ].next_end !== '0') {
          ret.push({
            x: new Date(this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + i ].next_start).getTime(),
            x2: new Date(this.$store.state.trdFilter.latest.ai_f_location_schedule['location' + i ].next_end).getTime(),
            y: i
          })
        }
      }
