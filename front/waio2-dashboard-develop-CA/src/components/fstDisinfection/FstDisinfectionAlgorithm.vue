  ...
  <div class="main">
    ...
    <div class="top">
      ...
      <div class="right">
        <div class="right-contents">
          <div class="right-contents__text-first">AI 운전 모드<br>(2단계공업)</div>
    ...
    <div class="contents">
      <div class="contents__left">
        <FstDisinfectionLeftContents/>
      </div>
      <div class="contents__right">
        <FstDisinfectionRightContents/>
      </div>
    </div>
  </div>  
</template>
<script>
import FstDisinfectionLeftContents from '@/components/fstDisinfection/FstDisinfectionLeftContents' 
import FstDisinfectionRightContents from '@/components/fstDisinfection/FstDisinfectionRightContents'
import { SET_OVERLAY } from '@/store'
import { GET_FSTDISINFECTION_LATEST, PUT_DISINFECTION_HISTORY_EVAPORATION, PUT_DISINFECTION_HISTORY_CHOLRATE } from '@/store/modules/fstDisinfection'
import { PUT_SEDIMENTATION_HISTORY_CL } from '@/store/modules/sedimentation'
import { PUT_CLEAR_HISTORY_CL} from '@/store/modules/fstClear'
import TopNavigator from '@/components/core/TopNavigator'
import { OPEN_AI_MODE_DIALOG } from '@/store/modules/dialog'
export default {
  name:'FstDisinfectionAlgorithm',
  data: () => ({
  ...
  mounted: function () {
    ...
    Promise.all([
      this.$store.dispatch(GET_FSTDISINFECTION_LATEST, { disinfectionStep : this.$store.state.fstDisinfection.disinfectionStep}),
      this.$store.dispatch(PUT_DISINFECTION_HISTORY_EVAPORATION, { disinfectionStep : this.$store.state.fstDisinfection.disinfectionStep}),
      this.$store.dispatch(PUT_SEDIMENTATION_HISTORY_CL, { disinfectionStep : this.$store.state.fstDisinfection.disinfectionStep}),
      this.$store.dispatch(PUT_DISINFECTION_HISTORY_CHOLRATE, { disinfectionStep : this.$store.state.fstDisinfection.disinfectionStep}),
      this.$store.dispatch(PUT_CLEAR_HISTORY_CL, { disinfectionStep : this.$store.state.fstDisinfection.disinfectionStep})
    ]).finally(() => {
