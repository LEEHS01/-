<template>
  <div class="main">
    <div class="btn-tab">
      <div class=" btn-tab__box-disabled" @click="$routingByIndex(7.1)">생활</div>
    ...
    <div class="contents-wrap">
      <div class="contents-value">
        ...
        <div class="contents-value__row">
          ...
          <div class="contents-value__number"><input v-if="this.$store.state.indDisinfection.isModifyMode" type="text" :value="this.$store.state.indDisinfection.latestModify.g_e_residual_cl_holding" v-on:input="updateInput($event, 'g_e_residual_cl_holding')"/><span v-else>{{ this.$store.state.indDisinfection.latestModify.g_e_residual_cl_holding | numFormat('0.00') }}</span></div>
        </div>
        <div class="contents-value__row">
...
export default {
  ...
  computed:{
    getDisplayStylePump1(){
      if (this.$store.state.indDisinfection.latest.g_pump_1_run == 1) {
          return 'block'
        }
      return 'none'
    },
    getDisplayStylePump2(){
      if (this.$store.state.indDisinfection.latest.g_pump_2_run == 1) {
          return 'block'
        }
      return 'none'
    },
    ...
    updateControl: function() {
      ...
      if (this.$store.state.indDisinfection.isModifyMode) {
        if (this.$store.state.indDisinfection.latestModify.g_pre_set_max === ''
          ...
          obj.g_pre_calib_cycle = this.$store.state.indDisinfection.latestModify.g_pre_calib_cycle
  ...
  watch: {
    ...
    //   }
    // },
    // '$store.state.indDisinfection.latestModify.g_pre_calib_cycle': function(newVal, oldVal) {
    //   if (newVal === '') {
    //     this.$store.state.indDisinfection.latestModify['g_pre_calib_cycle'] = newVal
    //   } else {
    //     if (this.checkNumberFormat(newVal)) {
    //       this.$store.state.indDisinfection.latestModify['g_pre_calib_cycle'] = newVal
    //     } else {
    //       this.$store.state.indDisinfection.latestModify['g_pre_calib_cycle'] = oldVal
    //     }
    //   }
    // },
