<template>
  <div class="main">
    <div class="top-contents">
        ...
        <div class="big-contents">
          <div class="contents-value">
            <div class="value-container">
              ...
              <div class="value-container__two">{{ this.$store.state.indDisinfection.latest.g_tei | numFormat('0.00') }}</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 수온(°C)</div>
              <div class="value-container__two">{{ this.$store.state.indDisinfection.latest.b_te | numFormat('0.00') }}</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">. 정수 pH</div>
              <div class="value-container__two">{{ this.$store.state.indDisinfection.latest.h_ph | numFormat('0.00') }}</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 적용 유량(m<sup>3</sup>/h)</div>
              <div class="value-container__two">{{ this.$store.state.indDisinfection.latest.b_in_fr | numFormat('0,0') }}</div>
              <!-- <div class="value-container__two">{{ this.$store.state.indDisinfection.latest.d_ser_in_fr | numFormat('0,0') }}</div> -->
            </div>
            <div class="value-container">
              <div class="value-container__one">· 혼화지 잔류 염소(ppm)</div>
              <div class="value-container__two">{{ this.$store.state.indDisinfection.latest.g_d_residual_cl | numFormat('0.00') }}</div>
              <!-- <div class="value-container__two">{{ this.$store.state.indDisinfection.latest.d_ser_cl | numFormat('0.0') }}</div> -->
            </div>
            <div class="value-container">
              <div class="value-container__one">· 침전지 잔류 염소(ppm)</div>
              <div class="value-container__two">{{ this.$store.state.indDisinfection.latest.g_e_residual_cl | numFormat('0.00') }}</div>
              <!-- <div class="value-container__two">{{ this.$store.state.indDisinfection.latest.d_ser_cl | numFormat('0.0') }}</div> -->
            </div>
            <div class="value-container">
              <div class="value-container__one">· 현재 주입률(ppm)</div>
              <div class="value-container__two">{{ this.$store.state.indDisinfection.latest.g_pre_chol_rate | numFormat('0.00') }}</div>
              <!-- <div class="value-container__two">{{ this.$store.state.indDisinfection.latest.g_pre2_chlorination | numFormat('0.0') }}</div> -->
            </div>
          </div>
        </div>
      </div>
      <div class="two-contents">
        ...
        <div class="small-contents">
          <div class="little-title-one">전차염 증발량 예측</div>
          <div class="small-contents-value">
            <div class="small-contents-value__title">{{this.$store.state.indDisinfection.latest.g_pump_1_run == 1? '1' : '2'}}호기</div>
            <div class="value">
              <div class="value__num">{{ this.$store.state.indDisinfection.latest.ai_g_evap | numFormat('0.00') }}</div>
      ...
      <div class="three-contents">
        <div class="three-contents__title">
          ...
          <div class="three-contents__timerbox-outter">
            <div class="timerbox">
              {{ this.$store.state.indDisinfection.latest.upd_ti | moment('YYYY-MM-DD HH:mm:ss') }}
            </div>
          </div>
        </div>
        <div class="three-small-contents">
          ...
          <div class="real-box real-two">
            ...
            <div class="real-value">{{ this.$store.state.indDisinfection.latest.g_pre_chol_rate | numFormat('0.00') }}</div>
          </div> 
          <div class="three-small-contents-value">
            <div class="three-small-contents-value__title">{{this.$store.state.indDisinfection.latest.g_pump_1_run == 1? '1' : '2'}}호기</div>
            <div class="three-value">
              <div class="three-value__num">{{ this.$store.state.indDisinfection.latest.ai_g_chol_rate | numFormat('0.00') }}</div>
    ...
    <div class="bottom-contents">
      <div class="contents">
        <div class="contents__title">전염소 증발량 예측</div>
        <div class="contents-gragh">
          <highcharts :options="ChartOptionGEvap" style="height:100%;"/>
        </div>
      </div>
      <div class="contents paddingleft">
        <div class="contents__title">침전수 잔류염소</div>
        <div class="real-box real-three">
          <div class="real-text">현재 잔류염소</div>
          <div class="real-value">{{ this.$store.state.indDisinfection.latest.g_e_residual_cl | numFormat('0.00')}}</div>
        </div> 
        <div class="contents-gragh">
...
export default {
  ...
  computed: {
    ChartOptionGEvap: function () {
      let chart = this.chartOptionGEvap
      let now = new Date()
      ...
      chart.series = []
      if (this.$store.state.indDisinfection.pre_evaporation !== null && Object.keys(this.$store.state.indDisinfection.pre_evaporation).length > 0) {
        let yesterdayData = []
        let todayData = []
        // TODO:  currentTimestamp 교체
        let currentTimestamp = new Date()
        // let currentTimestamp = new Date(2023, 7, 20, 23, 0, 0)
        ...
        for (let key in this.$store.state.indDisinfection.pre_evaporation){
          let _timestamp = new Date(key).getTime()
          if(_timestamp < todayTimestamp) {
            yesterdayData.push([_timestamp + this.$getMilliSecondFromHour(24), this.$store.state.indDisinfection.pre_evaporation[key]])
          }else{
            todayData.push([_timestamp, this.$store.state.indDisinfection.pre_evaporation[key]])
          }
        }
      ...
      chart.exporting.filename = this.$moment().format('YYYYMMDDHHmmss') + '_전차염 증발량 예측 트렌드'
      return chart
    },
    ...
    ChartOptionCl: function () {
      ...
      chart.series = []
      if (this.$store.state.indSedimentation.clTrend.series1 !== null && Object.keys(this.$store.state.indSedimentation.clTrend.series1).length > 0) {
        let dataSeries1 = []
        for (let key in this.$store.state.indSedimentation.clTrend.series1) {
          dataSeries1.push([new Date(key).getTime(), this.$store.state.indSedimentation.clTrend.series1[key]])
        }
        chart.series.push({
          name: '잔류염소',
          data: dataSeries1,
      ...
      // if (this.$store.state.indSedimentation.clTrend.series2 !== null && Object.keys(this.$store.state.indSedimentation.clTrend.series2).length > 0) {
      //   let dataSeries2 = []
      //   for (let key in this.$store.state.indSedimentation.clTrend.series2) {
      //     dataSeries2.push([new Date(key).getTime(), this.$store.state.indSedimentation.clTrend.series2[key]])
      //   }
      //   chart.series.push({
      //     name: '2계열',
      //     data: dataSeries2,
  ...
  methods: {
    downloadExcel: function() {
      const eva = this.$store.state.indDisinfection.pre_evaporation
