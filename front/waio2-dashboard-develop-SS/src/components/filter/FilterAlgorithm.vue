<template>
  <div class="main">
    ...
    <div class="top">
      ...
      <div class="title">생활여과<p>전체 현황</p></div>
      <div class="right">
        ...
        <div class="right-contents" style="margin-top: 10px; align-items: end;">
          ...
          <div class="right-contents__rightCon">
            ...
            <div class="right-contents__wrap">
              ...
              <div class="right-contents__input-box">
                <input v-if="this.$store.state.filter.isModifyMode" type="text" :value="this.$store.state.filter.latestModify.f_bw_tank_le" v-on:input="updateInput($event,'f_bw_tank_le')"/>
                <span v-else>{{ this.$store.state.filter.latestModify.f_bw_tank_le | numFormat('0.00') }}</span>
              </div>
            </div>
            <div class="right-contents__wrap">
...
export default {
    ...
    updateControl: function() {
      ...
      if(this.$store.state.filter.isModifyMode){
        if(this.$store.state.filter.latest.f_location_ti_set_max === '' || this.$store.state.filter.latest.f_location_wl_max === '' 
            || this.$store.state.filter.latest.f_bw_tank_le === '' || this.$store.state.filter.latest.f_back_le === '' 
        ){
        ...
        } else if (parseFloat(this.$store.state.filter.latestModify.f_bw_tank_le) < wl_min 
        || parseFloat(this.$store.state.filter.latestModify.f_bw_tank_le) > wl_max
        ...
        || parseFloat(this.$store.state.filter.latestModify.f_back_le) > wl_max) {
          ...
          obj.f_bw_tank_le = parseFloat(this.$store.state.filter.latestModify.f_bw_tank_le).toFixed(2)
  ...
  //     }
  //   },
  //   f_location_wl_max: function(newVal, oldVal) {
  //     // console.log(newVal, oldVal)
  //     if (newVal === '') {
  //       this.f_location_wl_max = newVal
  //     } else {
  //       if (this.checkFloatFormat(newVal)) {
  //         this.f_location_wl_max = newVal
  //       } else {
  //         this.f_location_wl_max = oldVal
  //       }
  //     }
  //   }
  // }
