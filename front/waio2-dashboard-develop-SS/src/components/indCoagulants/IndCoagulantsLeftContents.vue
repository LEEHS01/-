<template>
  <div class="main">
    <div class="btn-tab">
        <div class="btn-tab__box-disabled" @click="$routingByIndex(2)">생활</div>
        <div class="btn-tab__box-abled box-center-margin">공업</div>
    </div>
...
<script>
import { PUT_COAGULANT_AI_CF_COAGULANT } from '@/store/modules/indCoagulants'
export default {
  methods:{
  ...
  computed: {
    // 첫번째 약품통 애니메이션 여부
    getDisplayStyleC1() {
      if (this.$store.state.indCoagulants.latest.c_injector1 == 1) {
          return 'block'
        }
      return 'none'
    },
    // 두번째 약품통 애니메이션 여부
    getDisplayStyleC2() {
      if (this.$store.state.indCoagulants.latest.c_injector2 == 1) {
          return 'block'
        }
      return 'none'
    }
