        ...
        <div class="big-contents">
          <div class="contents-value">
            <div class="value-container">
              ...
              <div class="value-container__two">{{ this.$store.state.indCoagulants.latest.b_tb | numFormat('0.00') }}</div>
              <div class="value-container__three">NTU</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 원수 pH</div>
              <div class="value-container__two">{{ this.$store.state.indCoagulants.latest.b_ph | numFormat('0.00') }}</div>
              <div class="value-container__three"></div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 원수 수온</div>
              <div class="value-container__two">{{ this.$store.state.indCoagulants.latest.b_te | numFormat('0.00') }}</div>
              <div class="value-container__three">℃</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 원수 전기전도도</div>
              <div class="value-container__two">{{ this.$store.state.indCoagulants.latest.b_cu | numFormat('0.00') }}</div>
      ...
      <div class="two-contents">
        ...
        <div class="small-contents">
          ...
          <div class="real-box">
            ...
            <div class="real-value">{{this.$store.state.indCoagulants.latest.c_injector1 == 1 ? this.$store.state.indCoagulants.latest.c1_cf : this.$store.state.indCoagulants.latest.c2_cf  | numFormat('0.00') }}</div>
          </div>
          <div class="small-contents-value">
            <div class="small-contents-value__title"> <span>{{this.$store.state.indCoagulants.latest.c_injector1 == 1 ? '1' : '2'}}호기</span> {{ this.$store.state.indCoagulants.latest.c_cf_coagulant }}</div>
            <div class="value">
              <div class="value__num">{{ this.$store.state.indCoagulants.latest.ai_c_cf_norm_co | numFormat('0.00') }}</div>
            ...
            <div class="user-value-wrap">
              ...
              <div class="user-value-num">
                ...
                <input class="user-num" v-if="this.$store.state.indCoagulants.isModifyModeUser" type="text"
                :value="this.$store.state.indCoagulants.latestModify.c_user_correct" v-on:input="updateInput($event, 'c_user_correct')" maxlength="5">
                <span v-else>{{ this.$store.state.indCoagulants.latestModify.c_user_correct | numFormat('0.00') }}</span>
              </div>
              <div v-if="$store.state.login.user.tkn !== null" class="contents-value__icon">
                <div class="custom-icon" @click="updateControl">
                  <div :class="[ this.$store.state.indCoagulants.isModifyModeUser ? 'custom-icon__checkbox' : 'custom-icon__pencil' ]"></div>
                </div>
                <div v-if="this.$store.state.indCoagulants.isModifyModeUser" class="custom-cancel-icon" style="margin-top: 3px;" @click="cancelControl">
      ...
      <div class="three-contents">
        <div class="three-contents__title">최종 결과
          <div class="three-contents__timerbox-outter">
            <!-- 예측 결과 시간 -->
            <div class="timerbox">
              {{ this.$store.state.indCoagulants.latest.upd_ti | moment('YYYY-MM-DD HH:mm:ss') }}
            </div>
          </div>
        </div>
        <div class="three-small-contents">
          <div class="three-small-contents-value">
            <div class="three-small-contents-value__title"><span>{{this.$store.state.indCoagulants.latest.c_injector1 == 1 ? '1' : '2'}}호기</span> {{ this.$store.state.indCoagulants.latest.c_cf_coagulant }}</div>
            <div class="three-value">
              <div class="three-value__num">{{ this.$store.state.indCoagulants.latest.ai_c_cf | numFormat('0.00') }}</div>
    ...
    <div class="bottom-contents">
      ...
      <div class="contents paddingleft">
        ...
        <div class="contents__real-box">
          ...
          <div class="contents__real-value">{{ this.$store.state.indCoagulants.latest.e1_tb_b | numFormat('0.00')}}</div>
        </div>
        <div class="contents-gragh">
...
import { PUT_COAGULANT_CONTROL_USER_CORRECT } from '@/store/modules/indCoagulants'
...
export default {
  ...
  computed: {
    // 클래스 설명
    // ClassDescription() {
    //   if (this.$store.state.indCoagulants.latest.ai_clst_id !== null && this.$store.state.indCoagulants.class_info !== null) {
    //     if (this.$store.state.indCoagulants.class_info[this.$store.state.indCoagulants.latest.ai_clst_id] !== undefined) {
    //       return this.$store.state.indCoagulants.class_info[this.$store.state.indCoagulants.latest.ai_clst_id].description
    //     } else { 
    //       return null
    //     }
    //   }
    ...
    ScatterData1() {
      if (this.$store.state.indSedimentation.tbScatter.series1 && Object.keys(this.$store.state.indSedimentation.tbScatter.series1).length > 0) {
        let ar = []
        Object.keys(this.$store.state.indSedimentation.tbScatter.series1).forEach((key) => {
          let parseKey = parseFloat(key)
          ar.push([parseKey, this.$store.state.indSedimentation.tbScatter.series1[parseKey]])
        })
        return ar
      }
      return null
    ...
    // ScatterData2() {
    //   if (this.$store.state.indSedimentation.tbScatter.series2 && Object.keys(this.$store.state.indSedimentation.tbScatter.series2).length > 0) {
    //     let ar = []
    //     Object.keys(this.$store.state.indSedimentation.tbScatter.series2).forEach((key) => {
    //       let parseKey = parseFloat(key)
    //       ar.push([parseKey, this.$store.state.indSedimentation.tbScatter.series2[parseKey]])
    //     })
    //     return ar
    //   }
    //   return null
    ...
    // 선택된 클러스터 정보
    // selectedClusterInfo () {
    //   if (this.$store.state.indCoagulants.latest.ai_clst_id !== null && this.$store.state.indCoagulants.cluster_info !== null) {
    //     return this.$store.state.indCoagulants.cluster_info.find((elem) => {
    //       console.log(elem.cluster_id, this.$store.state.indCoagulants.latest.ai_clst_id)
    //       if (elem.cluster_id === this.$store.state.indCoagulants.latest.ai_clst_id) {
    //         return true
    //       } else {
    ...
    ChartTrend: function () {
      ...
      chart.series = []
      if (this.$store.state.indSedimentation.tbTrend.series1 && Object.keys(this.$store.state.indSedimentation.tbTrend.series1).length > 0) {
        let dataSeries1 = []
        for (let key in this.$store.state.indSedimentation.tbTrend.series1) {
          dataSeries1.push([new Date(key).getTime(), this.$store.state.indSedimentation.tbTrend.series1[key]])
        }
        chart.series.push({
          // name: '1호기',
          name: '침전지 탁도',
      ...
      // if (this.$store.state.indSedimentation.tbTrend.series2 !== null && Object.keys(this.$store.state.indSedimentation.tbTrend.series2).length > 0) {
        chart.tooltip = {
        useHTML: true,
        style: {
            pointerEvents: 'none'
      ...
      chart.plotOptions = {
        ...
        //   let dataSeries2 = []
      //   for (let key in this.$store.state.indSedimentation.tbTrend.series2) {
      //     dataSeries2.push([new Date(key).getTime(), this.$store.state.indSedimentation.tbTrend.series2[key]])
      //   }
      //   chart.series.push({
      //     name: '2계열',
      //     data: dataSeries2,
      ...
      //   })
      // }
      chart.yAxis.plotLines[0].value = this.$store.state.indCoagulants.latestModify.c_user_tb_e
  ...
  methods: {
    updateInput: function (event, key) {
      this.$store.state.indCoagulants.latestModify[key] = event.target.value
    },
    updateControl: function() {      
      if (this.$store.state.indCoagulants.isModifyModeUser) {
        if (this.$store.state.indCoagulants.latestModify.c_user_correct === '') {
          this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '값을 입력해주세요' })
        } else {
          let obj = {}
          obj.c_user_correct = parseFloat(this.$store.state.indCoagulants.latestModify.c_user_correct).toFixed(2)
          this.$store.dispatch(PUT_COAGULANT_CONTROL_USER_CORRECT, obj)
          this.$store.state.indCoagulants.isModifyModeUser = !this.$store.state.indCoagulants.isModifyModeUser
          this.$store.state.indCoagulants.latest.c_user_correct = parseFloat(this.$store.state.indCoagulants.latestModify.c_user_correct).toFixed(2)
        }
      } else {
        this.$store.state.indCoagulants.isModifyModeUser = !this.$store.state.indCoagulants.isModifyModeUser
      }
    ...
    cancelControl: function() {
      this.$store.state.indCoagulants.latestModify = Object.assign({}, this.$store.state.indCoagulants.latest)
      this.$store.state.indCoagulants.isModifyModeUser = !this.$store.state.indCoagulants.isModifyModeUser
    },
    // 알고리즘 예측 클래스 회전 함수
    // rotateCube: function () {
    //   if (this.carousel !== null) {
    //     let angle = this.theta * this.$store.state.indCoagulants.latest.ai_clst_id * -1
    //     if (angle === 0) {
    //       angle = -360
    //     }
    //     console.log('this.$store.state.indCoagulants.latest.ai_clst_id: ', this.$store.state.indCoagulants.latest.ai_clst_id, 'angle: ', angle)
  ...
  watch: {
    /**
     * $store.state.indCoagulants.latest.ai_clst_id 값 모니터링
     * 해당 값 변경시 회전 애니메이션
     */
    // '$store.state.indCoagulants.latest.ai_clst_id': function (newVal, oldVal) {
    //   if (oldVal !== newVal) {
    //     this.rotateCube()
    //   }
    // }
      ...
      .small-contents-value{
        ...
        .user-value-wrap {
          ...
          .user-value {
            ...
            font-size: 14px;
            color: #fff;
            background-image: url('../../assets/ss_images/user-value.png');
            margin-bottom: 10px;
          }
          .user-value-num 
            > span {
