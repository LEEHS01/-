<template>
  <div class="main">
    ...
    <div class="top">
      <div class="title">생활소독<p> 세부 현황</p></div>
      <div class="right">
        <div class="right-contents">
          <div class="right-contents__text-first">AI 운전 모드<br>(생활)</div>
        ...
        <div class="modBtnWrap">
          <button v-if="$store.state.disinfection.disinfectionStep == 1 " class="modBtn" @click="onClickModCholRate()">시간대별 주입률 설정</button>
        </div>
      </div>
...
import { GET_DISINFECTION_LATEST, PUT_DISINFECTION_HISTORY_EVAPORATION, PUT_DISINFECTION_HISTORY_CHOLRATE, PUT_DISINFECTION_CORRECT_DEGREE } from '@/store/modules/disinfection'
  ...
  methods: {
    ...
    getInitialDisinfectionData: function(){
      Promise.all([
        ...
        this.$store.dispatch(PUT_DISINFECTION_CORRECT_DEGREE)
      ]).finally(() => {
    ...
    onClickModCholRate: function() {
      if (this.$store.state.login.user.tkn !== null) {
        this.$store.state.disinfection.isModifyMode = true
