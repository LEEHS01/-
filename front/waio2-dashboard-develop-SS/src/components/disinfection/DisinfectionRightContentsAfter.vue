<template>
  <div class="main">
      ...
      <div class="two-contents">
        ...
        <div class="small-contents">
          ...
          <div class="small-contents-value">
            ...
            <div class="value">
              <div class="value__num">{{ this.$store.state.disinfection.latest.ai_g_correct_degree | numFormat('0.00') }}</div>
    ...
    <div class="bottom-contents">
      <div class="contents">
        <div class="contents__title">후염소 주입률 보정값 예측</div>
        <div class="contents-gragh">
          <highcharts :options="ChartOptionCorrectDegree" style="height:100%;"/>
        </div>
      </div>
      <div class="contents paddingleft">
        ...
        <div class="real-box real-three">
          ...
          <div class="real-value">{{ this.$store.state.disinfection.latest.g_h_in_residual_cl  | numFormat('0.00')}}</div>
          <div class="real-text__title">유출 : </div>
          <div class="real-value">{{ this.$store.state.disinfection.latest.g_h_out_residual_cl  | numFormat('0.00')}}</div>
        </div>  
        <div class="contents-gragh">
...
export default {
  ...
  computed: {
    ChartOptionCorrectDegree: function () {
      let chart = this.chartOptionCorrectDegree
      let now = new Date()
      ...
      chart.series = []
      if (this.$store.state.disinfection.correctDegree !== null && Object.keys(this.$store.state.disinfection.correctDegree).length > 0) {
        let dataSeries1 = []
        for (let key in this.$store.state.disinfection.correctDegree) {
          dataSeries1.push([new Date(key).getTime(), this.$store.state.disinfection.correctDegree[key]])
        }
        chart.series.push({
          name: '보정값',
          data: dataSeries1,
