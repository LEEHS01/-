<template>
  ...
  <div class="ai-record">
    ...
    <div class="top-info">
      <div class="step-area">
        <p class="step-area__title">{{ this.searchProcess.processStep == 1 ? '생활' : '공업' }} 단계</p>
      </div>
      <div class="ai-period">
        <div>조회기간 : </div>
    ...
    <div class="record-text-two">자율운영<p>AI운영 가동시간(7일)</p></div>
    <!-- 자율운영 차트 영역 -->
...
export default {
  ...
  data: () => ({
    ...
    processList: [{processStep : 1, text: '생활'},{processStep : 2, text: '공업'}],
    selectedProcess : {processStep : 1, text: '생활'},
    searchProcess: {processStep : 1, text: '생활'},
    chartOptions:{
      chart: {
        type: 'column',
  ...
  computed: {
    aiHistoryChart: function(){
      ...
      const categoryMap = {
        1: ['착수', '약품', '혼화응집', '침전', '여과', '전차염', '중차염', '후차염'],
        2: ['착수', '약품', '혼화응집', '침전', '전차염']
      };
      ...
      filteredHistorySearch = Object.fromEntries(
        Object.entries(aiOprHistorySearch).filter(([key]) =>
          key.includes(`${this.searchProcess.processStep == 1 ? 'LIV' : 'IND'}`)
        )
      );
  ...
  methods: {
    getAiOprDays:function(process, disinfectionIndex, opr) {
      ...
      let processCode = `${process}_${processStep == 1 ? 'LIV' : 'IND'}`
      if(processStep == 2 && (disinfectionIndex == 'PERI' || disinfectionIndex == 'POST')) { //2단계에서 중/후차염은 없음
        return '-'
      }
      if(processStep == 2 && process == 'F'){//2단계에서 여과 없음
        return '-'
      }
      return this.$store.state.aioprhistory.aiOprHistorySearch[`${processCode}_${disinfectionIndex}_${opr}`].days
    },
    getAiOprHours:function(process, disinfectionIndex, opr) {
      ...
      return this.$store.state.aioprhistory.aiOprHistorySearch[`${processCode}_${disinfectionIndex}_${opr}`].hours
    },
    getAiOprTotalHours:function(process, disinfectionIndex, opr) {
      ...
      return this.$store.state.aioprhistory.aiOprHistorySearch[`${processCode}_${disinfectionIndex}_${opr}`].totalHours
    },
    onClickSearchBtn () {
