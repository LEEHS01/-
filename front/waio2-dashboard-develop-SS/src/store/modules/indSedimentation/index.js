...
export default {
  ...
  getters: {
    ...
    isOpenInBalValve1: function (state) {
      if (state.latest.e_drawing_vv1_1 === 1 || state.latest.e_drawing_vv1_2 === 1 ) {
        return true
      } else {
        return false
      }
    ...
    isOpenInBalValve2: function (state) {
      if (state.latest.e_drawing_vv2_1 === 1 || state.latest.e_drawing_vv2_2 === 1 ) {
        return true
      } else {
        return false
      }
    ...
    isOpenInBalValve3: function (state) {
      if (state.latest.e_drawing_vv3_1 === 1 || state.latest.e_drawing_vv3_2 === 1 ) {
        return true
      } else {
        return false
      }
    ...
    isOpenInBalValve4: function (state) {
      if (state.latest.e_drawing_vv4_1 === 1 || state.latest.e_drawing_vv4_2 === 1 ) {
        return true
      } else {
        return false
      }
  ...
  actions: {
    ...
    [PUT_HISTORY_CL]: async function ({ commit, rootState}) {
      ...
      let oneDayTimestamp = 1000 * 60 * 60 * 24
      await axios.put(`${DEV_SERVER}/${URL.SEDIMENTATION_HISTORY_CL}/${PROCESS_STEP}/${rootState.indDisinfection.disinfectionStep}`, { 'start_time': new Date(nowTimestamp - oneDayTimestamp).toISOString(), 'end_time': new Date(nowTimestamp).toISOString() })
        .then(({ data }) => {
