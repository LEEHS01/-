  ...
  <div class="main">
    <div class="top-contents">
      ...
      <div class="one-contents">
        <div class="one-contents__title">중차염 주요인자</div>
        <div class="big-contents">
          <div class="contents-value">
            <div class="value-container">
              <div class="value-container__one">· 정수 탁도(NTU)</div>
              <div class="value-container__two">{{ this.$store.state.midDisinfection.latest.h_tb | numFormat('0.00') }}</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 정수 pH</div>
              <div class="value-container__two">{{ this.$store.state.midDisinfection.latest.h_ph | numFormat('0.00') }}</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 침전지 잔류염소(ppm)</div>
              <div class="value-container__two">{{ this.$store.state.midDisinfection.latest.e1_cl | numFormat('0.00') }}</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 여과지 유출 잔류염소(ppm)</div>
              <div class="value-container__two">{{ this.$store.state.midDisinfection.latest.g_f_out_residual_cl | numFormat('0.00') }}</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 현재 주입률(ppm)</div>
              <div class="value-container__two">{{ this.$store.state.midDisinfection.latest.g_peri_chol_rate | numFormat('0.00') }}</div>
            </div>
            <div class="value-container">
              <div class="value-container__one">· 착수정 유입유량(m<sup>3</sup>/h)</div>
              <div class="value-container__two">{{ this.$store.state.midDisinfection.latest.b_in_fr | numFormat('0,0') }}</div>
            </div>
          </div>
        </div>
      </div>
      <div class="three-contents">
        <div class="three-contents__title">
          ...
          <div class="three-contents__timerbox-outter">
            <div class="timerbox">
              {{ this.$store.state.midDisinfection.latest.upd_ti | moment('YYYY-MM-DD HH:mm:ss') }}
            </div>
          </div>
        </div>
        <div class="three-small-contents">
          <div class="little-title-two">AI 중차염 주입률 예측</div>
          <div class="real-box real-two">
            <div class="real-text">현재 주입률</div>
            <div class="real-value">{{ this.$store.state.midDisinfection.latest.g_peri_chol_rate | numFormat('0.00') }}</div>
          </div>  
          <div class="three-small-contents-value">
            <div class="three-small-contents-value__title">{{ getPumpRun }}</div>
            <div class="three-value">
              <div class="three-value__num">{{ this.$store.state.midDisinfection.latest.ai_g_peri_chol | numFormat('0.00') }}</div>
    ...
    <div class="bottom-contents">
      ...
      <div class="contents paddingleft">
        ...
        <div class="real-box real-three">
          ...
          <div class="real-value">{{ this.$store.state.midDisinfection.latest.e1_cl | numFormat('0.00') }}</div>
        </div>     
        <div class="contents-gragh">
...
export default {
  ...
  computed: {
    getPumpRun: function() {
      let pumpRun = ''
      if(this.$store.state.midDisinfection.latest.g_inj_b_run == 1) pumpRun += '차염B'
      if(this.$store.state.midDisinfection.latest.g_inj_d_run == 1) pumpRun += '차염D'
      return pumpRun
    },
    //중차염 주입률 예측 차트
    ChartOptionCholRate: function () {
      ...
      if (this.$store.state.midDisinfection.ai_g_peri_chol_rate_trend !== null) {
        
        let cholRateData = []
        for(let key in this.$store.state.midDisinfection.ai_g_peri_chol_rate_trend){
          cholRateData.push([new Date(key).getTime(), this.$store.state.midDisinfection.ai_g_peri_chol_rate_trend[key]])
        }
        chart.series.push({
          name: '주입률',
          data: cholRateData,
        ...
        chart.exporting.filename = this.$moment().format('YYYYMMDDHHmmss') + '_중차염 주입률 예측 비교'
  ...
  methods: {
    downloadExcel: function() {
      const eva = this.$store.state.midDisinfection.pre_evaporation
      ...
      XLSX.writeFile(wb, `${this.$moment().format('YYYYMMDDHHmmss')}_중차염 증발량 예측 비교.xlsx`);
    }
  },
...
.top-contents{
    ...
    .three-small-contents{
      ...
      .three-small-contents-value{
        ...
        .three-contents-value-underbar{
          ...
          width: 100%;
          height: 36px;
          background-image: url('../../assets/br_images/contents_value_underbar.png');
  ...
  .one-contents{
    ...
    .big-contents{
      ...
      height: 356px;
      padding-top: 16px;
      background-image: url('../../assets/br_images/big_contents_background.png');
      .contents-value{
        display: flex;
        ...
        width: 450px;
        height: 307px;
        background-image: url('../../assets/br_images/value_factor.png');
        padding: 15px;
        .value-container{
          display: flex;
          width: 100%;
