<template>
  <div class="main">
    ...
    <div class="btn-tab">
      <div :class="[ this.$store.state.disinfection.selectedDisinfectionIndex === 1 ? 'btn-tab__box-abled' : 'btn-tab__box-disabled']" @click="$routingByIndex(7)">전차염</div>
      <div :class="[ this.$store.state.disinfection.selectedDisinfectionIndex === 2 ? 'btn-tab__box-abled' : 'btn-tab__box-disabled']" @click="$routingByIndex(8)">중차염</div>
      <div :class="[ this.$store.state.disinfection.selectedDisinfectionIndex === 3 ? 'btn-tab__box-abled' : 'btn-tab__box-disabled']" @click="$routingByIndex(9)">후차염</div>
    </div>
    <div class="contents-img">
      <div class="contents-img-title first">차염A(전차염)</div>
    ...
    <div class="contents-wrap">
      <div class="contents-value">
        ...
        <div class="contents-value__row">
          ...
          <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_pre_calib_cycle" v-on:input="updateInput($event, 'g_pre_calib_cycle')" maxlength="5"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_pre_calib_cycle | numFormat('0') }}</span></div>
        </div>
        <div class="contents-value__row">
...
export default {
  ...
  computed: {
    getDisplayStyleFirst() {
      return this.$store.state.disinfection.latest.g_inj_a_run === 1 ? 'block' : 'none'
    },
    getDisplayStyleSecond() {
      return this.$store.state.disinfection.latest.g_inj_d_run === 1 ? 'block' : 'none'
    }
  },
  methods: {
    ...
    updateControl: function() {
      ...
      /**
       * pre_min_limit  : 전차염 주입률 하한
       * pre_max_limit  : 전차염 주입률 상한
       * cl_min_limit   : 전차염 혼화지 잔류염소 하한
       * cl_max_limit   : 전차염 혼화지 잔류염소 상한
       */
      let pre_min_limit = 0
      ...
      if (this.$store.state.disinfection.isModifyMode) {
        ...
        //전차염 주입률 상한, 전차염 주입률 하한, 1회 변경 주입률, 보정주지, 혼화지 목표 잔류염소 입력 값 공백 체그
        if (this.$store.state.disinfection.latestModify.g_pre_set_max === ''
          ...
          if ((obj.g_pre_set_max < pre_min_limit || obj.g_pre_set_max > pre_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '전차염 주입률 상한 설정 범위', text2: pre_min_limit + ' ~ ' + pre_max_limit})
            return
          }
          if ((obj.g_pre_set_min < pre_min_limit || obj.g_pre_set_min > pre_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '전차염 주입률 하한 설정 범위', text2: pre_min_limit + ' ~ ' + pre_max_limit})
            return
          }
          if ((obj.g_pre_chg_limit_for_onetime < pre_min_limit || obj.g_pre_chg_limit_for_onetime > pre_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '전차염 1회 변경 주입률 설정 범위', text2: pre_min_limit + ' ~ ' + pre_max_limit})
            return
          }
          if ((obj.g_e_obj_residual_cl < cl_min_limit || obj.g_e_obj_residual_cl > cl_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '전차염 침전지 목표 잔류염소 설정 범위', text2: cl_min_limit + ' ~ ' + cl_max_limit})
            return
          }

          //processStep 및 소독 단계 설정
          obj.processStep = this.$store.state.disinfection.processStep
