  ...
  data() {
    ...
    ChartData: function () {
      ...
      for(let i=idx; i<max; i++){
          ...
          if (this.$store.state.filter.latest.ai_f_location_schedule['location' + i].bw_start !== '0' 
        && this.$store.state.filter.latest.ai_f_location_schedule['location' + i].bw_end !== '0') {
          _data.push({
            x: new Date(this.$store.state.filter.latest.ai_f_location_schedule['location' + i].bw_start).getTime(),
            x2: new Date(this.$store.state.filter.latest.ai_f_location_schedule['location' + i].bw_end).getTime(),
            y: yAxisIdx,
            color: colorBW,
          })
        }
    ...
    ChartSchedule: function () {
      ...
      if (this.$store.state.filter.latest.ai_f_location_schedule !== null) {
        ...
        if (this.selectedLine === 1) {
          chart.yAxis.categories = ['11지','10지','9지','8지','7지','6지','5지','4지','3지','2지','1지']
        } else {
          chart.yAxis.categories = ['22지','21지','20지','19지','18지','17지','16지','15지','14지','13지','12지']
        }
