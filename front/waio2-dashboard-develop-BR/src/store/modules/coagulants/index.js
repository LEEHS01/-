...
export default {
  namespaced: true,
  state: {
    coagulant_type: ['APAC', 'PACS', 'PAC', 'PAHCS'],
    processStep: 1,
    isModifyMode: false,
  ...
  actions: {
    ...
    [PUT_CONTROL_OPERATION]: async function ({ commit }, { operation, processStep }) {
      await axios.put(`${DEV_SERVER}/`+ 'coagulant/control/operation/'+processStep, { 'operation': operation })
      .then(() => {
    ...
    [PUT_CONTROL_AI]: async function ({ commit }, { c_cf_min, c_cf_max, processStep, c_user_tb_e }) {
      await axios.put(`${DEV_SERVER}/`+'coagulant/control/ai/'+processStep, { c_cf_min, c_cf_max, c_user_tb_e })
      .then((response) => {
        let _data = {
          visible: true,
          title: '설정 성공',
    ...
    [PUT_CONTROL_COAGULANT]: async function ({ commit }, { c_cf_coagulant }) {
      await axios.put(`${DEV_SERVER}/${URL.COAGULANT_CONTROL_COAGULANT}/` + PROCESS_STEP, { c_cf_coagulant })
      .then((response) => {
        let _data = {
          visible: true,
          title: '설정 성공',
