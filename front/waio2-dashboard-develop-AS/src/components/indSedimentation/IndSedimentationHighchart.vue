    ...
    <div class="chart-contents chartwidth20" style="display: flex; flex-direction: column;">
      ...
      <div class="box-config">
        ...
        <div class="box-config--list">
          ...
          <div class="box-input" :class="{'box-input__active': this.$store.state.sedimentation.isModifyMode}">
            <div class="box-input--value"><input v-if="this.$store.state.sedimentation.isModifyMode" type="text" :value="this.$store.state.sedimentation.latestModify.e_sc_set_max_wait" v-on:input="updateInput($event, 'e_sc_set_max_wait')"/>
              <span v-else>{{ this.$store.state.sedimentation.latestModify.e_sc_set_max_wait }}</span> 
            </div>
  ...
  computed: {
    ...
    ChartSludgeAmount: function () {
      ...
      for (let i=0; i < this.location.length; i++) {
        ...
        // 슬러지 적산량
        let sludgeKey = 'e_location_sludge_' + (i + 11)
        let sludgeAmount = this.$store.state.sedimentation.latest[sludgeKey]
        // 슬러지 경과시간
        let sludgeElapsedTimeKey = 'e_location_sludge_elapsedTime_' + (i + 11)
