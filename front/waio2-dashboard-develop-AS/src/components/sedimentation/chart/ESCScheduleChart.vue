...
export default {
  ...
  computed: {
    ...
    ChartSchedule: function () {
      ...
      if(this.$store.state.sedimentation.latest.locationMin != null) {
        ...
        let data = []
        for (let i = this.$store.state.sedimentation.latest.locationMin; i <= this.$store.state.sedimentation.latest.locationMax; i ++) {
          // category.push(this.$store.state.sedimentation.latest.locationMax - i + '지')
          // 현재 대차 스케줄
          ...
          && this.$store.state.sedimentation.latest['e_sc' + i + '_schedule'].stop !== null) {    
            data.push({
              ...
              y: this.$store.state.sedimentation.latest.locationMax - i,
              color: colorXRange
            })
          }
          // 다음 대차 스케줄
        ...
        // chart.yAxis.categories = this.category
        chart.series = [{
          name: '수집기 운영 스케줄',
          pointWidth: 15,
          borderWidth: 0,
