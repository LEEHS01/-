<template>
  ...
  <div class="ai-record">
    ...
    <div class="top-info">
      <div class="step-area">
        <p class="step-area__title">{{ this.searchProcess }} 단계</p>
      </div>
      <div class="ai-period">
        <div>조회기간 : </div>
...
export default {
  ...
  data: () => ({
    ...
    processList: [{processStep : 1, text: '1단계'},{processStep : 2, text: '2단계'},{processStep : 3, text: '3단계'}],
    ...
    aiHistoryChart: function(){
      ...
      filteredHistorySearch = Object.fromEntries(
        Object.entries(aiOprHistorySearch).filter(([key]) =>
          key.includes(`${this.searchProcess}_`)
        )
      );
  ...
  methods: {
    getAiOprDays:function(process, opr) {
      ...
      if(processStep != 2 && process == 'F') { //2단계만 여과 존재
        return '-'
      }
    ...
    getAiOprDaysDisinfection:function(process, disinfectionIndex, opr) {
      let processStep = this.searchProcess
      if(processStep != 2 && disinfectionIndex == 'POST') { //2단계만 후차염 존재
        return '-'
      }
