    ...
    alarms7DaysFormatted () {
      ...
      if (this.$store.state.alarm.alarms7Days.length > 0) {
        this.$store.state.alarm.alarms7Days.forEach((it) => { 
          // 공정 및 단계 몇칭 변경
          it.processTitle = it.processStep + '단계 ' + this.$getProcessTitle(it.process == 'G' ? it.process + it.disinfectionIndex : it.process)
      ...
      // this.$store.state.alarm.selectedAlarmList = _it
      return _it
    }
  },
  methods: {
