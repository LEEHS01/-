...
export default {
    ...
    // 수집기 운영 스케줄 차트
      chartDataSchedule: {
        ...
        yAxis: {
          ...
          categories: ['12지','11지','10지','9지','8지','7지','6지','5지','4지','3지','2지','1지'],
          gridLineColor: false,
          labels: {
            style:{
              fontFamily:'NanumSquare',
  ...
  computed: {
    ...
    ChartSchedule: function () {
      ...
      let minMax = this.$getMinMaxTimestampAIELocationSchedule(this.$store.state.trtIndSedimentation.latest)
      //1. 가장 작은 시간
      ...
      for (let i = 2; i < 10; i ++) {
        // 현재 대차 스케줄
        if (this.$store.state.trtIndSedimentation.latest['e_sc' + i + '_schedule'].start !== null 
        && this.$store.state.trtIndSedimentation.latest['e_sc' + i + '_schedule'].stop !== null) {    
          data.push({
            x: new Date(this.$store.state.trtIndSedimentation.latest['e_sc' + i + '_schedule'].start).getTime(),
            x2: new Date(this.$store.state.trtIndSedimentation.latest['e_sc' + i + '_schedule'].stop).getTime(),
            y: 9 - i,
            color: colorXRange
          })
        }
        // 다음 대차 스케줄
        if (this.$store.state.trtIndSedimentation.latest['e_sc' + i + '_schedule'].next_start !== null 
        && this.$store.state.trtIndSedimentation.latest['e_sc' + i + '_schedule'].next_end !== null) {    
          data.push({
            x: new Date(this.$store.state.trtIndSedimentation.latest['e_sc' + i + '_schedule'].next_start).getTime(),
            x2: new Date(this.$store.state.trtIndSedimentation.latest['e_sc' + i + '_schedule'].next_end).getTime(),
            y: 9 - i,
            color: colorXRange
          })
        }
        // inbal 스케줄
        if (this.$store.state.trtIndSedimentation.latest['e_sc' + i + '_schedule'].inbal !== null 
        && this.$store.state.trtIndSedimentation.latest['e_sc' + i + '_schedule'].inbal !== '') {
          data.push({
            x: new Date(this.$store.state.trtIndSedimentation.latest['e_sc' + i + '_schedule'].inbal).getTime(),
            x2: new Date(this.$store.state.trtIndSedimentation.latest['e_sc' + i + '_schedule'].inbal).getTime() + inbalTimeLength,
            y: 9 - i,
            color: colorInbal
          })
        }
      ...
      chart.yAxis.plotLines = chart.yAxis.categories.map((_, index) => ({
        color: '#C0C0C0',
        width: 0.5,
        value: index + 0.5
      }))
      .filter((_, index) => index !== chart.yAxis.categories.length -1);
