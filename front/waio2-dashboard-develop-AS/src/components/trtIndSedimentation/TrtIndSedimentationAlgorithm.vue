<template>
  <div class="main">
    ...
    <div class="contents">
      <!-- 중앙 컨텐츠 -->
      <TrtIndSedimentationContents/>
    </div>
    <div class="bottom">
      <!-- 하단 컨텐츠 -->
      <TrtIndSedimentationHighchart/>
    </div>
  </div>  
</template>
<script>
import TrtIndSedimentationContents from '@/components/trtIndSedimentation/TrtIndSedimentationContents' 
import TrtIndSedimentationHighchart from '@/components/trtIndSedimentation/TrtIndSedimentationHighchart' 
...
export default {
  name:'TrtIndSedimentationAlgorithm',
  data: () => ({
  ...
  methods: {
    ...
    onClickAICheckbox: function(index, expectedValue) {
      if( this.$store.state.login.user.tkn !== null ) {
        ...
        this.$store.state.dialog.aiMode.processStep = this.$store.state.sedimentation.processStep
   ...
   */
  mounted: function() {
    this.$store.state.sedimentation.isModifyMode = false
    this.$store.state.selectedBuildingIndex = 18
    this.$store.state.sedimentation.processStep = 3
    this.$store.commit(SET_OVERLAY, true)
    Promise.all([
      this.$store.dispatch(GET_SEDIMENTATION_LATEST, this.$store.state.sedimentation.processStep)
      // this.$store.dispatch(PUT_SEDIMENTATION_INTERFACE, { numJi: 0, processStep: this.$store.state.sedimentation.processStep })
    ]).finally(() => {
