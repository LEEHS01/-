        ...
        <div class="top-container">
          <div class="top-one-contents-box">
            ...
            <div class="top-one-contents-value">
              ...
              <input v-if="this.$store.state.indMixing.isModifyMode" type="text" class="top-one-contents-value__input" :value="this.$store.state.indMixing.latestModify.d_g_value1" v-on:input="updateInput($event, 'd_g_value1')"/>
              <span class="top-one-contents-value__span" v-else>{{ this.$store.state.indMixing.latestModify.d_g_value1 | numFormat('0.0')}}</span>
              <div class="top-one-contents-value__unit">sec<sup>-1</sup></div>
            </div>
            <div class="top-one-contents-value">
              <div class="top-one-contents-value__text">#2</div>
              <input v-if="this.$store.state.indMixing.isModifyMode" type="text" class="top-one-contents-value__input" :value="this.$store.state.indMixing.latestModify.d_g_value2" v-on:input="updateInput($event, 'd_g_value2')"/>
              <span class="top-one-contents-value__span" v-else>{{ this.$store.state.indMixing.latestModify.d_g_value2 | numFormat('0.0')}}</span>
              <div class="top-one-contents-value__unit">sec<sup>-1</sup></div>
            </div>
            <div class="top-one-contents-value">
              <div class="top-one-contents-value__text">#3</div>
              <input v-if="this.$store.state.indMixing.isModifyMode" type="text" class="top-one-contents-value__input" :value="this.$store.state.indMixing.latestModify.d_g_value3" v-on:input="updateInput($event, 'd_g_value3')"/>
              <span class="top-one-contents-value__span" v-else>{{ this.$store.state.indMixing.latestModify.d_g_value3 | numFormat('0.0') }}</span>
              <div class="top-one-contents-value__unit">sec<sup>-1</sup></div>
            </div>
...
export default {
  ...
  methods: {
    ...
    updateControl: function() {      
      ...
      if (this.$store.state.indMixing.isModifyMode) {
        if (this.$store.state.indMixing.latestModify.d_g_value1 === ''
          ...
          || this.$store.state.indMixing.latestModify.d_g_value3 === '') {
          this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '값을 입력해주세요' })
        } else if (parseInt(this.$store.state.indMixing.latestModify.d_g_value1) <= min 
                  || parseInt(this.$store.state.indMixing.latestModify.d_g_value1) >= max
                  || parseInt(this.$store.state.indMixing.latestModify.d_g_value2) <= min
                  || parseInt(this.$store.state.indMixing.latestModify.d_g_value2) >= max
                  || parseInt(this.$store.state.indMixing.latestModify.d_g_value3) <= min
                  || parseInt(this.$store.state.indMixing.latestModify.d_g_value3) >= max) {
          ...
          obj.d_g_value_step3 = parseFloat(this.$store.state.indMixing.latestModify.d_g_value3)
    ...
    downloadExcel: function() {
      ...
      for (let i = 0; i < Object.keys(speedPrediction).length; i++) {
        for (let speedPrediction_obj in speedPrediction['location' + (i + 11)]) {
          for (let time in speedPrediction['location' + (i + 11)]['step1']) {
            const excelLogs = {
              DateTime: '', 
              계열1: '',
              계열2: '',
            ...
            excelLogs.계열1 = speedPrediction['location' + (i + 11)]['step1'][time]
            excelLogs.계열2 = speedPrediction['location' + (i + 11)]['step2'][time]
            excelLogs.계열3 = speedPrediction['location' + (i + 11)]['step3'][time]

            const speedPrediction_length = Object.keys(speedPrediction['location' + (i + 11)][speedPrediction_obj]).length
      
            if ( i === 0 ) {
              if( speedPrediction_length != this.ji11.length ) {
                this.ji11.push(excelLogs);
              }
            } else if ( i === 1 ) {
              if( speedPrediction_length != this.ji12.length ) {
                this.ji12.push(excelLogs);
              }
            } 
          }
        }      
      ...
      const dataWS11 = XLSX.utils.json_to_sheet(this.ji11);
      const dataWS12 = XLSX.utils.json_to_sheet(this.ji12);
      
      XLSX.utils.book_append_sheet(wb, dataWS11, '11지'); 
      XLSX.utils.book_append_sheet(wb, dataWS12, '12지');
    ...
    onClickDetail: function() {
      ...
      this.$store.state.dialog.autoModeCrt.processStep = this.$store.state.indMixing.processStep
      this.$store.state.dialog.autoModeCrt.d_g_step1_crt = this.$store.state.indMixing.latestModify.d_g_step1_crt.toFixed(2)
      this.$store.state.dialog.autoModeCrt.d_g_step2_crt = this.$store.state.indMixing.latestModify.d_g_step2_crt.toFixed(2)
      this.$store.state.dialog.autoModeCrt.d_g_step3_crt = this.$store.state.indMixing.latestModify.d_g_step3_crt.toFixed(2)
    }  
  },
  /**
   * 마운트 해제시 
  ...
  watch: {
  //   // 교반강도 #1
  //   '$store.state.indMixing.latestModify.d_g_value1': function(newVal, oldVal) {
  //     if (newVal === '') {
  ...
  //   },
  //   // 교반강도 #2
  //   '$store.state.indMixing.latestModify.d_g_value2': function(newVal, oldVal) {
  //     if (newVal === '') {
