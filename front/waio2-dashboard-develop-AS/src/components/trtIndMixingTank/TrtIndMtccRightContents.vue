      ...
      <div class="top-contents-container">
        ...
        <div class="top-container">
          <div class="top-one-contents-box">
            <div class="toggle-wrap">
              ...
              <div class="btn-contents" style="width: 60px;">
                <div class="btn-contents__btn">
                  <input type="checkbox" :disabled="this.$store.state.login.user.tkn === null" class="checkbox" :class="{ 'checked': this.$store.state.dialog.aiMode.d_g_value_ctr_flag === 1 }" true-value="1" false-value="0" v-model="$store.state.dialog.aiMode.d_g_value_ctr_flag" @click="ctrChk">
                </div>
              </div>
            </div>
            <div class="top-one-contents-value">
              <div class="top-one-contents-value__text">#1</div>
              <input v-if="this.$store.state.trtIndMixing.isModifyMode" type="text" class="top-one-contents-value__input" :value="this.$store.state.trtIndMixing.latestModify.d_g_value1" v-on:input="updateInput($event, 'd_g_value1')"/>
              <span class="top-one-contents-value__span" v-else>{{ this.$store.state.trtIndMixing.latestModify.d_g_value1 | numFormat('0.0')}}</span>
              <div class="top-one-contents-value__unit">sec<sup>-1</sup></div>
            </div>
            <div class="top-one-contents-value">
              <div class="top-one-contents-value__text">#2</div>
              <input v-if="this.$store.state.trtIndMixing.isModifyMode" type="text" class="top-one-contents-value__input" :value="this.$store.state.trtIndMixing.latestModify.d_g_value2" v-on:input="updateInput($event, 'd_g_value2')"/>
              <span class="top-one-contents-value__span" v-else>{{ this.$store.state.trtIndMixing.latestModify.d_g_value2 | numFormat('0.0')}}</span>
              <div class="top-one-contents-value__unit">sec<sup>-1</sup></div>
            </div>
            <div class="top-one-contents-value">
              <div class="top-one-contents-value__text">#3</div>
              <input v-if="this.$store.state.trtIndMixing.isModifyMode" type="text" class="top-one-contents-value__input" :value="this.$store.state.trtIndMixing.latestModify.d_g_value3" v-on:input="updateInput($event, 'd_g_value3')"/>
              <span class="top-one-contents-value__span" v-else>{{ this.$store.state.trtIndMixing.latestModify.d_g_value3 | numFormat('0.0') }}</span>
              <div class="top-one-contents-value__unit">sec<sup>-1</sup></div>
            </div>
            ...
            <div class="auto-btn" v-if="$store.state.login.user.tkn !== null && $store.state.dialog.aiMode.d_g_value_ctr_flag == 1"><button class="auto-btn__detail" @click="onClickDetail()">자동모드 상세</button></div>
          </div>
      ...
      <div class="top-two-contents-container">
        ...
        <div class="top-container">
          <div class="top-container__real-box">
            ...
            <div class="top-container__real-value">{{ this.getFCLocationSpeed(this.$store.state.trtIndMixing.selectedFCLocation).step1['1'] | numFormat('0.00') }}</div>
            <div class="top-container__real-unit">RPM</div>
          </div>
          <div class="top-two-contents-value">
            <div class="top-two-contents-value__text">· #1</div>
            <div class="top-two-contents-value__value">{{ this.getAIFCLocationSpeed(this.$store.state.trtIndMixing.selectedFCLocation).step1['1'] | numFormat('0.00') }}</div>
          ...
          <div class="top-container__real-box">
            ...
            <div class="top-container__real-value">{{ this.getFCLocationSpeed(this.$store.state.trtIndMixing.selectedFCLocation).step2['1'] | numFormat('0.00') }}</div>
            <div class="top-container__real-unit">RPM</div>
          </div>
          <div class="top-two-contents-value">
            <div class="top-two-contents-value__text">· #2</div>
            <div class="top-two-contents-value__value">{{ this.getAIFCLocationSpeed(this.$store.state.trtIndMixing.selectedFCLocation).step2['1'] | numFormat('0.00') }}</div>
            <div class="top-two-contents-value__unit">RPM</div>
          </div>
          <div class="contents-value-underbar"></div>
          <div class="top-container__real-box">
            <div class="top-container__real-text">#3 현재 응집기 속도</div>
            <div class="top-container__real-value">{{ this.getFCLocationSpeed(this.$store.state.trtIndMixing.selectedFCLocation).step3['1'] | numFormat('0.00') }}</div>
            <div class="top-container__real-unit">RPM</div>
          </div>
          <div class="top-two-contents-value">
            <div class="top-two-contents-value__text">· #3</div>
            <div class="top-two-contents-value__value">{{ this.getAIFCLocationSpeed(this.$store.state.trtIndMixing.selectedFCLocation).step3['1'] | numFormat('0.00') }}</div>
            <div class="top-two-contents-value__unit">RPM</div>
          </div>
        </div>
      </div>
...
import { PUT_TRTINDMIXING_CONTROL_AI } from '@/store/modules/trtIndMixing'
...
export default {
  ...
  methods: {
     ...
     */
     ctrChk: function () {
      if(this.$store.state.login.user.tkn !== null) {
        this.$store.dispatch('dialog/' + OPEN_AI_MODE_DIALOG, 'trtIndMixing')
      }
    },
    /**
     * 선택한 지의 속도 반환
    ...
    updateControl: function() {      
      ...
      if (this.$store.state.trtIndMixing.isModifyMode) {
        if (this.$store.state.trtIndMixing.latestModify.d_g_value1 === ''
        ...
        } else if (parseInt(this.$store.state.trtIndMixing.latestModify.d_g_value1) <= min 
                  || parseInt(this.$store.state.trtIndMixing.latestModify.d_g_value1) >= max
                  || parseInt(this.$store.state.trtIndMixing.latestModify.d_g_value2) <= min
                  || parseInt(this.$store.state.trtIndMixing.latestModify.d_g_value2) >= max
                  || parseInt(this.$store.state.trtIndMixing.latestModify.d_g_value3) <= min
                  || parseInt(this.$store.state.trtIndMixing.latestModify.d_g_value3) >= max) {
          ...
          this.$store.dispatch(PUT_TRTINDMIXING_CONTROL_AI, obj)
          this.$store.state.trtIndMixing.isModifyMode = !this.$store.state.trtIndMixing.isModifyMode
        }
      } else {
        this.$store.state.trtIndMixing.isModifyMode = !this.$store.state.trtIndMixing.isModifyMode
    ...
    onClickDetail: function() { 
      this.$store.state.dialog.autoModeCrt.visible = true
      this.$store.state.dialog.autoModeCrt.processStep = this.$store.state.trtIndMixing.processStep
      this.$store.state.dialog.autoModeCrt.d_g_step1_crt = this.$store.state.trtIndMixing.latestModify.d_g_step1_crt.toFixed(2)
      this.$store.state.dialog.autoModeCrt.d_g_step2_crt = this.$store.state.trtIndMixing.latestModify.d_g_step2_crt.toFixed(2)
      this.$store.state.dialog.autoModeCrt.d_g_step3_crt = this.$store.state.trtIndMixing.latestModify.d_g_step3_crt.toFixed(2)
    }  
  },
  /**
   * 마운트 해제시 
  ...
  // watch: {
  //   // 교반강도 #1
  //   '$store.state.trtIndMixing.latestModify.d_g_value1': function(newVal, oldVal) {
  //     if (newVal === '') {
  ...
  //   },
  //   // 교반강도 #2
  //   '$store.state.trtIndMixing.latestModify.d_g_value2': function(newVal, oldVal) {
  //     if (newVal === '') {
  ...
  //   },
  //   // 교반강도 #3
  //   '$store.state.trtIndMixing.latestModify.d_g_value3': function(newVal, oldVal) {
  //     if (newVal === '') {
  ...
  .top-two-contents-container{
    ...
    .top-container{
      ...
      .contents-value-underbar {
        ...
        height: 6px;
        margin: 10px 0;
        background: url(../../assets/as_images/underbar.png) no-repeat;
      }
      &__real-box {
        display: flex;
        align-items: center;
