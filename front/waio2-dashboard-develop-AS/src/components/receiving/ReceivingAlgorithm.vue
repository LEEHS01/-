<template>
  <div class="main">
      ...
      <div class="right">
        ...
        <div class="right-value">
          <div class="right-value__title">
            ...
            <div v-if="this.$store.state.receiving.processStep != 1" class="right-value__text">개도율 변화<span>(%)</span></div>
          </div>
          <div class="right-value__num">
            ...
            <!-- 개도율 변화 최대 -->
            <div v-if="this.$store.state.receiving.processStep != 1" class="right-value__input-box">
              <input v-if="this.$store.state.receiving.isModifyMode" type="text" 
              :value="this.$store.state.receiving.latestModify.b_valve_max" v-on:input="updateInput($event, 'b_valve_max')"/>
              <span class="right-value__text-num" v-else>
                {{ this.$store.state.receiving.latestModify.b_valve_max | numFormat('0.0') }}
              </span>
            </div>
          </div>
          <div class="right-value__num">
            ...
            <div v-if="this.$store.state.receiving.processStep != 1" class="right-value__input-box">
              ...
              <span class="right-value__text-num" v-else>
                {{ this.$store.state.receiving.latestModify.b_valve_min | numFormat('0.0') }}
              </span>
            </div>
          </div>
    ...
    <!-- 중앙 컨텐츠 -->
    <div class="contents">
      <IndReceivingRightContents v-if="this.$store.state.receiving.processStep === 1"/>
      <ReceivingRightContents v-if="this.$store.state.receiving.processStep === 2"/>
      <TrtIndReceivingRightContents v-if="this.$store.state.receiving.processStep === 3"/>
    </div>
    <!-- 하단 차트 -->
    <div class="bottom">
      <IndReceivingHighchart v-if="this.$store.state.receiving.processStep === 1"/>
      <ReceivingHighchart v-if="this.$store.state.receiving.processStep === 2"/>
      <TrtIndReceivingHighchart v-if="this.$store.state.receiving.processStep === 3"/>
    </div>
  </div>  
</template>
<script>
...
import TrtIndReceivingRightContents from '@/components/receiving/TrtIndReceivingRightContents' 
import TrtIndReceivingHighchart from '@/components/receiving/TrtIndReceivingHighchart'
...
export default {
  ...
  components: {
    ...
    TrtIndReceivingRightContents,
    TrtIndReceivingHighchart,
    TopNavigator,
  },
  methods: {
    /**
    ...
    onClickAICheckbox: function(index, expectedValue) {
      if( this.$store.state.login.user.tkn !== null ) {
        ...
        this.$store.state.dialog.aiMode.processStep = this.$store.state.receiving.processStep
    ...
    updateControl: function() {      
      ...
      if (this.$store.state.receiving.isModifyMode) {
        ...
        } else if (parseFloat(this.$store.state.receiving.latestModify.b_valve_min) < b_min 
        || parseFloat(this.$store.state.receiving.latestModify.b_valve_min) > b_max
        || parseFloat(this.$store.state.receiving.latestModify.b_valve_max) < b_min 
        || parseFloat(this.$store.state.receiving.latestModify.b_valve_max) > b_max) {
          ...
          if (this.$store.state.receiving.processStep != 1) {
          ...
          obj.processStep = this.$store.state.receiving.processStep
  ...
  watch: {
    '$store.state.receiving.processStep': function() {
      this.dataDetail()
    }
  },
  /**
