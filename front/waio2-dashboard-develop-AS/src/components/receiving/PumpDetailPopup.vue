<template>
    <div v-if="this.$store.state.receiving.visible" class="main">
      <div class="popup-main">
        <div class="popup-contents">
          ...
          <div class="top-title">총 가동 대수 예측
            <span class="top-title__num">{{ this.$store.state.receiving.latest.ai_b_pump_cnt | numFormat('0') }}</span>
          </div>
          <div class="contents-img">
            ...
            <div class="pump-contents">
              ...
              <ul class="pump-contents__title">
                <li v-for="(item, index) in $store.state.receiving.latest.ai_b_pump_cb" :key="index" class="pump-contents__text" :class="[item === 0 ? 'off' : 'on']">
                  {{ index.split('p')[1] }}
                </li>
              </ul>
            </div>
...
<script> 
import { CLOSE_POPUP } from '@/store/modules/receiving'
export default {
  ...
  methods: {
    ...
    closePopup: function () {
      ...
      this.$store.commit('receiving/' + CLOSE_POPUP)
    },
    // fullscreen 이벤트
  ...
  watch: {
    // 팝업 열림/닫힘 값 변화 감지
    '$store.state.receiving.popup.visible': function (newVal) {
      if (newVal) {
