<template>
  <div class="main">
    ...
    <div class="btn-tab">
      <div :class="[this.$store.state.disinfection.processStep == 1 ? 'btn-tab__box-abled' : 'btn-tab__box-disabled' ]" @click="$routingByIndex(7.1)">1단계공업</div>
      <div :class="[this.$store.state.disinfection.processStep == 2 ? 'btn-tab__box-abled' : 'btn-tab__box-disabled' ]" @click="$routingByIndex(7.4)">2단계생활</div>
      <div :class="[this.$store.state.disinfection.processStep == 3 ? 'btn-tab__box-abled' : 'btn-tab__box-disabled' ]" @click="$routingByIndex(7.5)">3단계공업</div>
    </div>
    ...
    <div class="contents-wrap">
      <div class="contents-value">
        ...
        <div class="contents-value__row">
          <div class="contents-value__title">변경 한계치</div>
        ...
        <div v-if="this.$store.state.disinfection.processStep == 2"  class="contents-value__row">
          <div class="contents-value__title" style="width: 16 0px;">목표 침전지 잔류염소 대비<br>잔류염소 허용 편차</div>
          <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_e_residual_cl_holding" v-on:input="updateInput($event, 'g_e_residual_cl_holding')" maxlength="5"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_e_residual_cl_holding | numFormat('0.00') }}</span></div>
        </div>
        <div class="contents-value__row">
        ...
        <div v-if="this.$store.state.disinfection.processStep == 2"  class="contents-value__row">
          ...
          <div class="contents-value__number"><span>{{ this.$store.state.disinfection.latestModify.g_elapsed_time | numFormat('0') }}</span></div>
        </div>
      </div>
    ...
    <!-- <div class="contents-value">
      <div v-show="this.$store.state.disinfection.processStep != 2">
      ...
      <div v-show="this.$store.state.disinfection.processStep == 2">
        <div v-if="$store.state.login.user.tkn !== null" class="contents-value__icon" style="right: 5px;">
          <div class="custom-icon" @click="updateControl">
            <div :class="[ this.$store.state.disinfection.isModifyMode ? 'custom-icon__checkbox' : 'custom-icon__pencil' ]"></div>
          </div>
          <div v-if="this.$store.state.disinfection.isModifyMode" class="custom-cancel-icon" style="margin-top: 3px;" @click="cancelControl">
      ...
      <div class="contents-value__row">
        ...
        <div class="contents-value__title">보정주기</div>
        <div v-if="this.$store.state.disinfection.processStep == 2" class="contents-value__title">주입 후<br/>경과 시간</div>
      </div>
      <div class="contents-value__row">
        <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_pre_set_max" v-on:input="updateInput($event, 'g_pre_set_max')"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_pre_set_max | numFormat('0.00') }}</span></div>
        <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_pre_set_min" v-on:input="updateInput($event, 'g_pre_set_min')"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_pre_set_min | numFormat('0.00') }}</span></div>
        ...
        <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_e_obj_residual_cl" v-on:input="updateInput($event, 'g_e_obj_residual_cl')"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_e_obj_residual_cl | numFormat('0.00') }}</span></div>
        <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_pre_calib_cycle" v-on:input="updateInput($event, 'g_pre_calib_cycle')"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_pre_calib_cycle | numFormat('0') }}</span></div>      
        <div v-if="this.$store.state.disinfection.processStep == 2" class="contents-value__number"><span>{{ this.$store.state.disinfection.latestModify.g_elapsed_time | numFormat('0') }}</span></div>
      </div>
    </div> -->
  </div>  
</template>
...
export default {
  ...
  computed: {
    ...
    getDisplayStyle1() {
      return this.$store.state.disinfection.latest.g_pump_1_run === 1 ? 'block' : 'none'
    },
    getDisplayStyle2() {
      return this.$store.state.disinfection.latest.g_pump_2_run === 1 ? 'block' : 'none'
    },
  },
  methods: {
    ...
    updateControl: function() {
      ...
      if (this.$store.state.disinfection.isModifyMode) {
        if (this.$store.state.disinfection.latestModify.g_pre_set_max === ''
           ...
           || this.$store.state.disinfection.latestModify.g_e_residual_cl_holding === '') {
          ...
          obj.g_pre_calib_cycle = parseFloat(this.$store.state.disinfection.latestModify.g_pre_calib_cycle)
          obj.g_e_obj_residual_cl = parseFloat(this.$store.state.disinfection.latestModify.g_e_obj_residual_cl)
          obj.g_e_residual_cl_holding = parseFloat(this.$store.state.disinfection.latestModify.g_e_residual_cl_holding)
          ...
          if ((obj.g_pre_calib_cycle < pre_min_limit || obj.g_pre_calib_cycle > pre_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '보정주기 설정 범위', text2: pre_min_limit + ' ~ ' + pre_max_limit})
            return
          }
          if ((obj.g_e_obj_residual_cl < pre_min_limit || obj.g_e_obj_residual_cl > pre_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '침전지 목표 잔류염소 설정 범위', text2: pre_min_limit + ' ~ ' + pre_max_limit})
            return
          }
...
@keyframes arrow-two{ 
  ...
  .contents-img{
    width: 584px;
    height: 527px;
    background-image: url('../../assets/as_images/sub_07.png');
