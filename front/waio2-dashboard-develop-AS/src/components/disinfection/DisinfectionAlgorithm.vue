<template>
  <div class="main">
    ...
    <div class="top">
      <div class="title">전차염<p>세부 현황</p></div>
      <div class="right">
        <div class="right-contents">
          ...
          <div class="right-contents__btn-first">
            ...
            <div v-if="this.$store.state.disinfection.selectedDisinfectionIndex === 2" class="control_box_operation">
              ...
              <div v-if="$store.state.disinfection.latest.peri_operation_mode === 1" class="control_box_operation__btn control_box_operation__btn--on">AI추천</div>
        ...
        <!-- 시간대별 주입률 설정 팝업 -->
        <div class="modBtnWrap" v-if="this.$store.state.disinfection.selectedDisinfectionIndex === 1 && this.$store.state.disinfection.processStep == 2">
          <button class="modBtn" @click="onClickModCholRate()">시간대별 주입률 설정</button>
        </div>
      </div>
      <DisinfectionModPopup/>
    </div>
    <div class="contents">
      <div class="contents__left">
        <DisinfectionLeftContentsPre v-if="this.$store.state.disinfection.selectedDisinfectionIndex === 1" :processTitle="this.processTitle" />
        <DisinfectionLeftContentsAfter v-else-if="this.$store.state.disinfection.selectedDisinfectionIndex === 3" :processTitle="this.processTitle" />
      </div>
      <div class="contents__right">
        <DisinfectionRightContentsPre v-if="this.$store.state.disinfection.selectedDisinfectionIndex === 1" :processTitle="this.processTitle" />
        <DisinfectionRightContentsAfter v-if="this.$store.state.disinfection.selectedDisinfectionIndex === 3" :processTitle="this.processTitle" />
      </div>
    </div>
  </div>  
</template>
...
import { GET_DISINFECTION_LATEST, PUT_DISINFECTION_HISTORY_EVAPORATION, PUT_DISINFECTION_HISTORY_CHOL_RATE_PRE } from '@/store/modules/disinfection'
...
import DisinfectionModPopup from '@/components/dialog/DisinfectionModPopup'
  ...
  methods: {
    onClickAICheckbox: function(index, expectedValue, disinfectionIndex) {
      if( this.$store.state.login.user.tkn !== null ) {
        this.$store.state.selectedBuildingIndex = parseFloat(index + "." + this.$store.state.disinfection.selectedDisinfectionIndex)
        this.$store.state.dialog.aiMode.disinfectionIndex = disinfectionIndex
        this.$store.state.dialog.aiMode.processStep = this.$store.state.disinfection.processStep

        this.$store.dispatch('dialog/' + OPEN_AI_MODE_DIALOG, expectedValue)
      }
    },
    dataDetail: function() {
      this.$store.state.disinfection.pre_evaporation = this.pre_evaporation // 증발량 예측 차트
      this.$store.state.sedimentation.clTrend = this.clTrend // 정수지 잔류염소 차트
      this.$store.state.disinfection.selectedDisinfectionIndex = this.selectedDisinfectionIndex
      ...
      this.$store.state.selectedBuildingIndex = parseFloat(7 + "." + this.$store.state.disinfection.selectedDisinfectionIndex)
      this.$store.commit(SET_OVERLAY, true)
      Promise.all([
        this.$store.dispatch(GET_DISINFECTION_LATEST, this.$store.state.disinfection.selectedDisinfectionIndex, this.$store.state.disinfection.processStep),
        this.$store.dispatch(PUT_DISINFECTION_HISTORY_EVAPORATION),
        this.$store.dispatch(PUT_DISINFECTION_HISTORY_CHOL_RATE_PRE),
      ...
      this.timer = setInterval(() => {
        obj = {selectedDisinfectionIndex : this.$store.state.disinfection.selectedDisinfectionIndex
                ,processStep : this.$store.state.disinfection.processStep
              }
        this.$store.dispatch(GET_DISINFECTION_LATEST, this.$store.state.disinfection.selectedDisinfectionIndex, this.$store.state.disinfection.processStep),
        this.$store.dispatch(PUT_DISINFECTION_HISTORY_EVAPORATION),
    ...
    onClickModCholRate: function() {
      if (this.$store.state.login.user.tkn !== null) {
        this.$store.state.dialog.disinfection.visible = true
        this.$store.state.disinfection.isPopupModifyMode = true
      }
    }
  },
  created: function () {
    this.clTrend = this.$options.parent.$store.state.sedimentation.clTrend
    this.pre_evaporation = this.$options.parent.$store.state.disinfection.pre_evaporation
  },
  mounted: function() {
    this.dataDetail()
    this.$store.state.sedimentation.clTrend = this.clTrend
  },
  watch: {
    '$store.state.disinfection.processStep': function() {
      this.dataDetail()
    }
  },
  destroyed: function () {
