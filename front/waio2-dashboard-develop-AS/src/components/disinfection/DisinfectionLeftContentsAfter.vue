<template>
  <div class="main">
    ...
    <div class="btn-tab">
      <!-- <div :class="[this.$store.state.disinfection.processStep == 1 ? 'btn-tab__box-abled' : 'btn-tab__box-disabled' ]" @click="$routingByIndex(7.3)">1단계공업</div> -->
      <div :class="[this.$store.state.disinfection.processStep == 2 ? 'btn-tab__box-abled' : 'btn-tab__box-disabled' ]" @click="$routingByIndex(7.6)">2단계생활</div>
    </div>
    ...
    <div class="contents-wrap">
      <div class="contents-value">
        ...
        <div class="contents-value__row">
          ...
          <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_h_obj_residual_cl" v-on:input="updateInput($event, 'g_h_obj_residual_cl')"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_h_obj_residual_cl | numFormat('0.00') }}</span></div>
        </div>
        <div class="contents-value__row">
          <div class="contents-value__title">정수지 유입 잔류염소<br/>홀딩범위</div>
    ...
    <!-- <div class="contents-value">
      ...
      <div class="contents-value__row">
        ...
        <div class="contents-value__title">변경한계치</div>
        <div class="contents-value__title">정수지유입 목표잔류염소</div>
        <div class="contents-value__title">보정주기</div>
        <div class="contents-value__title">보정상수</div>
    ...
    updateControl: function() {
      ...
      if (this.$store.state.disinfection.isModifyMode) {
        if (this.$store.state.disinfection.latestModify.g_post_set_max === ''
           ...
           || this.$store.state.disinfection.latestModify.g_h_obj_residual_cl === ''
           || this.$store.state.disinfection.latestModify.g_post_calib_cycle === ''
           || this.$store.state.disinfection.latestModify.g_post_calib_num === ''
          ...
          obj.g_h_obj_residual_cl = parseFloat(this.$store.state.disinfection.latestModify.g_h_obj_residual_cl)
          obj.g_post_calib_cycle = parseFloat(this.$store.state.disinfection.latestModify.g_post_calib_cycle)
          obj.g_post_calib_num = parseFloat(this.$store.state.disinfection.latestModify.g_post_calib_num)
          ...
          if ((obj.g_post_calib_cycle < pre_min_limit || obj.g_post_calib_cycle > pre_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '보정주기 범위', text2: pre_min_limit + ' ~ ' + pre_max_limit})
            return
          }
          if ((obj.g_h_obj_residual_cl < cl_min_limit || obj.g_h_obj_residual_cl > cl_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '정수지유입 목표 잔류염소 설정 범위', text2: cl_min_limit + ' ~ ' + cl_max_limit})
            return
          }
