...
const PUT_ALARMS_SEARCH_DETAIL = 'PUT_ALARMS_SEARCH_DETAIL'
const ALARMS = 'alarms'
...
export default {
  ...
  state: {
    ...
    // alarm : [{"seq":202,"alm_id":132201,"alm_ty":2,"message":"2222단계 약품 AI 제어","url":"http://smart-vip:38080/cgAlgorithm","alm_ntf_ti":1711612263000,"ctrList":[{"upd_ti":1692516301000,"rnti":1711612263000,"tag_sn":"606-354-SRC-2011","tag_val":"15","tag_cmp_val":"14","kfk_flg":1,"ctr_flg":0,"processStep":0,"disinfectionIndex":0,"tag_dp":"아산(정) 생활 2단계 PACS 주입률 설정"}],"timeLeft":30,"isVisible":true}]
    critical: 4, // 임계치
    semiAuto: 2,  // 반자동
    alarmIgnore: [],
    alarmsDetail: {
  ...
  mutations: {
    ...
    [PUT_ALARMS_SEARCH_DETAIL]: function (state, data) {
        if(data.alarms === undefined && data.alarmsDetail === undefined) {
          state.alarms = []
        }
      ...
      // 알림 수신시 알람에 310초를 더함(10초를 밑에서 뺴면 5분임)
        ...
        state.alarm.forEach((it) => {
          ...
          if(it.timeLeft == 240) { //1분이 지난 알람
            if(it.isVisible){ //1분이 지날때까지 처리되지 않은 제어 알람 - alarmIgnore 추가
  ...
  actions: {
    ...
    [PUT_ALARMS_SEARCH_DETAIL]: async function({ commit, state }, obj) {
      await axios.put(`${DEV_SERVER}/${ALARMS}/${SEARCH_DETAIL}`, obj)
      .then(({ data }) => {
        data.isInit = obj.isInit
        commit(PUT_ALARMS_SEARCH_DETAIL, data)
      })
      .catch(error => {
        state.alarmsDetail = {
          ctrHisList: [],
    ...
    // 알람 설정 정보 수정 API
    [PUT_ALARM]: async function ({ commit }, { seq, dp_nm, scd_snd }) {
      await axios.put(`${DEV_SERVER}/${ALARMS}/${SETTING}` + '/' + seq, { dp_nm, scd_snd })
        .then(() => {
          commit(PUT_ALARM, { seq, dp_nm, scd_snd })
    ...
    [PUT_CONTROL]: async function ({ commit }, { alm_id, alm_ntf_ti, kfk_flg, no_action_flg}) { // eslint-disable-line no-unused-vars
      await axios.put(`${DEV_SERVER}/${URL.ALARM_CONTROL}`, { alm_id, alm_ntf_ti, kfk_flg, no_action_flg })
        .then(() => {
          // commit()
        })
        .catch(error => {
