...
const URL = {
  ...
  SENSOR_AI: 'mixing/sensors/process'
}
export const GET_MIXING_LATEST = URL.MIXING_LATEST + '/get'
export const GET_PROCESS_AI = URL.SENSOR_AI + '/get'
...
const PROCESS_AI = GET_PROCESS_AI.substr(GET_PROCESS_AI.indexOf('/') + 1)
...
export default {
  ...
  mutations: {
    ...
    [PROCESS_AI]: function (state, data) {
      state.latest = data
      if (state.isModifyMode === false) {
        state.latestModify = Object.assign({}, data)
      }
  ...
  actions: {
    ...
    [PROCESS_AI]: async function ({ commit }) {
      await axios.get(`${DEV_SERVER}/${URL.SENSOR_AI}`) // processStep 1 : 공업, 2 : 생활, 3 : 공업3차
        .then(({ data }) => {
          commit(PROCESS_AI, data.latest)
        })
        .catch(error => {
          util.printError(error)
        })
