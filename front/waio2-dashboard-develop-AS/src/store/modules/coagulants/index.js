...
export const PUT_COAGULANT_CONTROL_COAGULANT = URL.COAGULANT_CONTROL_COAGULANT + '/put'
...
export default {
  ...
  mutations: {
    ...
    [SET_MODIFYED_FROM_LATEST]: function (state) {
      console.log( state.latest.c_cf_max)
    ...
    [PUT_CONTROL_COAGULANT]: function (state, c_cf_coagulant){
      state.latest.c_cf_coagulant = c_cf_coagulant
    }
  },
  actions: {
    [GET_LATEST]: async function ({ commit, state }) {
      await axios.get(`${DEV_SERVER}/`+ 'coagulant/latest/' + state.processStep)
        .then(({ data }) => {
    ...
    [GET_RAWWATER]: async function ({ commit, state }) {
      await axios.get(`${DEV_SERVER}/${URL.COAGULANTS_RAWWATER}/` + state.processStep)
        .then(({ data }) => {
    ...
    [PUT_RAWWATER]: async function ({ commit, state }, {class_index, description, tb, ph, te, cu}) {
      await axios.put(`${DEV_SERVER}/${URL.COAGULANTS_RAWWATER}/${class_index}/` + state.processStep, {description, tb, ph ,te ,cu})
        .then(({ data }) => {
    ...
    [PUT_CONTROL_AI]: async function ({ commit, state }, { c_cf_max, c_cf_min, c_cf_coagulant, c_user_tb_e }) {
      await axios.put(`${DEV_SERVER}/`+'coagulant/control/ai/' + state.processStep, { c_cf_max, c_cf_min, c_cf_coagulant, c_user_tb_e })
      .then((response) => {
        let _data = {
          visible: true,
          title: '설정 성공',
    ...
    [PUT_CONTROL_COAGULANT]: async function ({ commit , state }, { c_cf_coagulant }) {
      await axios.put(`${DEV_SERVER}/${URL.COAGULANT_CONTROL_COAGULANT}/` + state.processStep, { c_cf_coagulant })
      .then((response) => {
        let _data = {
          visible: true,
          title: '설정 성공',
        ...
        commit(PUT_CONTROL_COAGULANT, c_cf_coagulant)
        commit('alertDialog/OPEN_DIALOG', _data, { root: true })
        console.log('date', response)
      })
      .catch(error => {
    ...
    [PUT_CONTROL_USER_CORRECT]: async function ({ commit , state }, { c_user_correct }) {
      await axios.put(`${DEV_SERVER}/${URL.COAGULANT_CONTROL_USER_CORRECT}/` + state.processStep, { c_user_correct })
      .then((response) => {
        let _data = {
          visible: true,
          title: '설정 성공',
