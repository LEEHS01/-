    ...
    alarms7DaysFormatted () {
      ...
      if (this.$store.state.alarm.alarms7Days.length > 0) {
        this.$store.state.alarm.alarms7Days.forEach((it) => {
          it.processTitle = (it.processStep == 0 ? '펌프 ' : it.processStep +  '단계 ') + this.$getProcessTitle(it.process == 'G' ? it.process + it.disinfectionIndex : it.process) 
          let selectedAlmId = it.alm_id
          let classification = it.ctrYn === 'A' ? 'auto_control' : 'user_control'
