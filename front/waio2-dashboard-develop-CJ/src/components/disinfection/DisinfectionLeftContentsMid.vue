  ...
  <div class="main">
    ...
    <div class="contents-wrap">
      <div class="contents-value">
        ...
        <div class="contents-value__row">
          <div class="contents-value__title">여과지 유출<br/>목표 잔류염소</div>
        ...
        <div class="contents-value__row">
          <div class="contents-value__title">여과지 유출<br/>잔류염소 홀딩 범위</div>
          <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_f_out_residual_cl_holding" v-on:input="updateInput($event, 'g_f_out_residual_cl_holding')" maxlength="5"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_f_out_residual_cl_holding | numFormat('0.00') }}</span></div>
        </div>
        <div class="contents-value__row">
          <div class="contents-value__title">보정주기</div>
          <div class="contents-value__number"><input v-if="this.$store.state.disinfection.isModifyMode" type="text" :value="this.$store.state.disinfection.latestModify.g_peri_calib_cycle" v-on:input="updateInput($event, 'g_peri_calib_cycle')" maxlength="5"/><span v-else>{{ this.$store.state.disinfection.latestModify.g_peri_calib_cycle | numFormat('0') }}</span></div>
        </div>
        <div class="contents-value__row">
      ...
      <div class="contents-value-icon">
        <div class="custom-icon" v-if="$store.state.login.user.tkn !== null" @click="updateControl">
    ...
    <!-- <div class="contents-value">
      ...
      <div class="contents-value__row">
        ...
        <div class="contents-value__title">여과지유출<br/>목표<br/>잔류염소</div>
        <div class="contents-value__title">여과지유출<br/>잔류염소<br/>홀딩 범위</div>
        <div class="contents-value__title">보정주기</div>
...
export default {
    ...
    updateControl: function() {
      ...
      if (this.$store.state.disinfection.isModifyMode) {
        if (this.$store.state.disinfection.latestModify.g_peri_set_max === ''
           ...
           || this.$store.state.disinfection.latestModify.g_f_out_residual_cl_holding === ''
          ) {
          ...
          obj.g_peri_calib_cycle = parseFloat(this.$store.state.disinfection.latestModify.g_peri_calib_cycle)
          obj.g_f_out_residual_cl_holding =  parseFloat(this.$store.state.disinfection.latestModify.g_f_out_residual_cl_holding)
          ...
          if ((obj.g_f_out_obj_residual_cl < cl_min_limit || obj.g_f_out_obj_residual_cl > cl_max_limit)) {
            this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '여과지 유출 목표 잔류염소 설정 범위', text2: cl_min_limit + ' ~ ' + cl_max_limit})
            return
          }
  ...
  watch: {
    ...
    //   }
    // },
    // '$store.state.disinfection.latestModify.g_f_out_residual_cl_holding': function(newVal, oldVal) {
    //   if (newVal === '') {
    //     this.$store.state.disinfection.latestModify['g_f_out_residual_cl_holding'] = newVal
    //   } else {
    //     if (this.checkNumberFormat(newVal)) {
    //       this.$store.state.disinfection.latestModify['g_f_out_residual_cl_holding'] = newVal
    //     } else {
    //       this.$store.state.disinfection.latestModify['g_f_out_residual_cl_holding'] = oldVal
    //     }
    //   }
    // },
  }
