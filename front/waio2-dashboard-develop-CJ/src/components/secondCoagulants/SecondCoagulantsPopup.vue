<template>
  <div v-if="this.$store.state.secondCoagulants.popup.visible" class="main">
      ...
      <div class="popup-contents">
        ...
        <div class="record-table">
          <table class="search-table">
            ...
            <tbody>
              <template v-for="item in $store.state.secondCoagulants.popup.class_info">
...
<script>
import { PUT_COAGULANTS_RAWWATER } from '@/store/modules/secondCoagulants'
export default {
  data: () => ({
  }),
  methods: {
    // X 버튼 클릭시 수정 팝업을 닫음
    closePopup: function () {
      this.$store.commit('secondCoagulants/CLOSE_POPUP')
    },
    /**
     * 수정 버튼 클릭시
     * 변경 모드로 전환한다
    ...
    clickModifyBtn: function(class_index) {
      let elem = {}
      elem.class_index = this.$store.state.secondCoagulants.popup.class_info[class_index - 1].class_index
      elem.cu = this.$store.state.secondCoagulants.popup.class_info[class_index - 1].cu
      elem.description = this.$store.state.secondCoagulants.popup.class_info[class_index - 1].description
      elem.ph = this.$store.state.secondCoagulants.popup.class_info[class_index - 1].ph
      elem.tb = this.$store.state.secondCoagulants.popup.class_info[class_index - 1].tb
      elem.te = this.$store.state.secondCoagulants.popup.class_info[class_index - 1].te
      elem.isModify = true
      this.$store.state.secondCoagulants.popup.class_info[class_index - 1] = elem
      this.$store.state.secondCoagulants.popup.class_info = this.$store.state.secondCoagulants.popup.class_info.slice(0)
    },
    /**
     * 수정 완료 버튼 클릭시
    ...
    clickOkBtn: function(class_index) {
      this.$store.state.secondCoagulants.popup.class_info.map(it => {
        if (it.class_index === class_index) {
          it.isModify = false
        }
      })
      this.$store.state.secondCoagulants.class_info = this.$store.state.secondCoagulants.popup.class_info
      let elem = this.$store.state.secondCoagulants.class_info[class_index - 1]
