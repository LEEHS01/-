<template>
  <div v-if="this.$store.state.dialog.aiModeOfJi.visible" class="popup">
    <div class="popup__inner">
      ...
      <div class="popup__body" v-if="this.$route.path === '/indSedimentationAlgorithm'">
        <span v-if="this.$store.state.indSedimentation.latest['e_sc_set' + this.$store.state.dialog.aiModeOfJi.number] === 0" class="popup__body--large"> AI모드에 추가하시겠습니까?</span>
        <span v-if="this.$store.state.indSedimentation.latest['e_sc_set' + this.$store.state.dialog.aiModeOfJi.number] === 1" class="popup__body--large">AI모드에서 제외하시겠습니까?</span>
      </div>
      <div class="popup__body" v-if="this.$route.path === '/sedimentationAlgorithm'">
...
import { PUT_INDSEDIMENTATION_CONTROL_LOCATION } from '../../store/modules/indSedimentation'
export default {
  data: () => ({
  }),
  methods: {
    ...
    changeAIMode: function () {
      ...
      if(this.$route.path === '/indSedimentationAlgorithm') {
        _data.ai = this.$store.state.indSedimentation.latest['e_sc_set' + this.$store.state.dialog.aiModeOfJi.number] === 0 ? 1 : 0
        this.$store.dispatch(PUT_INDSEDIMENTATION_CONTROL_LOCATION, _data)
      } else {
