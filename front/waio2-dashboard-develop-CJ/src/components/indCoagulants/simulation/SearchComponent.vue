      ...
      <div class="record-menu">
        <div class="record-menu__input-box">
          <v-menu
            ...
            <template v-slot:activator="{ on }">
              <input class="record-menu__input" type="text" v-model="$store.state.indCoagulants.simulation.formattedStartTime" v-on="on" readonly>
            </template>
            <v-date-picker v-model="$store.state.indCoagulants.simulation.formattedStartTime" no-title scrollable class="custom">
            </v-date-picker>
          </v-menu>
        </div>
        ...
        <div class="record-menu__input-box">
          <v-menu
            ...
            offset-y>
            <template v-slot:activator="{ on }">
              <input class="record-menu__input" type="text" v-model="$store.state.indCoagulants.simulation.formattedEndTime" v-on="on" readonly>
            </template>
            <v-date-picker v-model="$store.state.indCoagulants.simulation.formattedEndTime" :min="$store.state.indCoagulants.simulation.formattedStartTime" no-title scrollable class="custom">
            </v-date-picker>
          </v-menu>
        </div>
...
import { PUT_COAGULANT_SIMULATION } from '@/store/modules/indCoagulants'
export default {
  ...
  computed: {
    ...
    calcData () {
        return this.$store.state.indCoagulants.simulation.searchList
    },
  ...
  methods: {
    ...
    onClickSearchBtnReset () {
      this.$store.state.indCoagulants.simulation.formattedStartTime = moment(this.initStartTime).format('YYYY-MM-DD')
      this.$store.state.indCoagulants.simulation.formattedEndTime = moment(this.endTime).format('YYYY-MM-DD')
    ...
    onClickSearchBtn () {
      let _startTime = moment(this.$store.state.indCoagulants.simulation.formattedStartTime)
      let _endTime = moment(this.$store.state.indCoagulants.simulation.formattedEndTime)
    ...
    clickSimulationList: function (index) {
      this.$store.state.indCoagulants.simulation.searchList.map((v) => {
        if (v.simulation_index === index) {
          this.$store.state.indCoagulants.simulation.b_tb = numeral(v.b_tb).format('0.[00]')
          this.$store.state.indCoagulants.simulation.b_ph = numeral(v.b_ph).format('0.[00]')
          this.$store.state.indCoagulants.simulation.b_te = numeral(v.b_te).format('0.[00]')
          this.$store.state.indCoagulants.simulation.b_cu = numeral(v.b_cu).format('0.[00]')
          this.$store.state.indCoagulants.simulation.b_in_fr = numeral(v.b_in_fr).format('0.[00]')
          this.$store.state.indCoagulants.simulation.e1_tb = numeral(v.e1_tb).format('0.[00]')
          this.$store.state.indCoagulants.simulation.e2_tb = numeral(v.e2_tb).format('0.[00]')
          this.$store.state.indCoagulants.simulation.ai_c1_cf = v.ai_c1_cf === null ? null : numeral(v.ai_c1_cf).format('0.[00]')
          this.$store.state.indCoagulants.simulation.ai_c1_cf_coagulant = v.ai_c1_cf_coagulant
          this.$store.state.indCoagulants.simulation.ai_c2_cf = v.ai_c2_cf === null ? null : numeral(v.ai_c2_cf).format('0.[00]')
          this.$store.state.indCoagulants.simulation.ai_c2_cf_coagulant = v.ai_c2_cf_coagulant
        }
      })
    }
  },
  ...
  mounted: function () {
    ...
    // now.add('days', 1)
    this.$store.state.indCoagulants.simulation.endTime = now
    this.$store.state.indCoagulants.simulation.startTime = now.clone().subtract(6, 'd')
    this.$store.state.indCoagulants.simulation.startTime.set({ hour: 0, minute: 0, second: 0, milliseconds: 0 })
    console.log(this.$store.state.indCoagulants.simulation.startTime.format('YYYY-MM-DD HH:mm:ss'), this.$store.state.indCoagulants.simulation.endTime.format('YYYY-MM-DD HH:mm:ss'))
    this.$store.state.indCoagulants.simulation.formattedStartTime = this.$store.state.indCoagulants.simulation.startTime.format('YYYY-MM-DD')
    this.$store.state.indCoagulants.simulation.formattedEndTime = this.$store.state.indCoagulants.simulation.endTime.format('YYYY-MM-DD')

    this.initStartTime = this.$store.state.indCoagulants.simulation.startTime
    this.initEndTime = this.$store.state.indCoagulants.simulation.endTime
