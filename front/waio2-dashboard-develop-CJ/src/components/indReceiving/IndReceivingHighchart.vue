...
export default {
    ...
    return {
      ...
      chartDataRawWaterFlow: {
        ...
        xAxis: {
          ...
          tickInterval: 1000 * 60 * 60 * 48 // 48시간
        },
        yAxis: {
          title: {
            align: 'high',
  ...
  computed: {
    ...
    ChartReceivingWaterFlow: function () {
      ...
      chart.series = []
      if (this.$store.state.indReceiving.latest.ai_b_out_fr_rtd !== null) {
        let dataSeries1 = []
        for (let key in this.$store.state.indReceiving.latest.ai_b_out_fr_rtd) {
          dataSeries1.push([new Date(key).getTime(), parseInt(this.$store.state.indReceiving.latest.ai_b_out_fr_rtd[key])])
        }
        chart.series.push({
          name: '정수지 유출 유량',
          data: dataSeries1,
    ...
    ChartRawWaterFlow: function () {
      ...
      chart.series = []
      if (this.$store.state.indReceiving.latest.ai_b_in_fr_rtd !== null && Object.keys(this.$store.state.indReceiving.latest.ai_b_in_fr_rtd).length > 0) {
        let dataSeries1 = []
        for (let key in this.$store.state.indReceiving.latest.ai_b_in_fr_rtd) {
          dataSeries1.push([new Date(key).getTime(), parseInt(this.$store.state.indReceiving.latest.ai_b_in_fr_rtd[key])])
        }
        chart.series.push({
          name: '유입 유량',
          data: dataSeries1,
