            ...
            <div class="toggle-wrap">
              ...
              <div class="btn-contents" style="width: 60px;">
                <div class="btn-contents__btn">
                  <input id="authchk" type="checkbox" :disabled="this.$store.state.login.user.tkn === null" class="checkbox" :class="{ 'checked': this.$store.state.dialog.aiMode.d_g_value_ctr_flag == 1 }" true-value="1" false-value="0" v-model="$store.state.dialog.aiMode.d_g_value_ctr_flag" @click="ctrChk">
                  <label for="authchk"></label>
                </div>
              </div>
            </div>
            <div class="ji-contents-wrap">
              <p class="ji-contents-wrap__title">{{ this.$store.state.indMixing.selectedFCLocation }}지</p>
              <div class="top-one-contents-value">
                <div class="top-one-contents-value__text">#1</div>
                <input v-if="this.$store.state.indMixing.isModifyMode" type="text" class="top-one-contents-value__input" :value="this.$store.state.indMixing.latestModify[`d_g_value${$store.state.indMixing.selectedFCLocation}_1`]" v-on:input="updateInput($event, `d_g_value${$store.state.indMixing.selectedFCLocation}_1`)"/>
                <span class="top-one-contents-value__span" v-else>{{ this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_1`] | numFormat('0.0')}}</span>
                <div class="top-one-contents-value__unit">sec<sup>-1</sup></div>
              </div>
              <div class="top-one-contents-value">
                <div class="top-one-contents-value__text">#2</div>
                <input v-if="this.$store.state.indMixing.isModifyMode" type="text" class="top-one-contents-value__input" :value="this.$store.state.indMixing.latestModify[`d_g_value${$store.state.indMixing.selectedFCLocation}_2`]" v-on:input="updateInput($event, `d_g_value${$store.state.indMixing.selectedFCLocation}_2`)"/>
                <span class="top-one-contents-value__span" v-else>{{ this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_2`]  | numFormat('0.0')}}</span>
                <div class="top-one-contents-value__unit">sec<sup>-1</sup></div>
              </div>
              <div class="top-one-contents-value">
                <div class="top-one-contents-value__text">#3</div>
                <input v-if="this.$store.state.indMixing.isModifyMode" type="text" class="top-one-contents-value__input" :value="this.$store.state.indMixing.latestModify[`d_g_value${$store.state.indMixing.selectedFCLocation}_3`]" v-on:input="updateInput($event, `d_g_value${$store.state.indMixing.selectedFCLocation}_3`)"/>
                <span class="top-one-contents-value__span" v-else>{{ this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_3`]  | numFormat('0.0') }}</span>
                <div class="top-one-contents-value__unit">sec<sup>-1</sup></div>
              </div>
              <div v-if="$store.state.login.user.tkn !== null && $store.state.dialog.aiMode.d_g_value_ctr_flag == 0" class="box-config--end">
                <div v-if="this.$store.state.indMixing.isModifyMode" class="box-config--reset" @click="cancelControl"></div>
                <div v-if="this.$store.state.indMixing.isModifyMode" class="box-config--confirm" @click="updateControl"><div class="box-config--check"></div><span>확인</span></div>
                <div v-else class="box-config--confirm" @click="checkControl"><span>설정</span></div>
              </div>
              <div class="auto-btn" v-show="$store.state.login.user.tkn !== null && $store.state.dialog.aiMode.d_g_value_ctr_flag == 1" ><button class="auto-btn__detail" @click="openAutoDetail()">자동모드 상세</button></div> 
            </div>
          </div>
          <div class="top-two-contents-box">
            <div class="top-two-contents-value">
              <div class="top-two-contents-value__text">· 원수 수온</div>
              <div class="top-two-contents-value__value">{{ this.$store.state.indMixing.latest.b_te | numFormat('0.00') }}</div>
              <div class="top-two-contents-value__unit">℃</div>
            </div>
            <!-- 20260424 펜타 하자보수 시각화 수정 요청 / 온더시스 이현수/ 패들면적삭제-->
            <div class="top-two-contents-value">
              <div class="top-two-contents-value__text">· 동점성 계수</div>
            ...
            <div class="top-two-contents-value">
              ...
              <div class="top-two-contents-value__unit">m<sup>2</sup></div>
            </div>-->
            <div class="top-two-contents-value">
            ...
            <div class="top-two-contents-value">
              ...
              <div class="top-two-contents-value__value">{{ this.$store.state.indMixing.latest.d_tb_e | numFormat('0.00') }}</div>
      ...
      <div class="top-two-contents-container">
        <div class="top-two-contents-container__title">
          ...
          <div class="top-two-contents-container__timerbox-outter">
            <!-- 예측 결과 시간 -->
            <div class="timerbox">
              {{ this.$store.state.indMixing.latest.upd_ti | moment('YYYY-MM-DD HH:mm:ss') }}
            </div>
          </div>
        </div>
        <!-- 20260424 펜타 하자보수 시각화 수정 요청 / 온더시스 이현수/ hz->rpm-->
        <div class="top-container">
          <div class="top-container__real-box">
            <div class="top-container__real-text">#1 현재 응집기 속도</div>
            <div class="top-container__real-value">{{ this.getFCLocationSpeed(this.$store.state.indMixing.selectedFCLocation).step1['1'] | numFormat('0') }}</div>
          ...
          <div class="top-two-contents-value">
            ...
            <div class="top-two-contents-value__value">{{ this.getAIFCLocationSpeed(this.$store.state.indMixing.selectedFCLocation).step1['1'] | numFormat('0') }}</div>
          ...
          <div class="top-container__real-box">
            ...
            <div class="top-container__real-value">{{ this.getFCLocationSpeed(this.$store.state.indMixing.selectedFCLocation).step2['1'] | numFormat('0') }}</div>
            <!--<div class="top-container__real-unit">Hz</div>-->
            <div class="top-container__real-unit">RPM</div>
          </div>
          <div class="top-two-contents-value">
            <div class="top-two-contents-value__text">· #2</div>
            <div class="top-two-contents-value__value">{{ this.getAIFCLocationSpeed(this.$store.state.indMixing.selectedFCLocation).step2['1'] | numFormat('0') }}</div>
            <!--<div class="top-two-contents-value__unit">Hz</div>-->
            <div class="top-two-contents-value__unit">RPM</div>
          </div>
          <div class="contents-value-underbar"></div>
          <div class="top-container__real-box">
            ...
            <div class="top-container__real-value">{{ this.getFCLocationSpeed(this.$store.state.indMixing.selectedFCLocation).step3['1'] | numFormat('0') }}</div>
            <!--<div class="top-container__real-unit">Hz</div>-->
            <div class="top-container__real-unit">RPM</div>
          </div>
          <div class="top-two-contents-value">
            <div class="top-two-contents-value__text">· #3</div>
            <div class="top-two-contents-value__value">{{ this.getAIFCLocationSpeed(this.$store.state.indMixing.selectedFCLocation).step3['1'] | numFormat('0') }}</div>
            <!--<div class="top-two-contents-value__unit">Hz</div>-->
            <div class="top-two-contents-value__unit">RPM</div>
          </div>
        </div>
...
import { PUT_INDMIXING_CONTROL_AI } from '@/store/modules/indMixing'
import { OPEN_AI_MODE_DIALOG } from '@/store/modules/dialog'

export default {
  name: 'IndMtccRightContents',
  data: () => ({
    d_g_value1: null,
    ...
    chartDataAGSpeed: {
      ...
      tooltip: {
        valueDecimals: 2,
        xDateFormat: '%Y-%m-%d %H:%M:%S',
        // 20260424 펜타 하자보수 시각화 수정요청 / 온더시스 이현수/ HZ-> RPM로 변경
        //valueSuffix: 'Hz'
        valueSuffix: 'RPM'
      },
      xAxis: {
  ...
  computed: {
    ...
    ChartDataAGSpeedByLocation: function () {
      return this.$store.state.indMixing.fc_sp['location' + this.selectedLocation]
    },
    // 응집기 설정 속도 예측 차트 옵션
  ...
  methods: {
     ...
     */
     ctrChk: function () {
      if(this.$store.state.login.user.tkn !== null) {
        this.$store.dispatch('dialog/' + OPEN_AI_MODE_DIALOG, 'indMixing')
      }
    },
    /**
     * 선택한 지의 속도 반환
    ...
    cancelControl: function() {
      if(this.$store.state.dialog.aiMode.mixingStep !== null) {
    ...
    openAutoDetail: function(){
      ...
      this.$store.state.indMixing.isModifyMode = true
      this.$store.state.dialog.autoModeDetailPopup.mixingIndex = 1;
    },
    /**
     * 사용자 설정 업데이트 버튼 선택시 
    ...
    updateControl: function() {      
      let min = 0
      let max = 100
      if (this.$store.state.indMixing.isModifyMode) {
        if (this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_1`] === ''
          || this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_2`] === ''
          || this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_3`] === '') {
          this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '값을 입력해주세요' })
        } else if (parseInt(this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_1`]) <= min 
                  || parseInt(this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_1`]) >= max
                  || parseInt(this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_2`]) <= min
                  || parseInt(this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_2`]) >= max
                  || parseInt(this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_3`]) <= min
                  || parseInt(this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_3`]) >= max) {
          this.$store.dispatch('alertDialog/OPEN_DIALOG', { title: '경고', text1: '설정 범위', text2: min + ' ~ ' + max })
        } else {
          let obj = {}
          obj.d_g_value = this.$store.state.indMixing.selectedFCLocation
          obj.d_g_value_step1 = parseFloat(this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_1`])
          obj.d_g_value_step2 = parseFloat(this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_2`])
          obj.d_g_value_step3 = parseFloat(this.$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_3`])
          obj.d_g_value_ctr_flag = 0
    ...
    downloadExcel: function() {
      const speedPrediction = this.$store.state.indMixing.fc_sp
  ...
  watch: {
    // 교반강도 #1
    /*'$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_1`]': function(newVal, oldVal) {
      if (newVal === '') {
        this.$store.state.indMixing.latestModify['d_g_value1'] = newVal
      } else {
        if (this.checkNumber(newVal)) {
          this.$store.state.indMixing.latestModify['d_g_value1'] = newVal
        } else {
          this.$store.state.indMixing.latestModify['d_g_value1'] = oldVal
        }
      }
    },*/
    // 교반강도 #2
    /*'$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_2`]': function(newVal, oldVal) {
      if (newVal === '') {
        this.$store.state.indMixing.latestModify['d_g_value2'] = newVal
      } else {
        if (this.checkNumber(newVal)) {
          this.$store.state.indMixing.latestModify['d_g_value2'] = newVal
        } else {
          this.$store.state.indMixing.latestModify['d_g_value2'] = oldVal
        }
      }
    },*/
    // 교반강도 #3
    /*'$store.state.indMixing.latestModify[`d_g_value${this.$store.state.indMixing.selectedFCLocation}_3`]': function(newVal, oldVal) {
      if (newVal === '') {
        this.$store.state.indMixing.latestModify['d_g_value3'] = newVal
      } else {
        if (this.checkNumber(newVal)) {
          this.$store.state.indMixing.latestModify['d_g_value3'] = newVal
        } else {
          this.$store.state.indMixing.latestModify['d_g_value3'] = oldVal
        }
      }
    },*/
  }
  ...
  .top-two-contents-container{
    ...
    .top-container{
      ...
      .contents-value-underbar {
        ...
        height: 6px;
        margin-bottom: 10px;
        background: url(../../assets/cj_images/underbar.png) no-repeat;
      }
      &__real-box {
        display: flex;
        align-items: center;
...
.ji-contents-wrap {
  width: 202px;
  height: 100%;
  background-image: url(../../assets/cj_images/ji_box.png);
  padding-top: 15px;
  margin-top: 12px;
  &__title {
    text-shadow: 0 0 9px #5cafff;
    ...
    text-align: center;
    color: #fff;
    background-image: url(../../assets/cj_images/ji_title.png);
