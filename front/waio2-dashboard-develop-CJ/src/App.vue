<template>
  <v-app :class='this.$store.getters.backgroundImage'>
    ...
    <AIModeDialogOfJi/>
    <!-- 여과지 운전 모드 변경 Dialog -->
    <AIModeDialogOfFilterJi/>
    <!-- 혼화응집 교반강도 자동모드 상세 Dialog -->
    <AutoModeDetailPopup/>
    <!-- 알람 API를 결과값(type)에 따라 팝업 발생 -->
    <template v-for="(item, index) in $store.state.alarm.alarm">
      <AlarmNotifyDialog :key="index" :index="index" :seq="item.seq" :alm_ty="item.alm_ty" :alarmId="item.alm_id" :alm_ntf_ti="item.alm_ntf_ti" :url="item.url" :message="item.message" :isVisible="item.isVisible" :ctrList="item.ctrList" />
    </template>
  </v-app>
</template>
...
import AIModeDialogOfFilterJi from '@/components/dialog/AIModeDialogOfFilterJi'
