...
exports.searchQuery4 = (reqbody) => {
  ...
  query += `
    ...
    AND tagname in ('606-485-FRI-8005','606-485-FRI-8002','606-485-FRI-8009')
    GROUP BY
  `;
  if (search3 === 'h') {
    query += ' ts';
...
exports.searchQuery6 = (reqbody) => {
  ...
  query += `
    ...
    AND tagname in ('606-485-PMB-8528','606-485-PMB-8529','606-485-PMB-8530','606-485-PMB-8531','606-485-PMB-8532')
  `;
    ...
    let query = `
      ...
      (
        ...
        where ts  = (select DATE_FORMAT(max(ts),'%Y-%m-%d %H:%i:00') from TB_DATA_RAW_TAG) 
          ...
          AND tagname IN ('606-485-PRI-8001') 
        GROUP BY a.ts 
      )f,
      (
        SELECT SUM(a) '정속가동대수' 
        FROM (
          ...
          where ts  = (select DATE_FORMAT(max(ts),'%Y-%m-%d %H:%i:00') from TB_DATA_RAW_TAG) 
            ...
            AND tagname IN ('606-485-PMB-8528') 
          UNION all
          SELECT value a from TB_DATA_RAW_TAG a 
          where ts  = (select DATE_FORMAT(max(ts),'%Y-%m-%d %H:%i:00') from TB_DATA_RAW_TAG) 
            -- AND tagname IN ('745-617-PMB-4116')
            AND tagname IN ('606-485-PMB-8529')
          UNION all
          SELECT VALUE a from TB_DATA_RAW_TAG a 
          where ts  = (select DATE_FORMAT(max(ts),'%Y-%m-%d %H:%i:00') from TB_DATA_RAW_TAG) 
            -- AND tagname IN ('745-617-PMB-4119') 
            AND tagname IN ('606-485-PMB-8530') 
          UNION all
          SELECT VALUE*0.5 a from TB_DATA_RAW_TAG a 
          where ts  = (select DATE_FORMAT(max(ts),'%Y-%m-%d %H:%i:00') from TB_DATA_RAW_TAG) 
            -- AND tagname IN ('745-617-PMB-4122') 
            AND tagname IN ('606-485-PMB-8531') 
          UNION all
          SELECT VALUE*0.5 a from TB_DATA_RAW_TAG a 
          where ts  = (select DATE_FORMAT(max(ts),'%Y-%m-%d %H:%i:00') from TB_DATA_RAW_TAG) 
            -- AND tagname IN ('745-617-PMB-4122') 
            AND tagname IN ('606-485-PMB-8532') 
        )t        
      )g,
      (
      ...
      --		AND tagname IN ('745-617-SPI-4213') 
          AND tagname IN ('606-485-SPB-9104') 
      )h,
      (
        SELECT FORMAT (sum(value) , 0) '주파수2' from TB_DATA_RAW_TAG a 
        where ts  = (select DATE_FORMAT(max(ts),'%Y-%m-%d %H:%i:00') from TB_DATA_RAW_TAG) 
          -- AND tagname IN ('745-617-CTI-4260') 
          AND tagname IN ('606-485-PMI-8010') 
      )h2,
      (
        SELECT FORMAT (sum(value) , 0) '주파수3' from TB_DATA_RAW_TAG a 
        where ts  = (select DATE_FORMAT(max(ts),'%Y-%m-%d %H:%i:00') from TB_DATA_RAW_TAG) 
          -- AND tagname IN ('745-617-CTI-4260') 
          AND tagname IN ('606-485-PMI-8026') 
      )h3,
      (
        SELECT FORMAT (sum(value) , 0) '주파수4' from TB_DATA_RAW_TAG a 
        where ts  = (select DATE_FORMAT(max(ts),'%Y-%m-%d %H:%i:00') from TB_DATA_RAW_TAG) 
          -- AND tagname IN ('745-617-CTI-4260') 
          AND tagname IN ('606-485-PMI-8045') 
      )h4,
      (
        SELECT FORMAT (sum(value) , 0) '주파수5' from TB_DATA_RAW_TAG a 
        where ts  = (select DATE_FORMAT(max(ts),'%Y-%m-%d %H:%i:00') from TB_DATA_RAW_TAG) 
          -- AND tagname IN ('745-617-CTI-4260') 
          AND tagname IN ('606-485-SPI-7001') 
      )h5
    `;
