...
exports.peakGoal = () => {
  const query = `
    ...
    FROM TB_PTR_CTR_INF
    WHERE tag = '606-354-PEAKGOAL-0001'
  `
...
exports.peakFac = (search) => {
  ...
  const query = `
    ...
    WHERE ts =  '${search}' 
  --  WHERE ts = '2023-02-28 00:00:00'
      AND tagname IN(
                  SELECT INS_TAG 
                  FROM TB_PEAK_TAG_INF 
                  -- WHERE INS_TAG NOT IN ('745-617-PWI-4100', '745-617-PWI-4070', '745-617-PWI-4150')
                  WHERE INS_TAG NOT IN ('606-354-PWI-9500', '606-354-PWI-9520', '606-354-PWI-9540')
                )
...
exports.nowPeak = () => {
  const query = `
    ...
    -- WHERE  tagname IN ( '745-617-PWI-4000','745-617-PWI-4100') 
    WHERE  tagname IN ( '606-354-PWI-9500','606-354-PWI-9520') 
      AND VALUE > 0 
